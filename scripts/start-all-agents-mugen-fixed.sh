#!/bin/bash
# Start All Agents on MUGEN - 86 Claude Code Instances (Fixed)
# ============================================================
...
for win in {15..20}; do
    ...
    send_to_window $win "claude --print 'Analyze recent code changes and suggest improvements' > /tmp/teamA-agent-${agent_num}.log 2>&1 &"
    send_to_window $win "echo '🔴 TeamA Agent $agent_num: Claude Code started (PID: \$!)'"
...
for win in {21..40}; do
    ...
    send_to_window $win "cd $PROJECT_ROOT"
    send_to_window $win "claude --print 'Review Rust code quality and suggest optimizations' > /tmp/teamB-agent-${agent_num}.log 2>&1 &"
    send_to_window $win "echo '🟢 TeamB Agent $agent_num: Claude Code started (PID: \$!)'"
...
for win in {41..60}; do
    ...
    send_to_window $win "cd $PROJECT_ROOT"
    send_to_window $win "claude --print 'Perform code review and identify potential bugs' > /tmp/teamC-agent-${agent_num}.log 2>&1 &"
    send_to_window $win "echo '🔵 TeamC Agent $agent_num: Claude Code started (PID: \$!)'"
...
for win in {61..80}; do
    ...
    send_to_window $win "cd $PROJECT_ROOT"
    send_to_window $win "claude --print 'Analyze business logic implementation and suggest improvements' > /tmp/teamD-agent-${agent_num}.log 2>&1 &"
    send_to_window $win "echo '🟡 TeamD Agent $agent_num: Claude Code started (PID: \$!)'"
...
for win in {81..100}; do
    ...
    send_to_window $win "cd $PROJECT_ROOT"
    send_to_window $win "claude --print 'Analyze project metrics and provide insights' > /tmp/teamE-agent-${agent_num}.log 2>&1 &"
    send_to_window $win "echo '🟣 TeamE Agent $agent_num: Claude Code started (PID: \$!)'"
...
echo ""
echo -e "${YELLOW}Logs:${NC} /tmp/team[A-E]-agent-*.log"
