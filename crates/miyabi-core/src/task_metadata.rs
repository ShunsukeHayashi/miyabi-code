...
impl TaskMetadataManager {
    ...
    pub fn new(project_root: &Path) -> Result<Self> {
        ...
        if !tasks_dir.exists() {
            fs::create_dir_all(&tasks_dir)
                .context("Failed to create .miyabi/tasks directory")?;
        }
    ...
    pub fn save(&self, metadata: &TaskMetadata) -> Result<PathBuf> {
        ...
        fs::write(&file_path, json)
            .context("Failed to write task metadata to disk")?;
    ...
    pub fn load(&self, task_id: &str) -> Result<TaskMetadata> {
        ...
        let json = fs::read_to_string(&file_path)
            .context("Failed to read task metadata from disk")?;
    ...
    pub fn delete(&self, task_id: &str) -> Result<()> {
        ...
        if file_path.exists() {
            fs::remove_file(&file_path)
                .context("Failed to delete task metadata")?;
        }
    ...
    pub fn get_statistics(&self) -> Result<TaskStatistics> {
        ...
        let completed_tasks: Vec<_> = tasks
            .iter()
            .filter(|t| t.duration_secs.is_some())
            .collect();
