...
impl A2AEnabled for PersonaAgent {
    ...
    async fn handle_a2a_task(&self, task: A2ATask) -> std::result::Result<A2ATaskResult, A2AIntegrationError> {
        ...
        match task.capability.as_str() {
            "analyze_personas" => {
                let product = task.input.get("product").and_then(|v| v.as_str())
                ...
                let internal_task = Task {
                    id: task.id.clone(), title: product.to_string(), description: market.to_string(),
                    task_type: miyabi_types::task::TaskType::Feature, priority: 1, severity: None, impact: None,
                    assigned_agent: Some(AgentType::PersonaAgent), dependencies: vec![], estimated_duration: Some(180),
