...
#[async_trait]
impl A2AEnabled for CRMAgent {
    fn agent_card(&self) -> A2AAgentCard {
        AgentCardBuilder::new("CRMAgent", "CRM strategy and customer relationship optimization agent")
            .version("0.1.1")
            .capability(AgentCapability {
                id: "plan_crm".to_string(),
                name: "Plan CRM Strategy".to_string(),
                description: "Create CRM strategy with segmentation, retention, and LTV maximization".to_string(),
                input_schema: Some(json!({"type": "object", "properties": {"business": {"type": "string"}, "customer_base": {"type": "string"}}, "required": ["business"]})),
                output_schema: Some(json!({"type": "object", "properties": {"crm_strategy": {"type": "object"}}})),
            })
            .build()
    }
