...
#[async_trait]
impl A2AEnabled for FunnelDesignAgent {
    fn agent_card(&self) -> A2AAgentCard {
        AgentCardBuilder::new("FunnelDesignAgent", "Funnel design and conversion optimization agent")
            .version("0.1.1")
            .capability(AgentCapability {
                id: "design_funnel".to_string(),
                name: "Design Funnel".to_string(),
                description: "Create AARRR funnel design with conversion optimization strategies".to_string(),
                input_schema: Some(json!({"type": "object", "properties": {"product": {"type": "string"}, "target": {"type": "string"}}, "required": ["product"]})),
                output_schema: Some(json!({"type": "object", "properties": {"aarrr_metrics": {"type": "object"}}})),
            })
            .build()
    }
    async fn handle_a2a_task(&self, task: A2ATask) -> std::result::Result<A2ATaskResult, A2AIntegrationError> {
        ...
        match task.capability.as_str() {
            "design_funnel" => {
                ...
                let internal_task = Task { id: task.id.clone(), title: product.to_string(), description: "Funnel design".to_string(), task_type: miyabi_types::task::TaskType::Feature, priority: 1, severity: None, impact: None, assigned_agent: Some(AgentType::FunnelDesignAgent), dependencies: vec![], estimated_duration: Some(180), status: None, start_time: None, end_time: None, metadata: None };
