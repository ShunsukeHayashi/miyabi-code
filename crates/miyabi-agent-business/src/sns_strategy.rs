        ...
        match task.capability.as_str() {
            "plan_sns" => {
                let brand = task
                    ...
                    .get("brand")
                    .and_then(|v| v.as_str())
                    .ok_or_else(|| A2AIntegrationError::TaskExecutionFailed("Missing brand".to_string()))?;
                let internal_task = Task {
                    id: task.id.clone(),
                    title: brand.to_string(),
                    description: "SNS strategy".to_string(),
                    task_type: miyabi_types::task::TaskType::Feature,
                    priority: 1,
                    severity: None,
                    impact: None,
                    assigned_agent: Some(AgentType::SNSStrategyAgent),
                    dependencies: vec![],
