...
impl Default for TaskQueueConfig {
    fn default() -> Self {
        Self { max_concurrent: 5, max_queue_size: 100 }
    }
}
...
impl TaskQueue {
    ...
    pub fn enqueue(&mut self, issue: Issue) -> Result<()> {
        ...
        if total_tasks >= self.config.max_queue_size {
            return Err(SchedulerError::InvalidConfig(format!("Queue full (max: {})", self.config.max_queue_size)));
        }
        ...
        // Create queued task
        let queued_task =
            QueuedTask { issue: issue.clone(), priority, state: TaskState::Ready, enqueued_at: chrono::Utc::now() };
    ...
    pub fn complete(&mut self, issue_number: u64) -> Result<()> {
        ...
        let _task = self
            .in_progress
            .remove(&issue_number)
            .ok_or_else(|| SchedulerError::SessionNotFound(format!("Task #{} not in progress", issue_number)))?;
...
mod tests {
    ...
    fn test_concurrent_limit() {
        let mut queue = TaskQueue::new(TaskQueueConfig { max_concurrent: 2, max_queue_size: 100 });
    ...
    fn test_queue_size_limit() {
        let mut queue = TaskQueue::new(TaskQueueConfig { max_concurrent: 5, max_queue_size: 2 });
