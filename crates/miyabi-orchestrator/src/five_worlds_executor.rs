            ...
            let handle = tokio::spawn(async move {
                ...
                let result = if enable_circuit_breaker {
                    ...
                    if let Some(breaker) = cb {
                        ...
                        let breaker_result = breaker.call(|| {
                            Box::pin(async move {
                                tokio::time::timeout(
                                    timeout,
                                ...
                                .await
                                .map_err(|_| std::io::Error::new(std::io::ErrorKind::TimedOut, "Execution timeout"))?
                            })
                        }).await;
        ...
        for (world_id, config) in world_configs {
            ...
            let result = if self.config.enable_circuit_breaker {
                ...
                if let Some(breaker) = cb {
                    ...
                    breaker.call(|| {
                        Box::pin(async move {
                            Self::execute_single_world_stub(world_id_clone, config_clone, task_clone).await
