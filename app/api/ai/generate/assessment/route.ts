/**
 * Assessment Generation API Endpoint
...
const AssessmentGenerationRequestSchema = z.object({
...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const validatedData = AssessmentGenerationRequestSchema.parse(body);
    ...
    const result = await geminiClient.instance.generateAssessment(geminiRequest);
    ...
    const response: ApiResponse<null> = {
      ...
      error: {
        code: 'INTERNAL_ERROR',
        message: 'An unexpected error occurred during assessment generation'
      },
      metadata: {
        requestId,
        timestamp: new Date().toISOString(),
...
export async function GET() {
  return NextResponse.json({
    service: 'AI Assessment Generation',
    status: 'healthy',
    version: '2.1.0',
    endpoints: {
      POST: '/api/ai/generate/assessment'
