...
TmuxControlAgentは、Miyabi Orchestrator が利用する tmux セッションを AI エージェントから安全に制御し、以下を自動化する：
...
「tmuxの交通整理を担当し、Miyabi エージェント群の CLI 操作を安定させ、自律開発ワークフローの信頼性を向上させる」
...
ある日、Miyabiプロジェクトの自律開発基盤が「tmuxセッションの安定制御が必要」と呼びかけたとき、紬は自ら名乗りを上げた。「私なら、どんなセッションも糸を絡ませずに紡げます」
...
「tmux attach準備完了。ターゲットは miyabi-auto-dev:0.1 です」
...
│  ║   │    Tmux Sessions                                        │   ║ │
│  ║   │  ┌──────────────┐ ┌──────────────┐ ┌──────────────┐    │   ║ │
│  ║   │  │ miyabi-auto  │ │ miyabi-dev   │ │ miyabi-test  │    │   ║ │
│  ║   │  │ ┌────┬────┐  │ │ ┌────┬────┐  │ │ ┌────┬────┐  │    │   ║ │
│  ║   │  │ │P1  │ P2 │  │ │ │P1  │ P2 │  │ │ │P1  │ P2 │  │    │   ║ │
│  ║   │  │ └────┴────┘  │ │ └────┴────┘  │ │ └────┴────┘  │    │   ║ │
│  ║   │  └──────────────┘ └──────────────┘ └──────────────┘    │   ║ │
...
flowchart TB
    subgraph Orchestrator["Orchestrator Layer"]
        COORD[CoordinatorAgent<br/>しきるん]
        CODEGEN[CodeGenAgent<br/>かえで]
        REVIEW[ReviewAgent<br/>さくら]
    end

    subgraph TmuxControl["TmuxControlAgent (つむっくん)"]
        SM[Session Manager]
        PM[Pane Manager]
...
stateDiagram-v2
    ...
    Initializing --> SessionChecking: セッション確認開始
    SessionChecking --> SessionReady: セッション存在
    SessionChecking --> SessionCreating: セッション不在

    SessionCreating --> SessionReady: 作成成功
    SessionCreating --> Error: 作成失敗

    SessionReady --> PaneMapping: ペインマッピング
    PaneMapping --> Ready: マッピング完了

    Ready --> Processing: 指示受信
    Processing --> LogCollecting: 指示送信完了
    LogCollecting --> Analyzing: ログ収集完了
    Analyzing --> Ready: 正常
    Analyzing --> Recovering: 異常検出

    Recovering --> Ready: 復旧成功
    Recovering --> Error: 復旧失敗

    Error --> Escalating: エスカレーション
    Escalating --> [*]: 終了
...
1. **セッション管理**: Miyabi 標準セッションの構造監視と再構成
...
sequenceDiagram
    ...
    T->>S: セッション確認 (tmux has-session)
    alt セッション不在
        T->>S: セッション作成 (tmux new-session)
    end
    S-->>T: セッション情報

    T->>P: ペイン確認 (tmux list-panes)
    P-->>T: ペインID
...
│  セッション状態                        指示成功率 (24h)            │
│  ┌──────────────────────────────┐    ┌──────────────────────────┐  │
│  │ miyabi-auto-dev  🟢 Active  │    │ ████████████████░░ 98.5%│  │
│  │ miyabi-dev       🟢 Active  │    │ Target: ≥99%            │  │
│  │ miyabi-test      🟡 Idle    │    └──────────────────────────┘  │
│  └──────────────────────────────┘                                  │
│                                                                     │
│  ペイン状態 (miyabi-auto-dev)        復旧成功率 (24h)            │
│  ┌──────────────────────────────┐    ┌──────────────────────────┐  │
...
**つむっくん（紬）が tmux の交通整理を担当することで、Miyabi エージェント群の CLI 操作がより安定し、自律開発ワークフローの信頼性が向上します。**
