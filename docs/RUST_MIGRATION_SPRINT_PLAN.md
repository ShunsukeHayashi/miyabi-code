...
| **テストカバレッジ** | 0% | 80%+ |
| **実装済みクレート** | miyabi-types (7ファイル) | 全6クレート |
...
### 🚧 Phase 3: 型定義移植完成 (1週間 - 2025-10-22)

**現状**: 7ファイル実装済み、テスト未作成

**タスク**:
1. ✅ agent.rs (24KB) - AgentType, AgentStatus, AgentConfig
2. ✅ task.rs (17KB) - Task, TaskDecomposition, TaskGroup
3. ✅ issue.rs (23KB) - Issue, IssueTraceLog, StateTransition
4. ✅ quality.rs (16KB) - QualityReport, QualityIssue
5. ✅ workflow.rs (17KB) - DAG, ExecutionPlan, ProgressStatus
6. ✅ error.rs (17KB) - MiyabiError, AgentError, EscalationError
7. ✅ lib.rs - モジュール公開
8. [ ] **tests/unit/** - 単体テスト作成（カバレッジ 80%+）
9. [ ] **tests/serde/** - シリアライゼーション検証
10. [ ] `cargo test` 全パス
...
### ⏳ Phase 4: CLI実装 (2週間 - 2025-11-05)

**タスク**:

#### 4.1 CLIコマンド実装 (miyabi-cli)
1. [ ] `miyabi init <project-name>` - 新規プロジェクト作成
   - [ ] テンプレートファイル生成 (.miyabi.yml, .github/workflows/)
   - [ ] Git初期化
   - [ ] 対話モード (dialoguer)
2. [ ] `miyabi install` - 既存プロジェクトにインストール
   - [ ] 既存ファイル検出
   - [ ] マージ処理
3. [ ] `miyabi status [--watch]` - ステータス確認
   - [ ] Issue一覧取得
   - [ ] Label状態表示
   - [ ] リアルタイム更新 (--watch)
4. [ ] `miyabi agent run <agent-type> [--issue=N]` - Agent実行
   - [ ] Agent種別検証
   - [ ] Issue番号パース
   - [ ] 実行ログ出力
