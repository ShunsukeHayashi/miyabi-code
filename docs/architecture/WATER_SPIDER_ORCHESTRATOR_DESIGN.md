...
7. [完全挙動可視化システム（VOICEVOX）](#完全挙動可視化システムvoicevox)
8. [実装Phase](#実装phase)
9. [技術スタック](#技術スタック)
...
## 完全挙動可視化システム（VOICEVOX）

### 概要

**VOICEVOX音声ナレーションシステム**により、オーケストレーター層の全45イベントを音声でリアルタイム通知し、完全な挙動透明性を実現します。

### なぜ必要か

> **"エージェントを完璧にコントロールすることで初めて、正確にこのプロジェクトの価値が発生する"**

Water Spiderオーケストレーターが非同期・並列で複雑な処理を行う中、ユーザーが「今何が起きているか」を正確に把握できることは、システムの信頼性と価値に直結します。
...
| カテゴリ | イベント数 | 主な内容 |
|---------|----------|---------|
| **5-Worlds実行** | 13イベント | 並行世界実行、Winner選出、コスト報告 |
| **サーキットブレーカー** | 8イベント | 障害保護、実行スキップ、復旧通知 |
| **動的スケーリング** | 9イベント | リソース監視、並行数調整、ボトルネック検出 |
| **フィードバックループ** | 15イベント | 反復実行、収束検知、自動改善 |
| **合計** | **45イベント** | **100%透明性** |
...
```
┌──────────────────────────────────────────────────────────┐
│ Orchestrator Rust Code (miyabi-orchestrator)             │
│                                                           │
│  ┌────────────────────────────────────────────────────┐  │
│  │ five_worlds_executor.rs                             │  │
│  │  - 5つの世界の並列実行                                │  │
│  │  - Winner決定ロジック                                │  │
│  │  - コスト計算                                        │  │
│  │                                                     │  │
│  │  Hook呼び出し例:                                     │  │
│  │  crate::hooks::notify_orchestrator_event(          │  │
│  │      "five_worlds_start",                          │  │
│  │      params                                         │  │
│  │  )                                                  │  │
│  └────────────────────────────────────────────────────┘  │
│                          ↓                                │
│  ┌────────────────────────────────────────────────────┐  │
│  │ hooks.rs (Hook統合モジュール)                         │  │
│  │                                                     │  │
│  │  pub fn notify_orchestrator_event(...)             │  │
│  │  pub fn notify_scaling_event(...)                  │  │
│  │  pub fn notify_circuit_breaker_event(...)          │  │
│  │  pub fn notify_loop_event(...)                     │  │
│  │                                                     │  │
│  │  - 非ブロッキング実行 (Command::spawn)               │  │
│  │  - エラーハンドリング完備                             │  │
│  └────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────┘
                           ↓ 非同期実行
...
│                                                           │
│  ┌────────────────────────────────────────────────────┐  │
│  │ circuit-breaker-event.sh                            │  │
│  │  - ブレーカー状態変化 (8種)                          │  │
│  │  - 障害保護・復旧通知                                 │  │
│  └────────────────────────────────────────────────────┘  │
│                                                           │
│  ┌────────────────────────────────────────────────────┐  │
│  │ feedback-loop-event.sh                              │  │
│  │  - ループ反復 (15種)                                 │  │
│  │  - 収束検知・自動改善                                 │  │
│  └────────────────────────────────────────────────────┘  │
└──────────────────────────────────────────────────────────┘
...
| 指標 | 値 |
|------|---|
| **Hook呼び出しオーバーヘッド** | ~1ms (非ブロッキング) |
| **音声生成時間** | 50-100ms (バックグラウンド) |
| **システム影響** | ほぼゼロ（<1% CPU） |
| **メモリ使用量** | ~5MB (音声キュー) |
...
```
🔊 "5つの並行世界での実行を開始するのだ！Issue #443のタスク「tonic依存追加」を、
   異なるパラメータで5回実行して、最高の結果を選ぶのだ！
   World Alpha(温度0.3、保守的)、World Beta(温度0.5、バランス型)、
   World Gamma(温度0.7、創造的)、World Delta(温度1.0、実験的)、
   World Epsilon(Claude 3.5 Sonnet、代替モデル)の5つの世界が競争するのだ！"
```
...
🔊 "Winner決定なのだ！World Betaが最高スコア95点で勝利！
   実装の品質が最も高く、テストカバレッジも85%を達成！
   他の世界の結果：World Alpha 82点、World Gamma 78点、
   World Delta 65点、World Epsilon 88点！
   World Betaのコードをメインブランチに統合するのだ！"
```

#### 動的スケーリング

```
🔊 "スケールアップなのだ！システムリソースに余裕があるから、
   並行実行数を5から6に増やすのだ！
   現在のメモリ使用率：45%、CPU使用率：32%で、まだまだ余裕があるのだ！"
```
...
1. VOICEVOX Engine起動中 (`http://localhost:50021`)
2. 環境変数設定:
   ```bash
...
- **実装サマリー**: [docs/HOOKS_INTEGRATION_COMPLETE.md](./HOOKS_INTEGRATION_COMPLETE.md)
  - 全45イベント一覧
  - 実装ファイル詳細
  - テスト結果

- **クイックスタート**: [docs/VOICEVOX_HOOKS_QUICKSTART.md](./VOICEVOX_HOOKS_QUICKSTART.md)
  - 3ステップセットアップ
  - 使用例・トラブルシューティング
  - カスタマイズガイド

- **提案書**: [docs/PERFECT_AGENT_CONTROL_PROPOSAL.md](./PERFECT_AGENT_CONTROL_PROPOSAL.md)
  - 設計思想・アーキテクチャ
  - 実装計画

- **ギャップ分析**: [docs/AGENT_CONTROL_TRANSPARENCY_GAP_ANALYSIS.md](./AGENT_CONTROL_TRANSPARENCY_GAP_ANALYSIS.md)
  - 実装前の課題分析
  - 優先度マトリックス
...
| Rustファイル | 統合数 | 主な機能 |
|------------|-------|---------|
| **five_worlds_executor.rs** | 10箇所 | 5-Worlds実行フロー全体 |
| **dynamic_scaling.rs** | 6箇所 | リソース監視・スケーリング |
| **feedback/infinite_loop.rs** | 8箇所 | フィードバックループ管理 |
| **feedback/goal_manager.rs** | 3箇所 | ゴール進化・ステータス更新 |
| **hooks.rs** | NEW | Hook呼び出しユーティリティ |

### 価値提案

1. **完全透明性**: オーケストレーター層の全挙動を音声で通知
2. **教育効果**: 初心者でもシステム動作を理解可能
3. **デバッグ支援**: 問題発生時の状況把握が容易
4. **信頼性向上**: "見えないもの"への不安を解消
5. **ゼロコスト**: パフォーマンス影響ほぼゼロ（<1% CPU）
