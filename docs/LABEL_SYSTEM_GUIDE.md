...
| 11 | **HIERARCHY** | 4 | 階層構造管理 | `🌳 hierarchy:root`, `📂 hierarchy:parent` |
...
### 1️⃣1️⃣ HIERARCHY Labels (4個) - 階層構造管理

Issue階層（親子関係）を管理し、大規模タスクを構造的に分解・追跡。

#### `🌳 hierarchy:root`
- **色**: `#8B4513` (ブラウン)
- **意味**: ルートIssue - 親を持たない最上位Issue
- **誰が付与**: IssueAgent（親Issue番号が未設定の場合）
- **用途**:
  - 大規模機能・エピックのトップレベル
  - プロジェクト全体の進捗追跡
  - 子Issueの完了率を集約表示
...
#### `📂 hierarchy:parent`
- **色**: `#0366D6` (ブルー)
- **意味**: 親Issue - 1つ以上の子Issueを持つ
- **誰が付与**: IssueAgent（子Issue作成時に自動付与）
- **処理**:
...
- **色**: `#C5DEF5` (ライトブルー)
- **意味**: 子Issue - 親Issueを持つ
- **誰が付与**: IssueAgent（`createSubIssue()`実行時）
- **処理**:
...
#### `🍃 hierarchy:leaf`
- **色**: `#28A745` (グリーン)
- **意味**: リーフIssue - 子を持たない最下層Issue
- **誰が付与**: IssueAgent（子Issueが0件の場合）
- **用途**:
  - 実装可能な最小単位のタスク
  - Agentが直接実行できるタスク
  - good-first-issueの候補
