 ...
 */
async function getClaudeLogs(lines) {
    try {
        // Find all log files
 ...
 */
async function searchClaudeLogs(query, logType) {
    try {
...
async function getLogFileList() {
    try {
        ...
        const fileInfos = await Promise.all(logFiles.map(async (file) => {
 ...
 */
async function getBackgroundShells() {
    try {
