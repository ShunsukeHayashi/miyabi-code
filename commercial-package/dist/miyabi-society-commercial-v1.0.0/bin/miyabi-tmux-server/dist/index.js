 ...
 */
async function listPanes(sessionFilter) {
    const filter = sessionFilter ? `-t ${sessionFilter}` : '-a';
    const output = await execTmux(`list-panes ${filter} -F "#{session_name}|#{window_index}|#{pane_index}|#{pane_id}|#{pane_current_command}|#{pane_current_path}"`);
    return output.split('\n').filter(Boolean).map(line => {
 ...
 */
async function sendMessage(paneId, message) {
 ...
 * Get CommHub status
 */
async function getCommHubStatus() {
    try {
        const panes = await listPanes('miyabi-orchestra');
 ...
 */
async function broadcastMessage(message, fromSource = 'MCP') {
    const sessions = await listSessions();
 ...
 * Search pane content
 */
async function searchPane(paneId, pattern) {
 ...
 */
async function getCurrentCommand(paneId) {
    ...
    try {
        switch (name) {
            ...
            case "tmux_list_panes": {
                const panes = await listPanes(args?.session);
                return {
                    content: [
                        {
                            type: "text",
            ...
            case "tmux_broadcast": {
                const { message, from_source } = args;
