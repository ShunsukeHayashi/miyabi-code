    ...
    schema: {
        data: zod_1.z.object({
            ...
            task_list: zod_1.z
                ...
                extra: zod_1.z
                    .string()
                    .describe('Extend json, the reason for the end of the task needs to be passed complete_reason field. The enumeration value and the corresponding description:- approved: agreed- rejected: rejected- node_auto_reject: Automatic rejection (due to logical judgment)- specific_rollback: return (including return to the sponsor, return to any intermediate approver)- add: and sign (add a new approver, approve with me)- add_pre: Pre-approver (add new approver, approve before me)- add_post: post-approver (add new approver, approve after me)- delete_assignee: Reductions- forward: manual transfer- forward_resign: automatic transfer upon resignation- recall: revocation (withdrawal of documents, invalidation of documents)- delete: delete the approval slip- admin_forward: the administrator operates the forwarding in the background- system_forward: the system automatically transfers- auto_skip: Auto Pass- manual_skip: manually skip- submit_again: resubmit tasks- restart: restart the process- others: others (as a backstop)')
                    .optional(),
                create_time: zod_1.z.string().describe('Task creation time, Unix millisecond timestamp'),
                end_time: zod_1.z
                    .string()
                    .describe('Task completion time: 0 for unfinished approvals, Unix millisecond timestamp'),
                update_time: zod_1.z
                    .string()
                    .describe('The last update time of the task, which is used to push data versioning; update the policy update_time in the same instance')
                    .optional(),
                action_context: zod_1.z
                    .string()
                    .describe('Operation context, when the user operates, this parameter is included in the callback request to pass the context data of the task')
                    .optional(),
                action_configs: zod_1.z
                    .array(zod_1.z.object({
                    action_type: zod_1.z
                        .string()
                        .describe('Operation type, each task can be configured with 2 operations, which will be displayed in the approval list. When the user operates, the callback request will bring this field, indicating whether the user has agreed to the operation or refused the operation** Optional values are **:< Md-enum >< md-enum-item key = "APPROVE" > Agree </md-enum-item >< md-enum-item key = "REJECT" > Deny </md-enum-item >< Md-enum-item key = "{KEY}" > Any string, if any string is used, you need to provide action_name </md-enum-item ></Md-enum >'),
                    action_name: zod_1.z
                        .string()
                        .describe('Action name. If `action_type` is not equal to `APPROVAL` or `REJECT`, this field must be provided to display the specific action name.**Notes**:- The value passed here is an internationalization text Key (i.e., the Key in the `i18n_resources.texts` parameter), which needs to be assigned in the `i18n_resources.texts` parameter in a Key:Value format.- The Key must start with `@i18n@`')
                        .optional(),
                    is_need_reason: zod_1.z
                        .boolean()
                        .describe('Whether approval comments are required. When the value is `true`, after the approver operates on the task in the Approval Center, they will need to provide approval comments')
                        .optional(),
                    is_reason_required: zod_1.z.boolean().describe('Approval comment is required').optional(),
                    is_need_attachment: zod_1.z
                        .boolean()
                        .describe('Opinion whether to support uploading attachments')
                        .optional(),
                }))
                    .describe('Quick action configuration at the task level.**Note**: Shortcut keys are currently only supported on the Feishu mobile side')
                    .optional(),
                display_method: zod_1.z
                    .enum(['BROWSER', 'SIDEBAR', 'NORMAL', 'TRUSTEESHIP'])
                    .describe('How to open approval tasks on the list page Options:BROWSER(Jump to the default browser of the system to open),SIDEBAR(Sidbar Feishu middle side drawer open),NORMAL(Feishu inline page opens),TRUSTEESHIP(Open in hosted mode)')
                    .optional(),
                exclude_statistics: zod_1.z
                    .boolean()
                    .describe('Tripartite mission support is not included in efficiency statistics.- false: Include efficiency statistics.- true: efficiency statistics are not included')
                    .optional(),
                node_id: zod_1.z
                    .string()
                    .describe('Approval node ID. It must meet the following conditions:- Within an approval process, each node ID must be unique. For example, in a process, the node IDs for direct superior, senior superior, etc., must be different.- Within the same third-party approval definition, the node ID should remain consistent across different approval instances for the same nodes. For example, if User A and User B both initiate leave requests, the node ID for the direct superior node should be consistent in both approval instances')
                    .optional(),
                node_name: zod_1.z
                    .string()
                    .describe('Node name.**Notes**:- The value passed here is an internationalization text Key (i.e., the Key in the `i18n_resources.texts` parameter), which needs to be assigned in the `i18n_resources.texts` parameter in a Key:Value format.- The Key must start with `@i18n@`')
                    .optional(),
            }))
                .describe('Task list')
                .optional(),
            cc_list: zod_1.z
                .array(zod_1.z.object({
                cc_id: zod_1.z.string().describe('Unique identity within the approval instance'),
                user_id: zod_1.z
                    .string()
                    .describe("CC recipient's `user_id`. For the method of obtaining, please refer to .**Note**: At least one of the CC recipient's `open_id` and `user_id` must be provided")
                    .optional(),
                open_id: zod_1.z
                    .string()
                    .describe("CC recipient's `open_id`. For the method of obtaining, please refer to .**Note**: At least one of the CC recipient's `open_id` and `user_id` must be provided")
                    .optional(),
                links: zod_1.z
                    .object({
                    pc_link: zod_1.z
                        .string()
                        .describe('The jump link on the pc side, when the user uses the Feishu pc side, use this field to jump'),
                    mobile_link: zod_1.z
                        .string()
                        .describe('Third-party approval instance redirection link for mobile.**Note**:- When users view approval CC on Feishu mobile, this field is used for redirection.- At least one of `pc_link` and `mobile_link` must be provided')
                        .optional(),
                })
                    .describe("Approval CC redirection link. The configured link is used to redirect when clicking within the **CC'd** list in the approval center, taking you back to the third-party approval system to view approval CC details"),
                read_status: zod_1.z
                    .enum(['READ', 'UNREAD'])
                    .describe('Read status, null value means read unread is not supported. Options:READ(read),UNREAD(unread)'),
                extra: zod_1.z
                    .string()
                    .describe('Extended fields. Use JSON format, and the values need to be compressed and escaped into a string when transmitting')
                    .optional(),
                title: zod_1.z.string().describe('CC task name').optional(),
                create_time: zod_1.z.string().describe('CC initiation time, Unix millisecond timestamp'),
                update_time: zod_1.z
                    .string()
                    .describe('CC latest update time, used for pushing data versions. If the `update_mode` value is `UPDATE`, the approval instance information in the approval center will only be updated when the transmitted `update_time` changes (increases)'),
                display_method: zod_1.z
                    .enum(['BROWSER', 'SIDEBAR', 'NORMAL', 'TRUSTEESHIP'])
                    .describe('How to open approval tasks on the list page Options:BROWSER(Jump to the default browser of the system to open),SIDEBAR(Feishu middle side drawer open),NORMAL(Feishu inline page opens),TRUSTEESHIP(Open in hosted mode)')
                    .optional(),
            }))
                .describe('CC list')
                .optional(),
            i18n_resources: zod_1.z
                ...
                locale: zod_1.z
                    ...
                    .describe('Language Options:zh-CN(Zhcn Chinese),en-US(Enus English),ja-JP(Jajp Japanese)'),
                texts: zod_1.z
                    .array(zod_1.z.object({ key: zod_1.z.string().describe('Copywriting key'), value: zod_1.z.string().describe('copywriting') }))
                    .describe("Copy Key:Value. Keys need to start with `@i18n@` and pass Values according to the requirements of each parameter. This field is mainly used for internationalization, allowing users to input copies in multiple languages simultaneously. The approval center will use the corresponding copy according to the user's current language environment. If the copy for the user's current language environment is not provided, the default language copy will be used"),
                is_default: zod_1.z
                    .boolean()
                    .describe('Whether the default language, the default language needs to include all keys, non-default language If the key does not exist, the default language will be used instead'),
            }))
                .describe('International copywriting'),
            trusteeship_url_token: zod_1.z
                .string()
                .describe('Document escrow authentication token, the escrow callback will be accompanied by this token to help the business party authenticate')
                .optional(),
            trusteeship_user_id_type: zod_1.z
                .string()
                .describe('The type of user, which affects the selection of the user identity field of the request parameter, including the target user returned by the signature operation. Currently only "user_id" is supported')
                .optional(),
            trusteeship_urls: zod_1.z
                .object({
                form_detail_url: zod_1.z.string().describe('Get the url address of the data related to the form schema').optional(),
                action_definition_url: zod_1.z
                    .string()
                    .describe('Represents the url address for getting approval action area data')
                    .optional(),
                approval_node_url: zod_1.z.string().describe('Get the url address of the approval record related data').optional(),
                action_callback_url: zod_1.z
                    .string()
            ...
            trusteeship_cache_config: zod_1.z
                ...
                form_policy: zod_1.z
                    ...
                    .describe('Managed pre-cache policy Options:DISABLE(Not enabled, default),IMMUTABLE(Forms do not change with the process),BY_NODE(ByNode Update cache following process node changes),BY_USER(ByUser Store one copy for each to-do task)')
                    .optional(),
                form_vary_with_locale: zod_1.z.boolean().describe('Whether the form changes with internationalization').optional(),
                form_version: zod_1.z
                    .string()
                    .describe('The version number of the currently used form, ensure that after the form is changed, the version number is increased, and the actual value is an int64 integer')
                    .optional(),
            })
...
exports.approvalV4ExternalTaskList = {
    ...
    schema: {
        data: zod_1.z.object({
            approval_codes: zod_1.z
                .array(zod_1.z.string())
                .describe('Approval definition codes, which are used to specify that only the data under these definitions will be obtained')
                .optional(),
            instance_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('Approval instance IDs, which are used to specify that only the data under these instances will be obtained. Up to 20 IDs are supported')
                .optional(),
            user_ids: zod_1.z
                .array(zod_1.z.string())
...
exports.approvalV4InstanceAddSign = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            approval_code: zod_1.z.string(),
            instance_code: zod_1.z.string(),
            task_id: zod_1.z.string(),
            comment: zod_1.z.string().optional(),
            add_sign_user_ids: zod_1.z.array(zod_1.z.string()),
    ...
    schema: {
        data: zod_1.z.object({
            ...
            instance_code: zod_1.z
                .string()
                .describe('Approval instance Code. Acquisition methods:- After , obtain the approval instance code from the returned results.- Call  to get the approval instance code within the specified approval definition.- Call  and set filtering criteria to query the specified approval instance code'),
            user_id: zod_1.z
                .string()
                .describe('The user ID of the individual submitting the approval, where the ID type matches the value of the query parameter user_id_type'),
        }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional() }),
    },
};
exports.approvalV4InstanceCc = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            comment: zod_1.z.string().describe('CC comment').optional(),
        }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional() }),
    },
};
exports.approvalV4InstanceCommentCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-Approval-Native approval comments-Create comments-Create or modify comments, or comment on replies under an approval instance (excluding approved, rejected, transferred, and other additional reasons or opinions)',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            content: zod_1.z
                .string()
                .describe('Comment content in JSON format, which needs to be compressed and escaped to a string when passed in. The following example value is not escaped, you can refer to the example content in the request body to edit.**Parameter description within JSON**:- text: string type, comment text content.- files: Attachment[] type, attachment information. - url: string type, attachment link. - thumbnailURL: string type, thumbnail link. - fileSize: int64 type, file size. - title: string type, title. - type: string type, attachment type, value image indicates the image type.**Note**:- If you need to @user, set the username text in this parameter, for example `@username`, and implement the @ effect through the at_info_list parameter.- For attachments, using HTTP resource links to transfer image resources on the PC may cause thumbnail anomalies, it is recommended to use HTTPS to transfer resource attachments')
                .optional(),
            at_info_list: zod_1.z
                .array(zod_1.z.object({
                user_id: zod_1.z
                    .string()
                    .describe('The ID of the mentioned person should be consistent with the value of the query parameter user_id_type'),
                name: zod_1.z.string().describe('Name of the member mentioned'),
                offset: zod_1.z
                    .string()
                    .describe('The position of the person being mentioned in the comment, starting from 0. Used for offset override. For example:- Effect when the value is 0: @username text- Effect when the value is 2: te @username xt- Effect when the value is 4: text @username**Note**: This parameter takes effect by overwriting, so you need to first set the user name text content through the content parameter, and then use this parameter to overwrite the user name text content with the actual @effect'),
            }))
                .describe('List of members who have been mentioned in the comments (list may be empty)')
                .optional(),
            parent_comment_id: zod_1.z
                .string()
                .describe('Parent comment ID, if it is a reply comment, this value needs to be passed in. The method to obtain it:- Successfully calling the current interface will return the ID of this comment, you can save it for next use.- Call the  interface to get the comment ID')
                .optional(),
            comment_id: zod_1.z
                .string()
                .describe('Comment ID. If you need to edit or delete a comment, you need to pass the ID of that comment to the current parameter. The method to obtain it:- Successfully calling the current interface will return the ID of this comment, you can save it for next use.- Call the  interface to get the comment ID')
                .optional(),
            disable_bot: zod_1.z
                .boolean()
                .describe('Whether to disable Bot, when the value is true, only data is synchronized and Bot is not triggered.**Description**: In Feishu approval, enter false for custom approval and true for other cases')
                .optional(),
            extra: zod_1.z
                .string()
        ...
        params: zod_1.z.object({
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional(),
            user_id: zod_1.z.string().describe('The user ID should be consistent with the value of user_id_type'),
        }),
        path: zod_1.z.object({
            instance_id: zod_1.z
                .string()
                .describe('Approval instance Code. Acquisition methods:- After , obtain the approval instance code from the returned results.- Call  to get the approval instance code within the specified approval definition.- Call  and set filtering criteria to query the specified approval instance code.**Note**: Custom approval instance ID is supported'),
        }),
    },
};
exports.approvalV4InstanceCommentDelete = {
    ...
    httpMethod: 'DELETE',
    description: '[Feishu/Lark]-Approval-Native approval comments-Delete comments-Delete a comment or a reply under a specific approval instance (excluding additional reasons or opinions such as approval, rejection, transfer, etc.). After deletion, the comment will no longer be displayed in the approval instance in the approval center, but instead will show **Comment Deleted**',
    accessTokens: ['tenant'],
    schema: {
        params: zod_1.z.object({
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional(),
            user_id: zod_1.z.string().describe('Fill in user ID according to user_id_type'),
        }),
        path: zod_1.z.object({
            instance_id: zod_1.z
                .string()
                .describe('Approval instance Code. The methods to obtain it are as follows:-  and retrieve the approval instance Code from the returned result.- Call  to obtain the approval instance Code within the specified approval definition.- Call  and set filter conditions to query the specified approval instance Code.**Note**: Custom approval instance IDs are supported'),
            comment_id: zod_1.z
                .string()
                .describe('Comment ID. The methods to obtain it are as follows:- Call , and the comment ID will be returned upon successful creation.- Call  to obtain the comment ID'),
        }),
    },
};
exports.approvalV4InstanceCommentList = {
    ...
    path: '/open-apis/approval/v4/instances/:instance_id/comments',
    httpMethod: 'GET',
    description: '[Feishu/Lark]-Approval-Native approval comments-Receive comments-Obtain all comments and replies to comments under an approval instance according to Instance Code (excluding approved, rejected, transferred, and other additional reasons or opinions)',
    accessTokens: ['tenant'],
    schema: {
        params: zod_1.z.object({
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional(),
            user_id: zod_1.z.string().describe('User ID, the ID type should match the value of user_id_type'),
            page_token: zod_1.z
                .string()
                .describe('Page identifier. It is not filled in the first request, indicating traversal from the beginning; when there will be more groups, the new page_token will be returned at the same time, and the next traversal can use the page_token to get more groups')
                .optional(),
...
exports.approvalV4InstanceCommentRemove = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-Approval-Native approval comments-Clear comments-Delete all comments and replies to comments under an approval instance (excluding approved, rejected, transferred, and other additional reasons or opinions)',
    accessTokens: ['tenant'],
    schema: {
        params: zod_1.z.object({
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional(),
            user_id: zod_1.z.string().describe('Fill in user ID according to user_id_type').optional(),
        }),
        path: zod_1.z.object({
            instance_id: zod_1.z
                .string()
...
exports.approvalV4InstanceCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            user_id: zod_1.z
                .string()
                .describe("The initiator's user_id or open_id must be passed in, with priority given to user_id if both are provided. For the way to get it, refer to ")
                .optional(),
            open_id: zod_1.z
                .string()
                .describe("The initiator's open_id or user_id must be passed in, with priority given to user_id if both are provided. For the way to get it, refer to ")
                .optional(),
            department_id: zod_1.z
                .string()
                .describe("The initiator's department ID. If the user belongs to only one department, this field can be left empty. If the user belongs to multiple departments and no value is provided, the first department in the list will be selected by default. For the way to get it, refer to .**Notes**:- Filling in the root department is not supported.- The department ID should be of open_department_id type")
                .optional(),
            form: zod_1.z
                .string()
                .describe('Values of filled approval form controls must be provided as a JSON array, which needs to be compressed and escaped into a string when passed. Parameter descriptions for each control value can be found in '),
            node_approver_user_id_list: zod_1.z
                .array(zod_1.z.object({
                key: zod_1.z
                    .string()
                    .describe("The node_id or custom_node_id of the node can be obtained by calling the  API and retrieving it from the node_list parameter in the interface's response")
                    .optional(),
                value: zod_1.z
                    .array(zod_1.z.string())
            ...
            node_cc_user_id_list: zod_1.z
                ...
                key: zod_1.z
                    .string()
                    .describe("The node_id for a node can be obtained by calling the  API and retrieving it from the node_list parameter in the interface's response")
                    .optional(),
                value: zod_1.z
                    .array(zod_1.z.string())
            ...
            node_cc_open_id_list: zod_1.z
                ...
                key: zod_1.z
                    .string()
                    .describe('The node_id of a node can be obtained by calling the  API and retrieving it from the node_list parameter in the response of the API')
                    .optional(),
                value: zod_1.z
                    .array(zod_1.z.string())
                    .describe("The CC recipient list requires the user's open_id. Refer to  for the method to acquire open_ids")
                    .optional(),
            }))
            ...
            uuid: zod_1.z
                .string()
                .describe('Approval instance uuid, used for idempotent action. A single uuid can only be used to create 1 approval instance. In the case of a conflict, the error code 60012 will be returned. The format must be: XXXXXXXX-XXXX-XXXX-XXXX-XXXXXXXXXXXX (case-insensitive)')
                .optional(),
            allow_resubmit: zod_1.z
                .boolean()
                .describe('Whether the **Submit** button is configured is applicable when the approver of the task returns the approval document, allowing the approval submitter to click **Submit** within the same approval instance to submit the document again')
                .optional(),
            allow_submit_again: zod_1.z
                .boolean()
                .describe('Create an approval instance interface to support configuration whether to allow submit again or not, applicable to periodic bill of lading scenarios, where a new instance is launched again based on the current form content')
                .optional(),
            cancel_bot_notification: zod_1.z
                .string()
                .describe('Configure the bot to suppress specified notification results. Optional values:- 1: Cancel push.- 2: Cancel rejection push.- 4: Cancel the instance and cancel the push.Supports canceling a single bot notification, and also supports canceling multiple bot notifications at the same time. Bit operation, that is, if you need to cancel the two notifications 1 and 2, you need to pass in the sum value 3')
                .optional(),
            forbid_revoke: zod_1.z.boolean().describe('Configure whether revocation can be prohibited').optional(),
            i18n_resources: zod_1.z
                .array(zod_1.z.object({
                locale: zod_1.z
                    .enum(['zh-CN', 'en-US', 'ja-JP'])
            ...
            title: zod_1.z
                .string()
                .describe('Display name of the approval instance. If this parameter is filled in, the approval name in the approval list will use this parameter. If this parameter is not filled in, the approval name will use the name defined by the approval.**Explanation**: The Key for the internationalized copywriting is passed here (i.e., the Key in the i18n_resources.texts parameter). It must start with @i18n@ and also needs to be assigned in the i18n_resources.texts parameter in the format of Key:Value')
                .optional(),
            title_display_method: zod_1.z
                .number()
                .describe('Display mode for the title on the approval details page. Options:0(display_all If both the approval definition and approval instance have a title, display both, separated by a vertical line.),1(display_instance_title If both the approval definition and approval instance have a title, only display the title of the approval instance.)')
                .optional(),
            node_auto_approval_list: zod_1.z
                ...
                node_id: zod_1.z
                    .string()
                    .describe('The value of the node ID can be obtained by calling the  API. The node ID can be found in the node_list parameter of the API response')
                    .optional(),
            }))
...
exports.approvalV4InstanceGet = {
    ...
    schema: {
        params: zod_1.z.object({
            locale: zod_1.z
                .enum(['zh-CN', 'en-US', 'ja-JP'])
                .describe('Language. The default value is the language configured with the value `true` for `is_default` in the `i18n_resources` parameter when . Options:zh-CN(Zhcn Chinese),en-US(Enus English),ja-JP(Jajp Japanese)')
                .optional(),
            user_id: zod_1.z
                .string()
                .describe('The user ID of the person who initiated the approval, with the ID type specified by the `user_id_type` parameter')
                .optional(),
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional(),
        }),
        path: zod_1.z.object({
...
exports.approvalV4InstanceList = {
    ...
    schema: {
        params: zod_1.z.object({
            ...
            start_time: zod_1.z
                .string()
                .describe('The start interval for the creation time of the approval instance, in milliseconds timestamp.**Note**: start_time and end_time form the time interval query condition. The interface will return the approval instance data created within this time interval'),
            end_time: zod_1.z
                .string()
                .describe('The end interval for the creation time of the approval instance, in milliseconds timestamp.**Note**: start_time and end_time form the time interval query condition. The interface will return the Codes of the approval instances created within this time interval'),
        }),
    },
};
exports.approvalV4InstancePreview = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            instance_code: zod_1.z.string().optional(),
            locale: zod_1.z.string().optional(),
            task_id: zod_1.z.string().optional(),
        }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'user_id', 'union_id']).describe('User ID type').optional() }),
    },
};
exports.approvalV4InstanceQuery = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-Approval-Approval search-List of instances-The interface queries the list of qualified review instances in the review system through different conditions',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            user_id: zod_1.z.string().describe('Fill in the user id according to the user_id_type').optional(),
            approval_code: zod_1.z
                .string()
                .describe('Approval Instance Code. Obtaining method:- Call the  API and obtain the instance_code from the response parameters.- Call the  API to get the required approval instance code. **Note**:- user_id, approval_code, instance_code, instance_external_id, group_external_id cannot all be empty simultaneously.- Query results for instance_code and instance_external_id are a union set')
                .optional(),
            instance_code: zod_1.z
                .string()
            ...
            instance_external_id: zod_1.z
                .string()
                .describe('Third-party ID of the approval instance.**Note**:- user_id, approval_code, instance_code, instance_external_id, and group_external_id cannot all be empty simultaneously.- Query results for instance_code and instance_external_id are a union set')
                .optional(),
            group_external_id: zod_1.z
                .string()
                .describe('Third-party ID of the approval definition group.**Note**:- user_id, approval_code, instance_code, instance_external_id, and group_external_id cannot all be empty simultaneously.- Query results for approval_code and group_external_id are a union set')
                .optional(),
            instance_title: zod_1.z
                .string()
            ...
            instance_status: zod_1.z
                ...
                .describe('Approval instance status.**Default value**: ALL Options:PENDING(Under review),RECALL(Withdraw),REJECT(Reject),DELETED(Deleted),APPROVED(Approverd Pass),ALL(All states)')
                .optional(),
        ...
        params: zod_1.z.object({
            page_size: zod_1.z
                .number()
                .describe('Page size: If the current page contains revoked approval instances, the number of data entries per page in the query result might be less than the page_size value. For example, if page_size is set to 10, but the current page shows only 6 entries, it indicates that 4 entries are revoked approval instances')
                .optional(),
            page_token: zod_1.z
                .string()
                .describe('Page identifier. It is not filled in the first request, indicating traversal from the beginning; when there will be more groups, the new page_token will be returned at the same time, and the next traversal can use the page_token to get more groups')
...
exports.approvalV4InstanceSearchCc = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-Approval-Approval search-List of cc information-The interface queries the qualified review cc list in the review system through different conditions',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            user_id: zod_1.z.string().describe('Fill in the user id according to the x_user_type').optional(),
            approval_code: zod_1.z
                .string()
            ...
            instance_code: zod_1.z
                .string()
                .describe('Approval Instance Code. The retrieval methods are as follows:- Call the  interface and obtain the `instance_code` from the response parameters.- Call the  interface to get the required approval instance code.**Note**:- `user_id`, `approval_code`, `instance_code`, `instance_external_id`, and `group_external_id` cannot all be empty simultaneously.- Query results for `instance_code` and `instance_external_id` are obtained through union')
                .optional(),
            instance_external_id: zod_1.z
                .string()
                .describe('Third-party ID of Approval Instance.**Note**:- `user_id`, `approval_code`, `instance_code`, `instance_external_id`, and `group_external_id` cannot all be empty simultaneously.- Query results for `instance_code` and `instance_external_id` are obtained through union')
                .optional(),
            group_external_id: zod_1.z
                .string()
                .describe('Third-party ID of Approval Definition Group.**Note**:- `user_id`, `approval_code`, `instance_code`, `instance_external_id`, and `group_external_id` cannot all be empty simultaneously.- Query results for `approval_code` and `group_external_id` are obtained through union')
                .optional(),
            cc_title: zod_1.z.string().describe('Approval CC title (only for third-party reviews)').optional(),
            read_status: zod_1.z
                .enum(['READ', 'UNREAD', 'ALL'])
                .describe('Approval CC status Note: if not in the collection, report an error Options:READ(Read),UNREAD(Unread),ALL(All states)')
                .optional(),
            cc_create_time_from: zod_1.z
                .string()
                .describe('CC Query Start Time, Unix millisecond timestamp. Forms a time period query condition with the `cc_create_time_from` parameter, and will only return approval copies within that time period.**Note**: The query time span cannot exceed 30 days. Start and end times must both be set or both be unset')
                .optional(),
            cc_create_time_to: zod_1.z
                .string()
                .describe('CC Query End Time, Unix millisecond timestamp. Forms a time period query condition with the `cc_create_time_from` parameter, and will only return approval copies within that time period.**Note**: The query time span cannot exceed 30 days. Start and end times must both be set or both be unset')
                .optional(),
            locale: zod_1.z
                .enum(['zh-CN', 'en-US', 'ja-JP'])
                .describe('Language Options:zh-CN(ZhCn Chinese),en-US(EnUs English),ja-JP(JaJp Japanese)')
...
exports.approvalV4InstanceSpecifiedRollback = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-Approval-Approval tasks-Return approval task-Rolls back the current approval to one or more approved task nodes. After the rollback, the approved nodes will generate approval tasks again',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            user_id: zod_1.z
                .string()
                .describe('The user ID of the approver for the current approval task must match the ID type with the query parameter user_id_type. You can call  to retrieve the user ID from the task_list parameter in the returned result, where the task status must be PENDING'),
            task_id: zod_1.z
                .string()
                .describe('The approval task ID that needs to be reverted. You can call  to retrieve the task ID from the task_list parameter in the returned result, where the task status must be PENDING'),
            reason: zod_1.z.string().describe('Reason for rollback').optional(),
            extra: zod_1.z.string().describe('extra info').optional(),
            task_def_key_list: zod_1.z
                .array(zod_1.z.string())
                .describe('The node_key of the task that needs to be reverted to. You can call  to retrieve the node_key from the timeline parameter in the returned result, where the dynamic type must be PASS'),
        }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional() }),
    },
};
exports.approvalV4TaskApprove = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-Approval-Approval tasks-Agree to approval task-Approves a single approval task. After that, the approval process will be transferred to the next approver',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            approval_code: zod_1.z
            ...
            instance_code: zod_1.z
                .string()
                .describe('pproval instance Code. Acquisition methods:- After , obtain the approval instance code from the returned results.- Call  to get the approval instance code within the specified approval definition.- Call  and set filtering criteria to query the specified approval instance code'),
            user_id: zod_1.z.string().describe('Fill in the operation user id according to the user_id_type'),
            comment: zod_1.z.string().describe('Opinion').optional(),
            task_id: zod_1.z
                .string()
                .describe('Approval task ID, call  and retrieve the required ID from the `task_list` in the returned result'),
            form: zod_1.z
                .string()
                .describe('If a conditional branch is added in the process design of the approval definition, it is necessary to pass in the control data required for the conditional branch (a JSON array), otherwise, it will affect the subsequent branch condition flow.**Note**: When passing the value, the JSON must be serialized into a string. The example parameter values are not escaped; the correct example for passing values can be found in the **Request Body Example** below')
                .optional(),
        }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional() }),
    },
...
exports.approvalV4TaskQuery = {
    ...
    schema: {
        params: zod_1.z.object({
            ...
            user_id: zod_1.z.string().describe('User ID to query'),
            topic: zod_1.z
                .enum(['1', '2', '3', '17', '18'])
                .describe('Topic of task group to query, such as "Pending", "Done", etc. Options:1(TodoApproval 待办审批),2(DoneApproval 已办审批),3(InitiatedApproval 已发起审批),17(UnreadNotice 未读知会),18(ReadNotice 已读知会)'),
...
exports.approvalV4TaskReject = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-Approval-Approval tasks-Refuse to approve tasks-Rejects a single approval task. This ends the approval process',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            approval_code: zod_1.z
...
exports.approvalV4TaskResubmit = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-Approval-Approval tasks-Resubmit the task for approval-For the approval tasks that are returned to the initiator, the reinitiation operation can be performed. After initiation, the approval process will move to the next approver',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            approval_code: zod_1.z
            ...
            form: zod_1.z
                .string()
                .describe('Approval form control values in a JSON array, which need to be compressed and escaped into a string when passed. This parameter is used in the same way as the form parameter in '),
        }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('User ID type').optional() }),
    },
};
exports.approvalV4TaskSearch = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-Approval-Approval search-Query list of tasks-The interface queries the list of qualified review tasks in the review system through different conditions',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            user_id: zod_1.z.string().describe('Fill in the Approver id according to the user_id_type').optional(),
            approval_code: zod_1.z
                .string()
            ...
            instance_code: zod_1.z
                .string()
                .describe('Approval Instance Code. Methods to obtain:- After calling the  interface, retrieve from the response parameter `instance_code`.- Call the  interface to obtain the required approval instance Code.**Note**:- `user_id`, `approval_code`, `instance_code`, `instance_external_id`, and `group_external_id` cannot all be empty simultaneously.- The query results for `instance_code` and `instance_external_id` are the union of their results')
                .optional(),
            instance_external_id: zod_1.z
                .string()
                .describe('Third-party ID of the approval instance.**Note**:- `user_id`, `approval_code`, `instance_code`, `instance_external_id`, and `group_external_id` cannot all be empty simultaneously.- The query results for `instance_code` and `instance_external_id` are the union of their results')
                .optional(),
            group_external_id: zod_1.z
                .string()
                .describe('Third-party ID of the approval definition group.**Note**:- `user_id`, `approval_code`, `instance_code`, `instance_external_id`, and `group_external_id` cannot all be empty simultaneously.- The query results for `approval_code` and `group_external_id` are the union of their results')
                .optional(),
            task_title: zod_1.z.string().describe('Review task title (only for third-party reviews)').optional(),
            task_status: zod_1.z
                .enum(['PENDING', 'REJECTED', 'APPROVED', 'TRANSFERRED', 'DONE', 'RM_REPEAT', 'PROCESSED', 'ALL'])
                .describe('Review task statusNote: if not in the collection, report an error Options:PENDING(Under review),REJECTED(Reject),APPROVED(Approverd Pass),TRANSFERRED(Transfer),DONE(Completed),RM_REPEAT(Deduplicate),PROCESSED(Processed),ALL(All states)')
                .optional(),
            task_start_time_from: zod_1.z
                .string()
                .describe('Task query start time, Unix millisecond timestamp. Together with the `task_start_time_to` parameter, it forms a time period query condition, and only approval tasks within this time period will be returned.**Note**: The query time span must not exceed 30 days, and the start and end times must either be both set or both unset')
                .optional(),
            task_start_time_to: zod_1.z
                .string()
                .describe('Task query end time, Unix millisecond timestamp. Together with the `task_start_time_from` parameter, it forms a time period query condition, and only approval tasks within this time period will be returned.**Note**: The query time span must not exceed 30 days, and the start and end times must either be both set or both unset')
                .optional(),
            locale: zod_1.z
                .enum(['zh-CN', 'en-US', 'ja-JP'])
                .describe('Language. Options:zh-CN(ZhCn Chinese),en-US(EnUs English),ja-JP(JaJp Japanese)')
            ...
            task_status_list: zod_1.z
                .array(zod_1.z.string())
                .describe('Query tasks with multiple statuses. When this parameter is filled, the `task_status` parameter will become invalid.**Optional values are**:- `PENDING`: In approval- `REJECTED`: Rejected- `APPROVED`: Approved- `TRANSFERRED`: Transferred- `DONE`: Completed- `RM_REPEAT`: Deduplicated- `PROCESSED`: Processed')
                .optional(),
            order: zod_1.z
                .number()
                .describe('Sort by task time Options:0(UpdateTimeDESC Sort by approval task update time (`update_time`) in descending order.),1(UpdateTimeASC Sort by approval task update time (`update_time`) in ascending order.),2(StartTimeDESC Sort by approval task start time (`start_time`) in descending order.),3(StartTimeASC Sort by approval task start time (`start_time`) in ascending order.)')
                .optional(),
        }),
        params: zod_1.z.object({
            page_size: zod_1.z
                .number()
                .describe('Page size. If the current page contains tasks within revoked instances, the number of data entries per page in the query result may be less than the `page_size` value. For example, if `page_size` is set to 10, but the actual query result on the current page only shows 6 entries, this indicates that 4 entries are tasks within revoked instances')
                .optional(),
            page_token: zod_1.z
                .string()
                .describe('Page identifier. It is not filled in the first request, indicating traversal from the beginning; when there will be more groups, the new page_token will be returned at the same time, and the next traversal can use the page_token to get more groups')
...
exports.approvalV4TaskTransfer = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-Approval-Approval tasks-Transfer the approval task-Transfers a single approval task. After that, the approval process will be transferred to the transfer recipient',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            approval_code: zod_1.z
