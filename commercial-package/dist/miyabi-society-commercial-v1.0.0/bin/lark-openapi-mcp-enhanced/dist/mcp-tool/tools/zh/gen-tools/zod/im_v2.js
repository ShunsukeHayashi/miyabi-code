    ...
    schema: {
        data: zod_1.z.object({
            feed_cards: zod_1.z
                ...
                biz_id: zod_1.z.string().describe('业务 ID'),
                user_id: zod_1.z
                    .string()
                    .describe('用户 ID（ID 类型与 user_id_type 的取值一致。如果是商店应用，因不支持获取用户 userID 权限，所以无法值使用 user_id 类型的用户 ID）'),
            }))
                .describe('应用消息卡片')
                .optional(),
        }),
...
exports.imV2AppFeedCardBatchUpdate = {
    ...
    path: '/open-apis/im/v2/app_feed_card/batch',
    httpMethod: 'PUT',
    description: '[Feishu/Lark]-消息流-应用消息-更新应用消息流卡片-该接口用于更新消息流卡片的头像、标题、预览、标签状态、按钮等信息',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            feed_cards: zod_1.z
                ...
                app_feed_card: zod_1.z
                    ...
                    biz_id: zod_1.z
                        .string()
                        .describe('业务 ID（非必填字段，开发者可自定义业务 ID 以方便管理数据；若不传入，则 API 响应体中会返回系统自动分配的业务 ID）')
                        .optional(),
                    title: zod_1.z
                        .string()
                        .describe('主标题（在用户界面中最多展示一行，自动省略超出部分的内容；不支持定义字号及颜色）')
                        .optional(),
                    avatar_key: zod_1.z.string().describe('头像 key').optional(),
                    preview: zod_1.z
                        .string()
                        .describe('预览信息（在用户界面中最多展示一行，自动省略超出部分的内容；支持多个字段拼接、特殊符号和 emoji；不支持定义字号及颜色）')
                        .optional(),
                    status_label: zod_1.z
                        .object({
                        text: zod_1.z.string().describe('标签文字'),
                        type: zod_1.z
                    ...
                    buttons: zod_1.z
                        ...
                        buttons: zod_1.z
                            ...
                            multi_url: zod_1.z
                                .object({
                                url: zod_1.z.string().describe('默认 URL').optional(),
                                android_url: zod_1.z.string().describe('Android 平台 URL').optional(),
                                ios_url: zod_1.z.string().describe('iOS 平台 URL').optional(),
                            ...
                            action_type: zod_1.z
                                ...
                                .describe('交互类型（按钮交互方式可配置跳转 URL 页面，也可配置 webhook 回调） Options:url_page(URLPage URL 页面),webhook(回调)'),
                            text: zod_1.z.object({ text: zod_1.z.string().describe('文本') }).describe('文字'),
                            button_type: zod_1.z
                                .enum(['default', 'primary', 'success'])
                                .describe('按钮类型 Options:default(默认),primary(主要),success(成功)')
                                .optional(),
                            action_map: zod_1.z.object({}).catchall(zod_1.z.any()).describe('action 字典').optional(),
                        }))
                            .describe('按钮组合'),
                    })
                        ...
                        .optional(),
                    link: zod_1.z
                        .object({ link: zod_1.z.string().describe('卡片整体跳转链接（创建时该参数为必填参数）').optional() })
                        .describe('跳转链接')
                        .optional(),
                    ...
                    notify: zod_1.z
                        .object({
                        close_notify: zod_1.z.boolean().describe('是否关闭通知').optional(),
                        custom_sound_text: zod_1.z.string().describe('自定义语音播报文本内容（仅支持移动端）').optional(),
                ...
                update_fields: zod_1.z
                    ...
                    .describe('Options:1(TITLE 标题),2(AVATAR_KEY 头像 key),3(PREVIEW 预览),10(STATUS_LABEL 状态标签),11(BUTTONS 按钮),12(LINK 跳转链接),13(TIME_SENSITIVE 即时提醒状态),101(DISPLAY_TIME_TO_CURRENT 展示时间更新到当前),102(RERANK_TO_CURRENT 排序时间更新到当前),103(WITH_NOTIFY 进行通知)'))
                    .describe('更新字段列表'),
            }))
                .describe('应用消息卡片')
                .optional(),
...
exports.imV2AppFeedCardCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-消息流-应用消息-创建应用消息流卡片-应用消息流卡片是飞书为应用提供的消息触达能力，让应用可以直接在消息流发送消息，重要消息能更快触达用户',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            app_feed_card: zod_1.z
            ...
            user_ids: zod_1.z
                ...
                .describe('用户 ID 列表（ID 类型与 user_id_type 的取值一致。如果是商店应用，因不支持获取用户 userID 权限，所以无法值使用 user_id 类型的用户 ID）')
                .optional(),
        }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() }),
    },
...
exports.imV2BizEntityTagRelationCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            tag_biz_type: zod_1.z.literal('chat').describe('业务类型 Options:chat(chat 会话类型)'),
            biz_entity_id: zod_1.z.string().describe('业务实体 ID'),
            tag_ids: zod_1.z.array(zod_1.z.string()).describe('标签 ID').optional(),
        }),
    },
};
exports.imV2BizEntityTagRelationGet = {
    ...
    schema: {
        params: zod_1.z.object({
            tag_biz_type: zod_1.z.literal('chat').describe('业务类型 Options:chat(chat类型)'),
            biz_entity_id: zod_1.z.string().describe('业务实体id'),
        }),
    },
};
exports.imV2BizEntityTagRelationUpdate = {
    ...
    schema: {
        data: zod_1.z.object({
            tag_biz_type: zod_1.z.literal('chat').describe('业务类型 Options:chat(chat类型)'),
            biz_entity_id: zod_1.z.string().describe('业务实体id'),
            tag_ids: zod_1.z.array(zod_1.z.string()).describe('标签id').optional(),
        }),
    },
};
exports.imV2ChatButtonUpdate = {
    ...
    httpMethod: 'PUT',
    description: '[Feishu/Lark]-消息流-群聊或机器人消息-更新消息流卡片按钮-为群组消息、机器人消息的消息流卡片添加、更新、删除快捷操作按钮',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            user_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('用户 ID 列表（ID 类型与 user_id_type 的取值一致。如果是商店应用，因不支持获取用户 user ID 权限，所以无法使用 user_id 类型的用户 ID）')
                .optional(),
            chat_id: zod_1.z.string().describe('群 ID'),
            buttons: zod_1.z
                .object({
                buttons: zod_1.z
                    .array(zod_1.z.object({
                    ...
                    .describe('按钮组合，该字段为全量更新字段，若未传入字段原有值，则会清空字段数据。例如：- 在保持原有按钮的字段配置的前提下，新增一个按钮配置会添加一个按钮。- 在原有按钮的字段配置上做更新，会更新该按钮。- 清空原有按钮的字段配置，会删除该按钮'),
            })
                .describe('交互按钮（非必填字段，如未传入该字段，则不展示按钮；最多展示 2 个按钮）')
                .optional(),
        }),
...
exports.imV2FeedCardBotTimeSentive = {
    ...
    httpMethod: 'PATCH',
    description: '[Feishu/Lark]-消息流-群聊或机器人消息-机器人单聊即时提醒-可将机器人对话在消息列表中置顶展示，打开飞书首页即可处理重要任务',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            time_sensitive: zod_1.z
...
exports.imV2FeedCardPatch = {
    ...
    httpMethod: 'PATCH',
    description: '[Feishu/Lark]-消息流-群聊或机器人消息-即时提醒-即时提醒能力是飞书在消息列表中提供的强提醒能力，当有重要通知或任务需要及时触达用户，可将群组或机器人对话在消息列表中置顶展示，打开飞书首页即可处理重要任务',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            time_sensitive: zod_1.z
        ...
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型') }),
        path: zod_1.z.object({ feed_card_id: zod_1.z.string().describe('消息卡片 id，当前只支持群聊类型') }),
    },
};
exports.imV2TagCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            create_tag: zod_1.z
                .object({
                tag_type: zod_1.z.literal('tenant').describe('标签类型 Options:tenant(租户类型标签)'),
                name: zod_1.z.string().describe('标签默认名称'),
                i18n_names: zod_1.z
                    .array(zod_1.z.object({ locale: zod_1.z.string().describe('语言'), name: zod_1.z.string().describe('名称').optional() }))
...
exports.imV2TagPatch = {
    ...
    schema: {
        data: zod_1.z.object({
            patch_tag: zod_1.z
                .object({
                id: zod_1.z.string().describe('标签 ID').optional(),
                name: zod_1.z.string().describe('标签名称').optional(),
                i18n_names: zod_1.z
                    .array(zod_1.z.object({ locale: zod_1.z.string().describe('语言'), name: zod_1.z.string().describe('名称').optional() }))
        ...
        path: zod_1.z.object({ tag_id: zod_1.z.string().describe('标签 ID') }),
    },
};
exports.imV2UrlPreviewBatchUpdate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-消息-URL 预览-更新 URL 预览-该接口用于主动更新 ，调用后会重新触发一次客户端拉取，需要回调服务返回更新后的数据',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            preview_tokens: zod_1.z
                .array(zod_1.z.string())
                .describe('URL 预览的 preview_tokens 列表。需要通过回调获取 preview_tokens。**注意**：单个 token 限制更新频率为 1次/5秒'),
            open_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('需要更新 URL 预览的用户 open_id。若不传，则默认更新 URL 预览所在会话的所有成员；若用户不在 URL 所在会话，则无法触发更新该用户对应的 URL 预览结果。获取方式参见')
                .optional(),
        }),
    },
};
