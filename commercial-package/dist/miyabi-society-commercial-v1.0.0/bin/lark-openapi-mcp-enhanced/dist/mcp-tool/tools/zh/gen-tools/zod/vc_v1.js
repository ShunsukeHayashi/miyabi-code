...
exports.vcV1AlertList = {
    ...
    schema: {
        params: zod_1.z.object({
            start_time: zod_1.z.string().describe('开始时间（unix时间，单位秒）'),
            end_time: zod_1.z.string().describe('结束时间（unix时间，单位秒）'),
            query_type: zod_1.z
                .number()
                .describe('查询对象类型，不填返回所有 Options:1(room 会议室),2(erc 企业会议室连接器),3(sip SIP会议室系统)')
                .optional(),
            query_value: zod_1.z.string().describe('查询对象ID，会议室ID或企业会议室连接器ID').optional(),
            page_size: zod_1.z
                .number()
...
exports.vcV1ExportGet = {
    ...
    schema: {
        path: zod_1.z.object({ task_id: zod_1.z.string().describe('任务id').optional() }),
...
exports.vcV1ExportMeetingList = {
    ...
    schema: {
        data: zod_1.z.object({
            start_time: zod_1.z.string().describe('查询开始时间（unix时间，单位sec）'),
            end_time: zod_1.z.string().describe('查询结束时间（unix时间，单位sec）'),
            meeting_status: zod_1.z
                .number()
                .describe('会议状态（不传默认为已结束会议） Options:1(ongoing 进行中),2(past 已结束),3(future 待召开)')
                .optional(),
            meeting_no: zod_1.z.string().describe('按9位会议号筛选（最多一个筛选条件）').optional(),
            user_id: zod_1.z.string().describe('按参会Lark用户筛选（最多一个筛选条件）').optional(),
            room_id: zod_1.z.string().describe('按参会Rooms筛选（最多一个筛选条件）').optional(),
            meeting_type: zod_1.z
                .number()
                .describe('按会议类型筛选（最多一个筛选条件） Options:1(all 全部类型（默认）),2(meeting 视频会议),3(share_screen 本地投屏)')
                .optional(),
        }),
...
exports.vcV1ExportParticipantList = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-视频会议-导出-导出参会人明细-导出某个会议的参会人详情列表，具体权限要求请参考「资源介绍」',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            meeting_start_time: zod_1.z.string().describe('会议开始时间（unix时间，单位sec）'),
            ...
            meeting_status: zod_1.z
                ...
                .describe('会议状态（不传默认为已结束会议） Options:1(ongoing 进行中),2(past 已结束),3(future 待召开)')
                .optional(),
            meeting_no: zod_1.z.string().describe('9位会议号'),
            user_id: zod_1.z.string().describe('按参会Lark用户筛选（最多一个筛选条件）').optional(),
            room_id: zod_1.z.string().describe('按参会Rooms筛选（最多一个筛选条件）').optional(),
        }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() }),
...
exports.vcV1ExportParticipantQualityList = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-视频会议-导出-导出参会人会议质量数据-导出某场会议某个参会人的音视频&共享质量数据（仅支持已结束会议），具体权限要求请参考「资源介绍」',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            meeting_start_time: zod_1.z.string().describe('会议开始时间（需要精确到一分钟，unix时间，单位sec）'),
            meeting_end_time: zod_1.z.string().describe('会议结束时间（unix时间，单位sec）'),
            meeting_no: zod_1.z.string().describe('9位会议号'),
            join_time: zod_1.z.string().describe('参会人入会时间（unix时间，单位sec），可从「查询参会人明细」返回结果获取'),
            user_id: zod_1.z.string().describe('参会人为Lark用户时填入，room_id和user_id必须只填一个').optional(),
            room_id: zod_1.z.string().describe('参会人为Rooms时填入，room_id和user_id必须只填一个').optional(),
        }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
...
exports.vcV1ExportResourceReservationList = {
    ...
    schema: {
        data: zod_1.z.object({
            room_level_id: zod_1.z.string().describe('会议室层级id'),
            need_topic: zod_1.z.boolean().describe('是否展示会议主题').optional(),
            start_time: zod_1.z.string().describe('查询开始时间（unix时间，单位sec）'),
            end_time: zod_1.z.string().describe('查询结束时间（unix时间，单位sec）'),
            room_ids: zod_1.z.array(zod_1.z.string()).describe('待筛选的会议室id列表').optional(),
            is_exclude: zod_1.z.boolean().describe('若为true表示导出room_ids范围外的会议室，默认为false').optional(),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.vcV1MeetingListGet = {
    ...
    schema: {
        params: zod_1.z.object({
            ...
            meeting_status: zod_1.z
                .number()
                .describe('会议状态（不传默认为已结束会议） Options:1(ongoing 进行中),2(past 已结束),3(future 待召开。该枚举值只读，请求时不支持选择。)')
                .optional(),
            meeting_no: zod_1.z.string().describe('按9位会议号筛选（最多一个筛选条件）').optional(),
            user_id: zod_1.z.string().describe('按参会Lark用户筛选（最多一个筛选条件）').optional(),
            room_id: zod_1.z.string().describe('按参会Rooms筛选（最多一个筛选条件）').optional(),
            meeting_type: zod_1.z
                ...
                .describe('按会议类型筛选（最多一个筛选条件） Options:1(all 全部类型（默认）),2(meeting 视频会议),3(share_screen 本地投屏)')
                .optional(),
            page_size: zod_1.z.number().describe('分页尺寸大小').optional(),
            page_token: zod_1.z
                .string()
                .describe('分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果')
                .optional(),
...
exports.vcV1MeetingEnd = {
    ...
    schema: {
        path: zod_1.z.object({
            meeting_id: zod_1.z.string().describe('会议ID（视频会议的唯一标识，视频会议开始后才会产生）').optional(),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.vcV1MeetingGet = {
    ...
    schema: {
        params: zod_1.z.object({
            with_participants: zod_1.z.boolean().describe('是否需要参会人列表').optional(),
            with_meeting_ability: zod_1.z.boolean().describe('是否需要会中使用能力统计（仅限tenant_access_token）').optional(),
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional(),
        }),
        path: zod_1.z.object({
            meeting_id: zod_1.z.string().describe('会议ID（视频会议的唯一标识，视频会议开始后才会产生）').optional(),
    ...
    schema: {
        data: zod_1.z.object({
            invitees: zod_1.z
                ...
                id: zod_1.z.string().describe('用户ID').optional(),
                user_type: zod_1.z
                    .number()
                    .describe('用户类型 Options:1(lark_user 飞书用户),2(room_user rooms用户),3(doc_user 文档用户),4(neo_user neo单品用户),5(neo_guest_user neo单品游客用户),6(pstn_user pstn用户),7(sip_user sip用户)')
                    .optional(),
            }))
...
exports.vcV1MeetingKickout = {
    ...
    schema: {
        ...
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() }),
        path: zod_1.z.object({ meeting_id: zod_1.z.string().describe('会议ID').optional() }),
    },
};
exports.vcV1MeetingListByNo = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-视频会议-会议管理-获取与会议号关联的会议列表-获取指定时间范围（90天内)会议号关联的会议简要信息列表',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z.object({
            meeting_no: zod_1.z.string().describe('9位会议号'),
    ...
    schema: {
        data: zod_1.z.object({
            permission_objects: zod_1.z
                .array(zod_1.z.object({
                id: zod_1.z.string().describe('授权对象ID').optional(),
                type: zod_1.z
                    .number()
                    .describe('授权对象类型 Options:1(User 用户授权（id字段填入用户ID）),2(Group 群组授权（id字段填入群组open_chat_id）),3(Tenant 租户内授权（id字段不填）),4(Public 公网授权（id字段不填）)'),
                permission: zod_1.z.number().describe('权限 Options:1(View 查看)'),
            }))
                .describe('授权对象列表'),
            action_type: zod_1.z
                .number()
...
exports.vcV1MeetingRecordingStart = {
    ...
    accessTokens: ['user'],
    schema: {
        data: zod_1.z.object({ timezone: zod_1.z.number().describe('录制文件时间显示使用的时区[-12,12]').optional() }),
        path: zod_1.z.object({
            meeting_id: zod_1.z.string().describe('会议ID（视频会议的唯一标识，视频会议开始后才会产生）').optional(),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
...
exports.vcV1ParticipantQualityListGet = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-视频会议-会议数据-查询参会人会议质量数据-查询参会人会议质量数据（仅支持已结束会议），具体权限要求请参考「资源介绍」',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z.object({
            meeting_start_time: zod_1.z.string().describe('会议开始时间（需要精确到一分钟，unix时间，单位sec）'),
...
exports.vcV1ReportGetDaily = {
    ...
    schema: {
        params: zod_1.z.object({
            start_time: zod_1.z.string().describe('开始时间（unix时间，单位sec）'),
            end_time: zod_1.z.string().describe('结束时间（unix时间，单位sec）'),
            unit: zod_1.z
                .number()
                .describe('数据驻留地（传参前提是租户存在多个驻留地数据且开通了该查询功能） Options:0(CN 中国大陆),1(VA 美国),2(SG 新加坡),3(JP 日本)')
                .optional(),
        }),
    },
};
exports.vcV1ReportGetTopUser = {
    ...
    schema: {
        params: zod_1.z.object({
            start_time: zod_1.z.string().describe('开始时间（unix时间，单位sec）'),
            end_time: zod_1.z.string().describe('结束时间（unix时间，单位sec）'),
            limit: zod_1.z.number().describe('取前多少位'),
            order_by: zod_1.z.number().describe('排序依据（降序） Options:1(meeting_count 会议数量),2(meeting_duration 会议时长)'),
            unit: zod_1.z
                .number()
                .describe('数据驻留地（传参前提是租户存在多个驻留地数据且开通了该查询功能） Options:0(CN 中国大陆),1(VA 美国),2(SG 新加坡),3(JP 日本)')
                .optional(),
...
exports.vcV1ReserveConfigAdminGet = {
    ...
    schema: {
        params: zod_1.z.object({
            scope_type: zod_1.z.number().describe('会议室或层级'),
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional(),
        }),
        path: zod_1.z.object({ reserve_config_id: zod_1.z.string().describe('会议室或层级id').optional() }),
    },
};
exports.vcV1ReserveConfigAdminPatch = {
    ...
    schema: {
        data: zod_1.z.object({
            scope_type: zod_1.z.number().describe('1代表层级，2代表会议室'),
            reserve_admin_config: zod_1.z
                .object({
                depts: zod_1.z
                    .array(zod_1.z.object({ department_id: zod_1.z.string().describe('预定管理部门ID，使用open_department_id') }))
                    .describe('预定管理部门')
                    .optional(),
                users: zod_1.z
                    .array(zod_1.z.object({ user_id: zod_1.z.string().describe('预定管理员ID') }))
                    .describe('预定管理员')
                    .optional(),
            })
                .describe('预定管理员或部门'),
...
exports.vcV1ReserveConfigDisableInformGet = {
    ...
    schema: {
        params: zod_1.z.object({
            scope_type: zod_1.z.number().describe('1表示层级，2表示会议室'),
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional(),
        }),
        path: zod_1.z.object({ reserve_config_id: zod_1.z.string().describe('会议室或层级ID，可通过会议室或层级相关查询接口获取') }),
    },
};
exports.vcV1ReserveConfigDisableInformPatch = {
    ...
    schema: {
        data: zod_1.z.object({
            scope_type: zod_1.z.number().describe('1表示会议室层级，2表示会议室'),
            disable_inform: zod_1.z
                .object({
                if_cover_child_scope: zod_1.z.boolean().describe('是否覆盖子层级及会议室').optional(),
                if_inform: zod_1.z.boolean().describe('禁用状态变更通知开关'),
    ...
    schema: {
        data: zod_1.z.object({
            ...
            reserve_form_config: zod_1.z
                .object({
                if_cover_child_scope: zod_1.z.boolean().describe('是否覆盖子层级及会议室').optional(),
                reserve_form: zod_1.z.boolean().describe('预定表单开关，true表示打开，false表示关闭'),
                notified_users: zod_1.z
                    .array(zod_1.z.object({ user_id: zod_1.z.string().describe('预定管理员ID') }))
                    .describe('通知人列表')
                    .optional(),
                notified_time: zod_1.z.number().describe('最晚于会议开始前 notified_time收到通知（单位：分/时/天）').optional(),
                time_unit: zod_1.z.number().describe('时间单位，1为分钟；2为小时；3为天，默认为天').optional(),
            })
                .describe('预定表单设置'),
        }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() }),
...
exports.vcV1ReserveConfigPatch = {
    ...
    schema: {
        data: zod_1.z.object({
            scope_type: zod_1.z.string().describe('1 代表层级，2 代表会议室'),
            approval_config: zod_1.z
                .object({
                approval_switch: zod_1.z
                    .number()
                    .describe('预定审批开关：0 代表关闭，1 代表打开。<b>说明</b>：1. 未设置值时不更新原开关的值，但此时必填 approval_condition2. 设置值为 1 时，必填 approval_condition 3. 设置值为 0 时整个 approval_config 其他字段均可省略')
                    .optional(),
                approval_condition: zod_1.z
                    .number()
                    .describe('预定审批条件：0 代表所有预定均需审批，1 代表满足条件的需审批<b>说明</b>：为 1 时必填 meeting_duration')
                    .optional(),
                meeting_duration: zod_1.z
                    .number()
                    .describe('超过 meeting_duration的预定需要审批（单位：小时，取值范围[0.1-99]）<b>说明</b>：1. 当 approval_condition 为 0 ，更新时如果未设置值，默认更新为 99 .2. 传入的值小数点后超过 2 位，自动四舍五入保留两位')
                    .optional(),
                approvers: zod_1.z
                    .array(zod_1.z.object({ user_id: zod_1.z.string().describe('预定管理员ID') }))
            ...
            time_config: zod_1.z
                .object({
                if_cover_child_scope: zod_1.z.boolean().describe('是否覆盖子层级及会议室').optional(),
                time_switch: zod_1.z.number().describe('预定时间开关：0 代表关闭，1 代表开启'),
                days_in_advance: zod_1.z
                    .number()
                    .describe('最早可提前 days_in_advance 预定会议室（单位：天，取值范围[1-730]）<b>说明</b>：不填写时，默认更新为 365')
                    .optional(),
                opening_hour: zod_1.z
                    .string()
                    .describe('开放当天可于 opening_hour 开始预定（单位：秒，取值范围[0,86400]）<b>说明</b>：1. 不填写时默认更新为 28800 2. 如果填写的值不是 60 的倍数，则自动会更新为离其最近的 60 整数倍的值')
                    .optional(),
                start_time: zod_1.z
                    .string()
                    .describe('每日可预定时间范围的开始时间（单位：秒，取值范围[0,86400]）<b>说明</b>：1. 不填写时，默认更新为 0 ，此时填写的 end_time 不得小于 30。2. 当 start_time 与 end_time 均填写时， end_time 至少超过 start_time 30 。3. 如果填写的值不是 60 的倍数，则自动会更新为离其最近的 60 整数倍的值')
                    .optional(),
                end_time: zod_1.z
                    .string()
                    .describe('每日可预定时间范围结束时间（单位：秒，取值范围[0,86400]）<b>说明</b>：1. 不填写时，默认更新为 86400 ，此时填写的 start_time 不得大于等于 86370 。2. 当 start_time 与 end_time 均填写时， end_time 至少要超过 start_time 30。3. 如果填写的值不是 60 的倍数，则自动会更新为离其最近的 60 整数倍的值')
                    .optional(),
            ...
            reserve_scope_config: zod_1.z
                ...
                allow_all_users: zod_1.z
                    .number()
                    .describe('可预定成员范围：0 代表部分成员，1 代表全部成员。<b>说明</b>：1. 此值必填。2. 当设置为 0 时，至少需要 1 个预定部门或预定人')
                    .optional(),
                allow_users: zod_1.z
                    .array(zod_1.z.object({ user_id: zod_1.z.string().describe('预定管理员ID') }))
                    .describe('可预定成员列表')
        ...
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() }),
        path: zod_1.z.object({ reserve_config_id: zod_1.z.string().describe('会议室或层级id') }),
    },
};
exports.vcV1ReserveConfigReserveScope = {
    ...
    schema: {
        params: zod_1.z.object({
            scope_id: zod_1.z.string().describe('会议室或层级id'),
            scope_type: zod_1.z.string().describe('1 代表层级，2 代表会议室'),
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional(),
        }),
    },
...
exports.vcV1ReserveApply = {
    ...
    schema: {
        data: zod_1.z.object({
            end_time: zod_1.z.string().describe('预约到期时间（unix时间，单位sec），多人会议必填').optional(),
            owner_id: zod_1.z
                .string()
                .describe('指定会议归属人，使用 tenant_access_token 时生效且必传，指定对象必须为同租户下的合法飞书用户使用 user_access_token 时，该参数不生效，设置归属人无意义')
                .optional(),
            meeting_settings: zod_1.z
                .object({
                topic: zod_1.z.string().describe('会议主题').optional(),
                action_permissions: zod_1.z
                    .array(zod_1.z.object({
                    permission: zod_1.z
                        .number()
                        .describe('权限项 Options:1(can_be_host 是否能成为主持人),2(can_invite 是否能邀请参会人),3(can_join 是否能加入会议)'),
                    permission_checkers: zod_1.z
                        .array(zod_1.z.object({
                        check_field: zod_1.z
                            .number()
                            .describe('检查字段类型 Options:1(user_id 用户ID（check_list填入用户ID）),2(user_type 用户类型（check_list可选值有 "1"：飞书用户、"2"：rooms用户、"6"：pstn用户、"7"：sip用户）),3(tenant_id 租户ID（check_list填入租户tenant_key）)'),
                        check_mode: zod_1.z
                            .number()
                            .describe('检查方式 Options:1(within 在check_list中为有权限（白名单）),2(without 不在check_list中为有权限（黑名单）)'),
                        check_list: zod_1.z.array(zod_1.z.string()).describe('检查字段列表（根据check_field的类型填入对应内容）'),
                    }))
                        .describe('权限检查器列表，权限检查器之间为"逻辑或"的关系（即 有一个为true则拥有该权限）'),
                }))
                    .describe('会议权限配置列表，如果存在相同的权限配置项则它们之间为"逻辑或"的关系（即 有一个为true则拥有该权限）')
                    .optional(),
                meeting_initial_type: zod_1.z
                    .number()
                    .describe('会议初始类型 Options:1(group_meeting 多人会议),2(call 1v1呼叫(仅支持预约PSTN用户))')
                    .optional(),
                meeting_connect: zod_1.z.boolean().describe('该会议是否支持互通，不支持更新（注：该字段内测中）').optional(),
                call_setting: zod_1.z
                    ...
                    callee: zod_1.z
                        ...
                        user_type: zod_1.z
                            .number()
                            .describe('用户类型，当前仅支持用户类型6(pstn用户) Options:1(lark_user 飞书用户),2(room_user rooms用户),3(doc_user 文档用户),4(neo_user neo单品用户),5(neo_guest_user neo单品游客用户),6(pstn_user pstn用户),7(sip_user sip用户)'),
                        pstn_sip_info: zod_1.z
                            .object({
                            nickname: zod_1.z.string().describe('给pstn/sip用户设置的临时昵称').optional(),
                            main_address: zod_1.z
                                .string()
                                .describe('pstn/sip主机号，格式为：[国际冠字]-[电话区号][电话号码]，当前仅支持国内手机及固定电话号码'),
                        })
                            .describe('pstn/sip信息')
                            .optional(),
                    })
                    ...
                    .describe('1v1呼叫相关参数')
                    .optional(),
                auto_record: zod_1.z.boolean().describe('使用飞书视频会议时，是否开启自动录制，默认false').optional(),
                ...
                password: zod_1.z
                    .string()
                    .describe('设置会议密码，不传则根据个人设置决定是否使用密码及分配随机密码，传空则不使用密码，传 4-9 位数字则设置密码')
                    .optional(),
            })
                .describe('会议设置'),
        }),
...
exports.vcV1ReserveDelete = {
    ...
    accessTokens: ['tenant', 'user'],
    schema: {
        path: zod_1.z.object({ reserve_id: zod_1.z.string().describe('预约ID（预约的唯一标识）').optional() }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
...
exports.vcV1ReserveGetActiveMeeting = {
    ...
    schema: {
        params: zod_1.z.object({
            with_participants: zod_1.z.boolean().describe('是否需要参会人列表，默认为false').optional(),
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional(),
        }),
        path: zod_1.z.object({ reserve_id: zod_1.z.string().describe('预约ID（预约的唯一标识）').optional() }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
...
exports.vcV1ReserveUpdate = {
    ...
    schema: {
        data: zod_1.z.object({
            end_time: zod_1.z.string().describe('预约到期时间（unix时间，单位sec）').optional(),
            meeting_settings: zod_1.z
                .object({
                topic: zod_1.z.string().describe('会议主题').optional(),
                action_permissions: zod_1.z
...
exports.vcV1ResourceReservationListGet = {
    ...
    schema: {
        params: zod_1.z.object({
            room_level_id: zod_1.z.string().describe('层级ID，如传递非omb前缀的异常ID时，会默认使用租户层级进行兜底'),
            need_topic: zod_1.z.boolean().describe('是否展示会议主题').optional(),
            start_time: zod_1.z.string().describe('查询开始时间（unix时间，单位sec）'),
            end_time: zod_1.z.string().describe('查询结束时间（unix时间，单位sec）'),
            room_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('待筛选的会议室ID列表；如需要传递多个会议室ID，需要通过room_ids=aaaa&room_ids=bbbb&room_ids=cccc的形式传递'),
            is_exclude: zod_1.z
                .boolean()
                .describe('默认为false；若为false，则获取room_ids字段传入的会议室列表预定数据；若为true，则根据room_level_id字段获取层级下的会议室列表，并过滤掉room_ids范围的会议室，获取剩余会议室的预定数据')
                .optional(),
            page_size: zod_1.z.number().describe('分页尺寸大小').optional(),
            page_token: zod_1.z
                .string()
...
exports.vcV1RoomConfigQuery = {
    ...
    schema: {
        params: zod_1.z.object({
            scope: zod_1.z
                .number()
                .describe('查询节点范围 Options:1(tenant 租户),2(country_district 国家/地区),3(city 城市),4(building 建筑),5(floor 楼层),6(room 会议室)'),
            country_id: zod_1.z.string().describe('国家/地区ID scope为2，3时需要此参数').optional(),
            district_id: zod_1.z.string().describe('城市ID scope为3时需要此参数').optional(),
            building_id: zod_1.z.string().describe('建筑ID scope为4，5时需要此参数').optional(),
            floor_name: zod_1.z.string().describe('楼层 scope为5时需要此参数').optional(),
            room_id: zod_1.z.string().describe('会议室ID scope为6时需要此参数').optional(),
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional(),
        }),
    },
};
exports.vcV1RoomConfigSet = {
    ...
    schema: {
        data: zod_1.z.object({
            scope: zod_1.z
                .number()
                .describe('设置节点范围 Options:1(tenant 租户),2(country_district 国家/地区),3(city 城市),4(building 建筑),5(floor 楼层),6(room 会议室)'),
            country_id: zod_1.z.string().describe('国家/地区ID scope为2，3时需要此参数').optional(),
            district_id: zod_1.z.string().describe('城市ID scope为3时需要此参数').optional(),
            building_id: zod_1.z.string().describe('建筑ID scope为4，5时需要此参数').optional(),
            floor_name: zod_1.z.string().describe('楼层 scope为5时需要此参数').optional(),
            ...
            room_config: zod_1.z
                .object({
                room_background: zod_1.z.string().describe('飞书会议室背景图').optional(),
                display_background: zod_1.z.string().describe('飞书签到板背景图').optional(),
                digital_signage: zod_1.z
                    .object({
                    enable: zod_1.z.boolean().describe('是否开启数字标牌功能').optional(),
                    mute: zod_1.z.boolean().describe('是否静音播放').optional(),
                    ...
                    materials: zod_1.z
                        .array(zod_1.z.object({
                        id: zod_1.z.string().describe('素材ID，当设置新素材时，无需传递该字段').optional(),
                        name: zod_1.z.string().describe('素材名称').optional(),
                        material_type: zod_1.z.number().describe('素材类型 Options:1(pic 图片),2(video 视频),3(gif)').optional(),
                        url: zod_1.z.string().describe('素材url').optional(),
                        duration: zod_1.z.number().describe('播放时长（单位sec），取值1~43200').optional(),
                        cover: zod_1.z.string().describe('素材封面url').optional(),
                        md5: zod_1.z.string().describe('素材文件md5').optional(),
                        vid: zod_1.z.string().describe('素材文件vid').optional(),
                        size: zod_1.z.string().describe('素材文件大小（单位byte）').optional(),
                    }))
                        .describe('素材列表')
                        .optional(),
                })
                ...
                room_status: zod_1.z
                    .object({
                    status: zod_1.z.boolean().describe('是否启用会议室'),
                    disable_start_time: zod_1.z.string().describe('禁用开始时间（unix时间，单位sec）').optional(),
                    disable_end_time: zod_1.z.string().describe('禁用结束时间（unix时间，单位sec，数值0表示永久禁用）').optional(),
                    disable_reason: zod_1.z.string().describe('禁用原因').optional(),
                    contact_ids: zod_1.z.array(zod_1.z.string()).describe('联系人列表，id类型由user_id_type参数决定').optional(),
                    disable_notice: zod_1.z.boolean().describe('是否在禁用时发送通知给预定了该会议室的员工').optional(),
                    resume_notice: zod_1.z.boolean().describe('是否在恢复启用时发送通知给联系人').optional(),
                })
                    .describe('会议室状态')
                    .optional(),
            })
...
exports.vcV1RoomConfigSetCheckboardAccessCode = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            floor_name: zod_1.z.string().describe('楼层 scope为5时需要此参数').optional(),
            room_id: zod_1.z.string().describe('会议室ID scope为6时需要此参数').optional(),
            valid_day: zod_1.z.number().describe('有效天数 Options:1(day 1天),7(week 7天),30(month 30天)'),
        }),
    },
};
...
exports.vcV1RoomLevelCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            name: zod_1.z.string().describe('层级名称'),
            parent_id: zod_1.z
                .string()
                .describe('父层级ID。**说明**：如需在租户层级（即根层级）下创建会议室层级，可以先调用接口，将路径参数 `room_level_id` 传入 `0` 进行查询，返回结果中的 `room_level_id` 值即为根层级 ID'),
            custom_group_id: zod_1.z.string().describe('自定义层级ID').optional(),
        }),
    },
};
exports.vcV1RoomLevelDel = {
    ...
    schema: {
        data: zod_1.z.object({
            room_level_id: zod_1.z.string().describe('层级ID'),
            delete_child: zod_1.z.boolean().describe('是否删除所有子层级').optional(),
        }),
    },
};
exports.vcV1RoomLevelGet = {
    ...
    accessTokens: ['tenant'],
    schema: {
        path: zod_1.z.object({ room_level_id: zod_1.z.string().describe('层级ID，查询租户层级可传0') }),
    },
};
exports.vcV1RoomLevelList = {
    ...
    schema: {
        params: zod_1.z.object({
            room_level_id: zod_1.z.string().describe('层级ID，当需要获取租户下层级列表时，room_level_id可传空').optional(),
            page_size: zod_1.z.number().describe('分页尺寸大小').optional(),
            page_token: zod_1.z
                .string()
                .describe('分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果')
...
exports.vcV1RoomLevelMget = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-视频会议-会议室层级-批量查询会议室层级详情-该接口可以使用会议室层级 ID 批量查询会议室层级详情',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({ level_ids: zod_1.z.array(zod_1.z.string()).describe('层级ID列表') }),
    },
};
exports.vcV1RoomLevelPatch = {
    ...
    schema: {
        data: zod_1.z.object({
            name: zod_1.z.string().describe('层级名称'),
            parent_id: zod_1.z.string().describe('父层级ID'),
            custom_group_id: zod_1.z.string().describe('自定义层级ID').optional(),
        }),
        path: zod_1.z.object({ room_level_id: zod_1.z.string().describe('层级ID') }),
    },
};
exports.vcV1RoomLevelSearch = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-视频会议-会议室层级-搜索会议室层级-该接口可以用来搜索会议室层级，支持使用自定义会议室层级 ID 进行查询',
    accessTokens: ['tenant'],
    schema: {
        params: zod_1.z.object({ custom_level_ids: zod_1.z.string().describe('用于查询指定会议室层级的自定义会议室层级ID') }),
    },
};
exports.vcV1RoomCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            name: zod_1.z.string().describe('会议室名称'),
            capacity: zod_1.z.number().describe('会议室能容纳的人数'),
            description: zod_1.z.string().describe('会议室的相关描述').optional(),
            custom_room_id: zod_1.z.string().describe('自定义的会议室ID').optional(),
            room_level_id: zod_1.z.string().describe('层级ID'),
            room_status: zod_1.z
                .object({
                status: zod_1.z.boolean().describe('是否启用会议室'),
                ...
                .optional(),
            device: zod_1.z
                .array(zod_1.z.object({ name: zod_1.z.string().describe('设施名称') }))
                .describe('设施信息列表')
                .optional(),
        }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() }),
...
exports.vcV1RoomDelete = {
    ...
    accessTokens: ['tenant'],
    schema: {
        path: zod_1.z.object({ room_id: zod_1.z.string().describe('会议室ID') }),
    },
};
exports.vcV1RoomGet = {
    project: 'vc',
    ...
    schema: {
        params: zod_1.z.object({
            ...
            page_token: zod_1.z
                ...
                .describe('分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果')
                .optional(),
            room_level_id: zod_1.z.string().describe('层级ID，当需要获取租户下会议室列表时，room_level_id可传空').optional(),
            user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional(),
        }),
    },
};
exports.vcV1RoomMget = {
    ...
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({ room_ids: zod_1.z.array(zod_1.z.string()).describe('会议室id列表') }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() }),
    },
};
exports.vcV1RoomPatch = {
    ...
    schema: {
        data: zod_1.z.object({
            name: zod_1.z.string().describe('会议室名称').optional(),
            capacity: zod_1.z.number().describe('会议室能容纳的人数').optional(),
            description: zod_1.z.string().describe('会议室的相关描述').optional(),
            custom_room_id: zod_1.z.string().describe('自定义的会议室ID').optional(),
            room_level_id: zod_1.z.string().describe('层级ID').optional(),
            room_status: zod_1.z
                .object({
                status: zod_1.z.boolean().describe('是否启用会议室'),
                schedule_status: zod_1.z
...
exports.vcV1RoomSearch = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-视频会议-会议室管理-搜索会议室-该接口可以用来搜索会议室，支持使用关键词进行搜索，也支持使用自定义会议室 ID 进行查询。该接口只会返回用户有预定权限的会议室列表',
    accessTokens: ['user'],
    schema: {
        data: zod_1.z.object({
            custom_room_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('用于查询指定会议室的租户自定义会议室ID列表，优先使用该字段进行查询')
                .optional(),
            keyword: zod_1.z.string().describe('会议室搜索关键词（当custom_room_ids为空时，使用该字段进行查询）').optional(),
...
exports.vcV1ScopeConfigCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-视频会议-会议室配置-设置会议室配置-该接口可以用来设置某个会议层级范围下或者某个会议室的配置',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            scope_type: zod_1.z.number().describe('查询节点范围 Options:1(RoomLevel 会议室层级),2(Room 会议室)'),
            scope_id: zod_1.z.string().describe('查询节点ID：如果scope_type为1，则为层级ID，如果scope_type为2，则为会议室ID'),
...
exports.vcV1ScopeConfigGet = {
    ...
    path: '/open-apis/vc/v1/scope_config',
    httpMethod: 'GET',
    description: '[Feishu/Lark]-视频会议-会议室配置-查询会议室配置-该接口可以用来查询某个会议层级范围下或者某个会议室的配置',
    accessTokens: ['tenant'],
    schema: {
        params: zod_1.z.object({
            scope_type: zod_1.z.number().describe('查询节点范围 Options:1(RoomLevel 会议室层级),2(Room 会议室)'),
