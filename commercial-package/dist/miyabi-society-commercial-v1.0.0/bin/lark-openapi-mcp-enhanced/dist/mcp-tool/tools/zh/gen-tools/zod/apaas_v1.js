...
exports.apaasV1ApplicationAuditLogAuditLogList = {
    ...
    schema: {
        params: zod_1.z.object({
            ...
            offset: zod_1.z.string().describe('翻页数量'),
            quick_query: zod_1.z.string().describe('模糊查询').optional(),
            from: zod_1.z.string().describe('查询时间范围：开始时间（单位为毫秒级时间戳）'),
            to: zod_1.z.string().describe('查询时间范围：结束时间（单位为毫秒级时间戳）'),
            log_type: zod_1.z
                .string()
                .describe('日志类型：- 10000: 全部日志- 10001: 企业管理日志- 10002: 登录日志- 10003: 应用管理日志'),
            filter: zod_1.z.string().describe('日志查询：筛选能力').optional(),
            columns: zod_1.z
                .array(zod_1.z.string())
                .describe('日志列表：选择展示行信息，例如["opTime","appName","eventName","clientIP","operator","status"]')
                .optional(),
            sort_by: zod_1.z.string().describe('查询排序字段：可选项为操作时间（opTime）').optional(),
            sort_order: zod_1.z.string().describe('查询排序：默认按时间从大到小；从小到大使用 asc').optional(),
            app_type: zod_1.z.string().describe('应用类型，0为apaas类型，1为aily类型').optional(),
        }),
        path: zod_1.z.object({ namespace: zod_1.z.string().describe('应用命名空间') }),
...
exports.apaasV1ApplicationAuditLogDataChangeLogDetail = {
    ...
    accessTokens: ['user'],
    schema: {
        params: zod_1.z.object({ log_id: zod_1.z.string().describe('数据变更日志ID信息') }),
        path: zod_1.z.object({ namespace: zod_1.z.string().describe('应用命名空间') }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.apaasV1ApplicationAuditLogDataChangeLogsList = {
    ...
    schema: {
        params: zod_1.z.object({
            ...
            page_size: zod_1.z.string().describe('分页大小'),
            offset: zod_1.z.string().describe('翻页数量'),
            from: zod_1.z.string().describe('查询时间范围：开始时间').optional(),
            to: zod_1.z.string().describe('查询时间范围：结束时间').optional(),
            log_type: zod_1.z.string().describe('”日志类型：10007-数据变更日志“'),
            filter: zod_1.z.string().describe('日志查询：筛选能力').optional(),
            columns: zod_1.z
                .array(zod_1.z.string())
                .describe('日志列表：选择展示行信息，例如["opTime","appName","eventName","clientIP","operator","status"]')
                .optional(),
            sort_by: zod_1.z.string().describe('查询排序字段：可选项为操作时间（opTime）').optional(),
            sort_order: zod_1.z.string().describe('查询排序：按时间从小到大使用 asc').optional(),
            app_type: zod_1.z.string().describe('应用类型，0为apaas类型，1为aily类型').optional(),
        }),
        path: zod_1.z.object({ namespace: zod_1.z.string().describe('应用命名空间') }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
...
exports.apaasV1ApplicationAuditLogGet = {
    ...
    schema: {
        params: zod_1.z.object({
            log_id: zod_1.z.string().describe('审计日志ID信息（通过获取单条日志ID）'),
        }),
        path: zod_1.z.object({ namespace: zod_1.z.string().describe('应用命名空间') }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
...
exports.apaasV1ApplicationEnvironmentVariableGet = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-飞书低代码平台-环境变量-查询环境变量详情-查询基于飞书应用引擎开发的应用的环境变量详情，包括名称、描述、变量值等',
    accessTokens: ['tenant'],
    schema: {
        path: zod_1.z.object({
            namespace: zod_1.z.string().describe('应用命名空间'),
            environment_variable_api_name: zod_1.z.string().describe('环境变量 API 名称'),
        }),
    },
};
exports.apaasV1ApplicationEnvironmentVariableQuery = {
    ...
    schema: {
        data: zod_1.z.object({
            filter: zod_1.z
                .object({ quick_query: zod_1.z.string().describe('模糊查询关键词').optional() })
                .describe('过滤条件')
                .optional(),
            limit: zod_1.z.number().describe('限制的条数，默认为 500，不可超过 500').optional(),
            offset: zod_1.z.number().describe('返回记录的偏移量，默认为 0，即从查询到的第一个记录开始返回').optional(),
        }),
        path: zod_1.z.object({ namespace: zod_1.z.string().describe('应用命名空间') }),
    },
};
exports.apaasV1ApplicationFlowExecute = {
    ...
    schema: {
        data: zod_1.z.object({
            is_async: zod_1.z.boolean().describe('是否异步执行(不传默认false)').optional(),
            idempotent_key: zod_1.z.string().describe('幂等键（建议本地生成uuid传入，重复的话请求会报错）').optional(),
            loop_masks: zod_1.z.array(zod_1.z.string()).describe('循环标志信息(当前版本可不传)').optional(),
            params: zod_1.z.string().describe('流程入参（json 字符串，无入参不传）').optional(),
            operator: zod_1.z
                .string()
                .describe('操作人（_id和email至少填一个，低代码平台用户的 id和email，需要从低代码平台获取，json字符串）'),
        }),
        path: zod_1.z.object({
            namespace: zod_1.z.string().describe('应用命名空间(低代码平台->我的应用->应用管理->可查看到)'),
            flow_id: zod_1.z.string().describe('流程API名称（低代码平台->我的应用->开发->流程->展开为表格->可查看到）'),
        }),
    },
};
exports.apaasV1ApplicationFunctionInvoke = {
    ...
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({ params: zod_1.z.string().describe('函数输入参数（JSON 序列化后的字符串）').optional() }),
        path: zod_1.z.object({
            namespace: zod_1.z.string().describe('应用命名空间'),
            function_api_name: zod_1.z.string().describe('函数 API 名称'),
        }),
    },
};
exports.apaasV1ApplicationObjectOqlQuery = {
    ...
    schema: {
        data: zod_1.z.object({
            query: zod_1.z.string().describe('待执行的 OQL 语句'),
            args: zod_1.z.string().describe('用于指定 OQL 语句中匿名参数的具体值').optional(),
            named_args: zod_1.z.string().describe('用于指定 OQL 语句中具名参数的具体值').optional(),
        }),
        path: zod_1.z.object({ namespace: zod_1.z.string().describe('应用命名空间') }),
    },
};
exports.apaasV1ApplicationObjectRecordBatchCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            records: zod_1.z.string().describe('记录详情列表，格式为 List<Map<string, ANY>>，操作记录数上限为 500 条'),
        }),
        path: zod_1.z.object({
            namespace: zod_1.z.string().describe('应用命名空间'),
            object_api_name: zod_1.z.string().describe('对象唯一标识'),
        }),
    },
};
exports.apaasV1ApplicationObjectRecordBatchDelete = {
    ...
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({ ids: zod_1.z.array(zod_1.z.string()).describe('记录 ID 列表，操作记录数上限为 500') }),
        path: zod_1.z.object({
            namespace: zod_1.z.string().describe('应用命名空间'),
            object_api_name: zod_1.z.string().describe('对象唯一标识'),
        }),
...
exports.apaasV1ApplicationObjectRecordBatchQuery = {
    ...
    schema: {
        data: zod_1.z.object({
            select: zod_1.z.array(zod_1.z.string()).describe('需要获取的字段，使用字段唯一标识符进行查询，关联字段可使用「.」进行下钻'),
            filter: zod_1.z
                .object({
                conditions: zod_1.z
                    .array(zod_1.z.object({
                    index: zod_1.z.string().describe('序号').optional(),
                    left: zod_1.z
                        .object({
                        type: zod_1.z.string().describe('左值类型，只支持 "metadataVariable"，表示字段').optional(),
                        settings: zod_1.z
                            .string()
                            .describe('字段具体值，以 JSONString 表示，格式：{"fieldPath":[{"fieldApiName": "字段名","objectApiName": "对象名"}]}')
                            .optional(),
                    })
                        .describe('左值')
                        .optional(),
                    right: zod_1.z
                        .object({
                        type: zod_1.z.string().describe('右值类型，只支持 "constant"，表示常量').optional(),
                        settings: zod_1.z
                            .string()
                        ...
                        .describe('右值')
                        .optional(),
                    operator: zod_1.z.string().describe('操作符').optional(),
                }))
                    .describe('查询条件')
                    .optional(),
                logic_expression: zod_1.z.string().describe('逻辑关系').optional(),
            })
                .describe('筛选条件，通过 JSON 格式指定条件')
                .optional(),
            order_by: zod_1.z
                .array(zod_1.z.object({
                field: zod_1.z.string().describe('字段唯一标识'),
                direction: zod_1.z
                    .enum(['ASC', 'DESC'])
                    .describe('排序方向，值为 "ASC" 或者 "DESC"，代表升序和降序 Options:ASC(SortDirectionASC 升序),DESC(SortDirectionDESC 降序)'),
            }))
                .describe('排序参数，通过 JSON 格式指定条件。其中， field 为参与排序字段，direction 为排序方向，多个条件按其在数组中的顺序生效')
                .optional(),
            group_by: zod_1.z
                .array(zod_1.z.object({ field: zod_1.z.string().describe('字段唯一标识') }))
            ...
            use_page_token: zod_1.z
                .boolean()
                .describe('是否使用 page_token 功能。为 True 时将使用 page_token 的值作为起始位置查询记录，并且会在 Response 中返回 next_page_token 。默认为 False')
                .optional(),
            page_size: zod_1.z.number().describe('期望服务端返回的记录条数，上限 500 条。不填则取默认值，默认值为 500').optional(),
            offset: zod_1.z
                .number()
                .describe('返回记录的偏移量，默认为 0 ，即从查询到的第一条记录开始返回。offset 较大时查询性能较差，可能引起接口响应超时，拉取全部记录时建议使用 ID 游标分页，具体见 ID 游标分页说明')
                .optional(),
        ...
        path: zod_1.z.object({
            namespace: zod_1.z.string().describe('命名空间'),
            object_api_name: zod_1.z.string().describe('目标对象的唯一标识符'),
        }),
    },
};
...
exports.apaasV1ApplicationObjectRecordCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            record: zod_1.z.string().describe('创建对象使用的数据，键为字段 API 名称，值为字段值，格式可参考字段值格式'),
        }),
        path: zod_1.z.object({
            namespace: zod_1.z.string().describe('应用命名空间'),
            object_api_name: zod_1.z.string().describe('对象唯一标识'),
...
exports.apaasV1ApplicationObjectSearch = {
    ...
    schema: {
        data: zod_1.z.object({
            q: zod_1.z.string().describe('搜索词').optional(),
            search_objects: zod_1.z
                .array(zod_1.z.object({
                api_name: zod_1.z.string().describe('对象 APIName').optional(),
                search_fields: zod_1.z.array(zod_1.z.string()).describe('搜索字段 SearchFields 列表').optional(),
                select: zod_1.z.array(zod_1.z.string()).describe('召回字段 APIID/APIName 列表').optional(),
                filter: zod_1.z
                    .object({
                    conditions: zod_1.z
                        .array(zod_1.z.object({
                ...
                order_by: zod_1.z
                    .object({
                    field: zod_1.z.string().describe('字段名').optional(),
                    order_type: zod_1.z
                        .enum(['asc', 'desc'])
            ...
            page_token: zod_1.z
                ...
                .describe('分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果')
                .optional(),
            page_size: zod_1.z.string().describe('返回数量，默认为50，最大不超过2000').optional(),
            metadata: zod_1.z
                .enum(['Label', 'SearchLayout'])
                .describe('返回元数据枚举值 Options:Label(MetadataOptionLabel 只返回 Label),SearchLayout(MetadataOptionSearchLayout 返回搜索布局信息)')
                .optional(),
        }),
        path: zod_1.z.object({ namespace: zod_1.z.string().describe('应用命名空间') }),
    },
...
exports.apaasV1ApplicationRecordPermissionMemberBatchCreateAuthorization = {
    ...
    schema: {
        data: zod_1.z.object({
            user_ids: zod_1.z.array(zod_1.z.string()).describe('需要新增的用户 ID 列表，使用飞书低代码平台的用户 ID').optional(),
        }),
        path: zod_1.z.object({
            namespace: zod_1.z.string().describe('应用命名空间'),
            record_permission_api_name: zod_1.z.string().describe('记录权限 API 名称'),
        }),
    },
};
exports.apaasV1ApplicationRecordPermissionMemberBatchRemoveAuthorization = {
    ...
    schema: {
        data: zod_1.z.object({
            user_ids: zod_1.z.array(zod_1.z.string()).describe('需要删除的用户 ID 列表，使用飞书低代码平台的用户 ID').optional(),
        }),
        path: zod_1.z.object({
            namespace: zod_1.z.string().describe('应用命名空间'),
            record_permission_api_name: zod_1.z.string().describe('记录权限 API 名称'),
...
exports.apaasV1ApplicationRoleMemberBatchCreateAuthorization = {
    ...
    schema: {
        data: zod_1.z.object({
            user_ids: zod_1.z.array(zod_1.z.string()).describe('需要新增的用户 ID 列表，使用飞书低代码平台的用户 ID').optional(),
            department_ids: zod_1.z.array(zod_1.z.string()).describe('需要新增的部门 ID 列表，使用飞书低代码平台的部门 ID').optional(),
        }),
        path: zod_1.z.object({
            namespace: zod_1.z.string().describe('应用命名空间'),
            role_api_name: zod_1.z.string().describe('角色 API 名称'),
        }),
    },
};
exports.apaasV1ApplicationRoleMemberBatchRemoveAuthorization = {
    ...
    schema: {
        data: zod_1.z.object({
            user_ids: zod_1.z.array(zod_1.z.string()).describe('需要删除的用户 ID 列表，使用飞书低代码平台的用户 ID').optional(),
            department_ids: zod_1.z.array(zod_1.z.string()).describe('需要删除的部门 ID 列表，使用飞书低代码平台的部门 ID').optional(),
        }),
        path: zod_1.z.object({
            namespace: zod_1.z.string().describe('应用命名空间'),
            role_api_name: zod_1.z.string().describe('角色 API 名称'),
...
exports.apaasV1ApplicationRoleMemberGet = {
    ...
    schema: {
        params: zod_1.z.object({
            need_display_name: zod_1.z.boolean().describe('是否需要公式的展示名称，便于前端展示').optional(),
            use_api_id: zod_1.z.boolean().describe('是否使用 API ID字段作为出入参，默认值为 false').optional(),
        }),
        path: zod_1.z.object({
            namespace: zod_1.z.string().describe('应用命名空间'),
            role_api_name: zod_1.z.string().describe('角色 API 名称'),
...
exports.apaasV1ApprovalInstanceCancel = {
    ...
    schema: {
        data: zod_1.z.object({
            user_id: zod_1.z.string().describe('操作用户的kunlunUserID,可通过Apaas用户管理页面获取'),
            opinion: zod_1.z.string().describe('撤销原因'),
        }),
        path: zod_1.z.object({
...
exports.apaasV1ApprovalTaskAddAssignee = {
    ...
    schema: {
        data: zod_1.z.object({
            user_id: zod_1.z.string().describe('操作人 ID'),
            approvers: zod_1.z.array(zod_1.z.string().describe('审批人id')).describe('审批人列表').optional(),
            add_assignee_type: zod_1.z
                .string()
                .describe('加签类型：- 并加签：currentAndAddAssign- 后加签：afterAndAddAssign')
                .optional(),
            opinion: zod_1.z.string().describe('加签原因').optional(),
        }),
        path: zod_1.z.object({ approval_task_id: zod_1.z.string().describe('人工任务 ID') }),
    },
};
exports.apaasV1ApprovalTaskAgree = {
    ...
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({ user_id: zod_1.z.string().describe('操作人 ID'), opinion: zod_1.z.string().describe('审批意见').optional() }),
        path: zod_1.z.object({ approval_task_id: zod_1.z.string().describe('人工任务 ID') }),
    },
};
...
exports.apaasV1ApprovalTaskTransfer = {
    ...
    schema: {
        data: zod_1.z.object({
            user_id: zod_1.z.string().describe('操作人 ID'),
            from_user_ids: zod_1.z.array(zod_1.z.string()).describe('原审批人id').optional(),
            to_user_ids: zod_1.z.array(zod_1.z.string()).describe('新审批人id').optional(),
            opinion: zod_1.z.string().describe('转交原因').optional(),
        }),
        path: zod_1.z.object({ approval_task_id: zod_1.z.string().describe('人工任务 ID') }),
    },
};
exports.apaasV1SeatActivityList = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-飞书低代码平台-席位活跃-查询席位活跃详情-获取租户下用户使用飞书低代码平台席位最近访问应用时间。需要飞书低代码平台系统管理员作为授权人调用当前API',
    accessTokens: ['user'],
    schema: {
        params: zod_1.z.object({
            seat_type: zod_1.z
                .enum(['per_user', 'per_user_per_app'])
                .describe('席位类型，枚举值：per_user、per_user_per_app Options:per_user(平台席位),per_user_per_app(应用访问席位)'),
            page_size: zod_1.z.string().describe('分页大小，范围：【0，500】'),
            page_token: zod_1.z
                .string()
                .describe('分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果')
                .optional(),
...
exports.apaasV1SeatAssignmentList = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-飞书低代码平台-席位分配-查询席位分配详情-获取租户下平台席位和应用访问席位分配详情，如用户 ID 、应用命名空间等，需要飞书低代码平台系统管理员作为授权人调用当前 API',
    accessTokens: ['user'],
    schema: {
        params: zod_1.z.object({
            seat_type: zod_1.z
...
exports.apaasV1UserTaskCc = {
    ...
    schema: {
        data: zod_1.z.object({
            cc_user_ids: zod_1.z.array(zod_1.z.string()).describe('抄送人的kunlunID列表,可通过Apaas用户管理页面获取'),
            operator_user_id: zod_1.z.string().describe('操作人kunlunUserID,可通过Apaas用户管理页面获取'),
        }),
        path: zod_1.z.object({
            task_id: zod_1.z
                .string()
...
exports.apaasV1UserTaskChatGroup = {
    ...
    schema: {
        data: zod_1.z.object({
            operator_user_id: zod_1.z.string().describe('操作人kunlunUserID,可通过Apaas用户管理页面获取'),
            invite_user_ids: zod_1.z.array(zod_1.z.string()).describe('邀请进群用户KunlunID列表,可通过Apaas用户管理页面获取').optional(),
            chat_id: zod_1.z.string().describe('要拉入的群openID，为空则新建群').optional(),
            chat_name: zod_1.z.string().describe('要加入的群名称，当chat_id为空时用该名称创建群聊').optional(),
        }),
        path: zod_1.z.object({
            task_id: zod_1.z
                .string()
...
exports.apaasV1UserTaskExpediting = {
    ...
    schema: {
        data: zod_1.z.object({
            operator_user_id: zod_1.z.string().describe('操作人kunlunUserID,可通过Apaas用户管理页面获取'),
            expediting_user_ids: zod_1.z.array(zod_1.z.string()).describe('催办人的kunlunID列表,可通过Apaas用户管理页面获取'),
            opinion: zod_1.z.string().describe('催办理由').optional(),
        }),
        path: zod_1.z.object({
            task_id: zod_1.z
                .string()
...
exports.apaasV1UserTaskQuery = {
    ...
    schema: {
        data: zod_1.z.object({
            type: zod_1.z.string().describe('类型- archived：已完成的- pending：待处理的').optional(),
            source: zod_1.z.string().describe('来源- fromMe:我发起的- assignMe:分配给我的- CCMe：抄送我的').optional(),
            limit: zod_1.z.string().describe('获取条数- 最小值：1- 最大值：50').optional(),
            offset: zod_1.z.string().describe('起始位置').optional(),
            start_time: zod_1.z.string().describe('开始时间（毫秒）').optional(),
            end_time: zod_1.z.string().describe('结束时间（毫秒）').optional(),
            api_ids: zod_1.z.array(zod_1.z.string()).describe('流程apiid列表，可以通过apaas流程列表页获取').optional(),
            kunlun_user_id: zod_1.z.string().describe('kunlunUserID,可通过Apaas用户管理页面获取'),
        }),
    },
};
exports.apaasV1UserTaskRollback = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            to_task_id: zod_1.z
                .string()
                .describe('退回到的任务ID，可以通过[查询人工任务可退回的点](/uAjLw4CM/ukTMukTMukTM/apaas-v1/user_task/rollback_points)获取'),
            opinion: zod_1.z.string().describe('退回原因'),
        }),
        path: zod_1.z.object({
            task_id: zod_1.z
                .string()
...
exports.apaasV1UserTaskRollbackPoints = {
    ...
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({ operator_user_id: zod_1.z.string().describe('操作人kunlunUserID,可通过Apaas用户管理页面获取') }),
        path: zod_1.z.object({
            task_id: zod_1.z
                .string()
                .describe('任务ID，可以通过[查询人工任务](/uAjLw4CM/ukTMukTMukTM/apaas-v1/user_task/query)获取'),
