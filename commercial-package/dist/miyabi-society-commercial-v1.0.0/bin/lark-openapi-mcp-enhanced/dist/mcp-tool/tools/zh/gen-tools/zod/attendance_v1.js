...
exports.attendanceV1ApprovalInfoProcess = {
    ...
    schema: {
        data: zod_1.z.object({
            approval_id: zod_1.z.string().describe('审批实例 ID，获取方式：1） 2） 3）'),
...
exports.attendanceV1ArchiveRuleDelReport = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            archive_rule_id: zod_1.z.string().describe('归档规则id，可根据获得'),
            user_ids: zod_1.z.array(zod_1.z.string()).describe('用户id，对应employee_type').optional(),
        }),
        params: zod_1.z.object({
            employee_type: zod_1.z
                .string()
                .describe('请求体中的 user_ids 和响应体中的 user_id 的员工ID类型。如果没有后台管理权限，可使用* `employee_id `：员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID* `employee_no`：员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号'),
        }),
...
exports.attendanceV1ArchiveRuleUploadReport = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-归档报表-写入归档报表结果-写入归档报表结果，对应假勤管理-考勤统计-报表-页签，点击报表名称进入后的导入功能。可以将数据直接写入归档报表',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            ...
            archive_report_datas: zod_1.z
                ...
                field_datas: zod_1.z
                    .array(zod_1.z.object({
                    code: zod_1.z.string().describe('字段编码，可根据 获取'),
...
exports.attendanceV1ArchiveRuleUserStatsFieldsQuery = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-归档报表-查询归档报表表头-查询归档报表表头，对应后台假勤管理-考勤统计-报表-中一个归档报表的表头信息。归档报表支持引用系统报表，可设置归档时间和数据归档周期，并且支持根据部门/人员、国家/地区、人员类型、工作地点、职级、序列、职务进行人员圈选',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
...
exports.attendanceV1GroupCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-考勤组管理-创建或修改考勤组-考勤组，是对部门或者员工在某个特定场所及特定时间段内的出勤情况（包括上下班、迟到、早退、病假、婚假、丧假、公休、工作时间、加班情况等）的一种规则设定。通过设置考勤组，可以从部门、员工两个维度，来设定考勤方式、考勤时间、考勤地点等考勤规则。对应功能同设置-假勤设置-的“新建”功能',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            group: zod_1.z
                ...
                locations: zod_1.z
                    ...
                    latitude: zod_1.z.number().describe('地址纬度').optional(),
                    longitude: zod_1.z.number().describe('地址经度').optional(),
                ...
                punch_day_shift_ids: zod_1.z
                    ...
                    .describe('班次 ID 列表。当考勤类型参数（group_type）设置为固定班制时，必须填够 7 个（从周一至周日）。当考勤类型参数（group_type）设置为排班制或自由班制时，请传入空数组。班次 ID 可以通过 和 获取。休息日填0'),
                ...
                need_punch_special_days: zod_1.z
                    ...
                    shift_id: zod_1.z.string().describe('班次 ID，可根据 和 获得'),
                }))
                    .describe('必须打卡的特殊日期')
                    .optional(),
                ...
                leave_need_punch_cfg: zod_1.z
                    ...
                    .optional(),
                go_out_need_punch: zod_1.z.number().describe('外出期间是否需打卡，默认为空').optional(),
                go_out_need_punch_cfg: zod_1.z
                    ...
                    .optional(),
                travel_need_punch: zod_1.z.number().describe('出差期间是否需打卡，默认为空').optional(),
                ...
                .describe('考勤组信息'),
            operator_id: zod_1.z.string().describe('操作人uid，对应employee_type，如果您未操作，则此字段为必填字段').optional(),
        }),
        params: zod_1.z.object({
            employee_type: zod_1.z
                .enum(['employee_id', 'employee_no'])
                .describe('响应体中的 user_id 的员工ID类型。如果没有后台管理权限，可使用 Options:employee_id(员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID),employee_no(员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号)'),
            dept_type: zod_1.z
                .literal('open_id')
...
exports.attendanceV1GroupDelete = {
    ...
    httpMethod: 'DELETE',
    description: '[Feishu/Lark]-考勤打卡-考勤组管理-删除考勤组-通过考勤组 ID 删除考勤组。对应设置-假勤设置-操作列的删除功能',
    accessTokens: ['tenant'],
    schema: {
        path: zod_1.z.object({
            group_id: zod_1.z.string().describe('考勤组 ID，获取方式：1） 2） 3）'),
        }),
    },
};
exports.attendanceV1GroupGet = {
    ...
    schema: {
        params: zod_1.z.object({
            employee_type: zod_1.z
                .enum(['employee_id', 'employee_no'])
                .describe('请求体和响应体中的 user_id 和 creator_id 的员工id类型。如果没有后台管理权限，可使用 Options:employee_id(员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID),employee_no(员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号)'),
            dept_type: zod_1.z
                .literal('open_id')
                .describe('部门 ID 的类型 Options:open_id(暂时只支持部门的 openid。具体概念请参考中的open_department_id)'),
        }),
...
exports.attendanceV1GroupListUser = {
    ...
    schema: {
        params: zod_1.z.object({
            employee_type: zod_1.z
                .string()
                .describe('响应体中 user_id 的员工 ID 类型。如果没有后台管理权限，可使用<b>可选值有</b>：1. employee_id：员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID2. employee_no：员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号'),
            dept_type: zod_1.z
                .string()
                .describe('响应体中 department_ids 的部门 ID 的类型<b>可选值有</b>：1. open_id：暂时只支持部门的 openid。具体概念请参考中的open_department_id'),
            page_size: zod_1.z.number().describe('分页大小').optional(),
            page_token: zod_1.z
                .string()
                .describe('分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果')
...
exports.attendanceV1GroupSearch = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-考勤组管理-按名称查询考勤组-按考勤组名称查询考勤组摘要信息。查询条件支持名称精确匹配和模糊匹配两种方式。查询结果按考勤组修改时间 desc 排序，且最大记录数为 10 条。对应页面设置-假勤设置-的名称搜索功能',
    accessTokens: ['tenant'],
    schema: {
...
exports.attendanceV1LeaveAccrualRecordPatch = {
    ...
    httpMethod: 'PATCH',
    description: '[Feishu/Lark]-考勤打卡-休假发放记录-修改发放记录-更新发放记录的发放数量和失效日期，对应假勤管理-休假管理-',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            leave_granting_record_id: zod_1.z.string().describe('发放记录的唯一ID，可通过'),
            employment_id: zod_1.z.string().describe('员工ID，类型对应user_id_type'),
            leave_type_id: zod_1.z.string().describe('假期类型ID，可通过获取'),
            reason: zod_1.z
        ...
        path: zod_1.z.object({
            leave_id: zod_1.z.string().describe('假期类型ID，可通过获取').optional(),
        }),
    },
};
...
exports.attendanceV1ShiftCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-考勤班次-创建班次-班次是描述一次考勤任务时间规则的统称，比如一天打多少次卡，每次卡的上下班时间，晚到多长时间算迟到，晚到多长时间算缺卡等。在假勤设置-中点击班次名称可以进行班次详情查看。如果入参中传入了班次id，那么支持编辑班次的能力',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            ...
            late_off_late_on_setting: zod_1.z
                ...
                .describe('晚走次日晚到配置规则')
                .optional(),
            id: zod_1.z.string().describe('班次id(更新班次时需要传递)，获取方式：1） 2）').optional(),
        }),
        params: zod_1.z.object({
            employee_type: zod_1.z
                .enum(['employee_id', 'employee_no'])
                .describe('请求体中的 user_ids 和响应体中的 user_id 的员工ID类型。如果没有后台管理权限，可使用 Options:employee_id(员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID，或者获取的userid。),employee_no(员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号)')
                .optional(),
        }),
    },
};
exports.attendanceV1ShiftDelete = {
    ...
    httpMethod: 'DELETE',
    description: '[Feishu/Lark]-考勤打卡-考勤班次-删除班次-通过班次 ID 删除班次。对应功能为假勤设置-班次列表中操作栏的删除按钮',
    accessTokens: ['tenant'],
    schema: {
        path: zod_1.z.object({
            shift_id: zod_1.z.string().describe('班次 ID，获取方式：1） 2）'),
        }),
    },
};
exports.attendanceV1ShiftGet = {
    ...
    path: '/open-apis/attendance/v1/shifts/:shift_id',
    httpMethod: 'GET',
    description: '[Feishu/Lark]-考勤打卡-考勤班次-按 ID 查询班次-通过班次 ID 获取班次详情。对应功能为假勤设置-班次列表中的具体班次，班次信息可以点击班次名称查看',
    accessTokens: ['tenant'],
    schema: {
        path: zod_1.z.object({
            shift_id: zod_1.z.string().describe('班次 ID，获取方式：1） 2）'),
...
exports.attendanceV1ShiftList = {
    ...
    path: '/open-apis/attendance/v1/shifts',
    httpMethod: 'GET',
    description: '[Feishu/Lark]-考勤打卡-考勤班次-查询所有班次-飞书人事管理后台中假勤设置-中的翻页查询所有班次功能，展示班次名称、打卡规则、弹性班次规则、休息规则等',
    accessTokens: ['tenant'],
    schema: {
        params: zod_1.z.object({
            page_size: zod_1.z.number().describe('分页大小').optional(),
...
exports.attendanceV1ShiftQuery = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-考勤班次-按名称查询班次-飞书人事管理后台中假勤设置-中的搜索班次名称功能，展示班次名称、打卡规则、弹性班次规则、休息规则等',
    accessTokens: ['tenant'],
    schema: {
    ...
    schema: {
                ...
                trips: zod_1.z
                    ...
                    departure: zod_1.z
                        ...
                        region_level: zod_1.z
                            ...
                            .optional(),
                        region_id: zod_1.z.string().describe('地理id可以通过或获取（仅支持飞书人事企业版使用）').optional(),
                    })
                        .describe('出发地（只有一个）')
                        .optional(),
        ...
        params: zod_1.z.object({
            employee_type: zod_1.z
                ...
                .describe('请求体和响应体中的 user_id 的员工ID类型。如果没有后台管理权限，可使用 Options:employee_id(员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID),employee_no(员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号),open_id(用户在某个应用中的身份，可以参考)'),
        }),
    },
};
exports.attendanceV1UserApprovalQuery = {
    ...
    schema: {
        ...
        params: zod_1.z.object({
            employee_type: zod_1.z
                .enum(['employee_id', 'employee_no'])
                .describe('请求体中的 user_ids 和响应体中的 user_id 的员工ID类型。如果没有后台管理权限，可使用 Options:employee_id(员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID),employee_no(员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号)'),
        }),
    },
};
    ...
    schema: {
        data: zod_1.z.object({
            user_daily_shifts: zod_1.z
                .array(zod_1.z.object({
                group_id: zod_1.z.string().describe('考勤组 ID，获取方式：1） 2） 3）'),
                shift_id: zod_1.z.string().describe('班次 ID，获取方式：1） 2）。传入0代表休息'),
            ...
            operator_id: zod_1.z.string().describe('操作人uid，与employee_type对应。如果您未操作，则此字段为必填字段').optional(),
        }),
        params: zod_1.z.object({
            employee_type: zod_1.z
                .enum(['employee_id', 'employee_no'])
                .describe('请求体和响应体中的 user_id 的员工ID类型。如果没有后台管理权限，可使用 Options:employee_id(员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID，或者获取的userid。),employee_no(员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号)'),
        }),
    },
};
exports.attendanceV1UserDailyShiftBatchCreateTemp = {
    ...
    schema: {
        ...
        params: zod_1.z.object({
            employee_type: zod_1.z
                .enum(['employee_id', 'employee_no'])
                .describe('请求体和响应体中的 user_id 的员工ID类型。如果没有后台管理权限，可使用 Options:employee_id(EmployeeID 员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID，或者获取的userid。),employee_no(EmployeeNo 员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号)'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.attendanceV1UserDailyShiftQuery = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-考勤排班-查询排班表-支持查询多个用户的排班情况，注意此接口返回的是用户维度的排班结果，与页面功能并不对应。可以通过返回结果中的group_id查询考勤组 ，shift_id查询班次 。查询的时间跨度不能超过 30 天',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
...
exports.attendanceV1UserFlowBatchCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-打卡信息管理-导入打卡流水-导入员工的打卡流水记录。导入后，会根据员工所在的考勤组班次规则，计算最终的打卡状态与结果。可在打卡管理-中查询',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
        ...
        params: zod_1.z.object({
            employee_type: zod_1.z
                .enum(['employee_id', 'employee_no'])
                .describe('请求体和响应体中的 user_id 和 creator_id 的员工id类型。如果没有后台管理权限，可使用 Options:employee_id(员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID，或者获取的userid。),employee_no(员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号)'),
        }),
    },
};
exports.attendanceV1UserFlowBatchDel = {
    ...
    schema: {
        data: zod_1.z.object({
            record_ids: zod_1.z.array(zod_1.z.string().describe('待删除的流水记录ID')).describe('打卡流水记录 ID，获取方式：1） 2） 3）'),
        }),
    },
};
exports.attendanceV1UserFlowGet = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-考勤打卡-打卡信息管理-查询打卡流水-通过打卡记录 ID 获取用户的打卡流水记录。返回信息主要包含：* 用户id和创建者id* 记录信息* 打卡位置信息、时间信息* 打卡方式信息 * GPS 打卡：location_name（定位地址信息） * Wi-Fi 打卡：ssid（wifi名称）、bssid（mac地址） * 考勤机打卡：device_id（考勤机设备id）对应页面功能打卡管理-',
    accessTokens: ['tenant'],
    schema: {
        params: zod_1.z.object({
            employee_type: zod_1.z
                .enum(['employee_id', 'employee_no'])
                .describe('响应体中的 user_id 和 creator_id 的员工ID类型。如果没有后台管理权限，可使用 Options:employee_id(员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID),employee_no(员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号)'),
        }),
        path: zod_1.z.object({
            user_flow_id: zod_1.z.string().describe('打卡流水记录 ID，获取方式：1） 2） 3）'),
        }),
    },
};
exports.attendanceV1UserFlowQuery = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-打卡信息管理-批量查询打卡流水-通过用户 ID 获取用户的打卡流水记录。返回信息主要包含：* 用户id和创建者id* 记录信息* 打卡位置信息、时间信息* 打卡方式信息 * GPS 打卡：location_name（定位地址信息） * Wi-Fi 打卡：ssid（wifi名称）、bssid（mac地址） * 考勤机打卡：device_id（考勤机设备id）对应页面功能打卡管理-',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
    ...
    schema: {
        data: zod_1.z.object({
            user_setting: zod_1.z
                ...
                face_key: zod_1.z.string().describe('人脸照片文件 ID，获取方式：'),
...
exports.attendanceV1UserSettingQuery = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-考勤打卡-考勤用户管理-批量查询用户人脸识别信息-批量查询授权内员工的用户设置信息，包括人脸照片文件 ID、人脸照片更新时间。对应页面假勤设置-。根据返回的face_key可以下载人脸信息',
    accessTokens: ['tenant'],
    schema: {
        ...
        params: zod_1.z.object({
            employee_type: zod_1.z
                .enum(['employee_id', 'employee_no'])
                .describe('请求体中的 user_ids 和响应体中的 user_id 的员工ID类型。如果没有后台管理权限，可使用 Options:employee_id(EmployeeId 员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID),employee_no(EmployeeNo 员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号)'),
        }),
    },
};
exports.attendanceV1UserStatsDataQuery = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-考勤统计-查询统计数据-查询日度统计或月度统计的统计数据。字段包含基本信息、考勤组信息、出勤统计、异常统计、请假统计、加班统计、打卡时间、考勤结果和自定义字段。具体报表可在考勤统计-中找到',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
...
exports.attendanceV1UserStatsFieldQuery = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-考勤统计-查询统计表头-查询考勤统计支持的日度统计或月度统计的统计表头。报表的表头信息可以在考勤统计-中查询到具体的报表信息，此接口专门用于查询表头数据',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            locale: zod_1.z.enum(['en', 'ja', 'zh']).describe('语言类型 Options:en(英语),ja(日语),zh(中文)'),
...
exports.attendanceV1UserStatsViewQuery = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-考勤统计-查询统计设置-查询考勤统计支持的日度统计或月度统计的统计表头。报表的表头信息可以在考勤统计-中查询到具体的报表信息，此接口专门用于查询表头数据。注意此接口和基本相同，区别点在于在兼容历史统计视图模型（历史统计数据模型可以按用户ID设置，后续统计升级为仅支持租户维度）',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            locale: zod_1.z.enum(['en', 'ja', 'zh']).describe('语言类型 Options:en(英语),ja(日语),zh(中文)'),
...
exports.attendanceV1UserStatsViewUpdate = {
    ...
    httpMethod: 'PUT',
    description: '[Feishu/Lark]-考勤打卡-考勤统计-更新统计设置-更新开发者定制的日度统计或月度统计的统计报表表头设置信息。报表的表头信息可以在考勤统计-中查询到具体的报表信息，此接口专门用于更新表头信息',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            view: zod_1.z
                .object({
                view_id: zod_1.z.string().describe('视图 ID，可通过获取'),
        ...
        path: zod_1.z.object({
            user_stats_view_id: zod_1.z.string().describe('用户视图 ID，获取方式：1）'),
        }),
    },
};
exports.attendanceV1UserTaskRemedyCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-考勤打卡-考勤补卡-通知补卡审批发起-对于只使用飞书考勤系统而未使用飞书审批系统的企业，可以通过该接口，将在三方审批系统中发起的补卡审批数据，写入到飞书考勤系统中，状态为审批中。写入后可以由 进行状态更新',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            user_id: zod_1.z.string().describe('用户 ID，对应employee_type'),
...
exports.attendanceV1UserTaskQuery = {
    ...
    schema: {
        ...
        params: zod_1.z.object({
            employee_type: zod_1.z
                .enum(['employee_id', 'employee_no'])
                .describe('员工ID类型。如果没有后台管理权限，可使用 Options:employee_id(员工 employee ID，即 > 组织架构 > 成员与部门 > 成员详情中的用户 ID),employee_no(员工工号，即 > 组织架构 > 成员与部门 > 成员详情中的工号)'),
