...
exports.passportV1SessionLogout = {
    ...
    schema: {
        data: zod_1.z.object({
            idp_credential_id: zod_1.z.string().describe('idp 侧的唯一标识，logout_type = 2 时必填').optional(),
            logout_type: zod_1.z
                .number()
                .describe('登出的方式 Options:1(UserID UserID，使用开放平台的维度登出),2(IdpCredentialID IdpCredentialID，使用 idp 侧的唯一标识登出),3(SessionUUID Session 标识符，基于session uuid 登出)'),
            terminal_type: zod_1.z
                .array(zod_1.z.number())
                .describe('登出的客户端类型，默认全部登出。可选值：- 1：PC 端- 2：Web 端- 3：Android 端- 4：iOS 端- 5：服务端- 6：旧版小程序端- 8：其他移动端')
                .optional(),
            user_id: zod_1.z
                .string()
            ...
            logout_reason: zod_1.z
                .number()
                .describe('登出提示语，非必填，不传时默认提示：你已在其他客户端上退出了当前设备，请重新登录。可选值：- 34：您已修改登录密码，请重新登录- 35：您的登录态已失效，请重新登录- 36：您的密码已过期，请在登录页面通过忘记密码功能修改密码后重新登录')
                .optional(),
            sid: zod_1.z.string().describe('需要精确登出的 session 标识符，logout_type = 3 时必填').optional(),
        }),
...
exports.passportV1SessionQuery = {
    ...
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({ user_ids: zod_1.z.array(zod_1.z.string()).describe('用户 ID').optional() }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() }),
    },
};
