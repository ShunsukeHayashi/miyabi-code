...
import { readFile, writeFile, readdir, stat, mkdir, unlink, rmdir, copyFile, rename, access, } from "fs/promises";
...
function validatePath(filePath) {
    ...
    if (!config.allowOutsideBasePath && !absolutePath.startsWith(basePath)) {
        throw new Error(`Access denied: Path '${filePath}' is outside the allowed base path '${basePath}'`);
    }
    return absolutePath;
}
/**
...
async function readFileContent(filePath, encoding = "utf-8") {
    ...
    if (stats.size > config.maxFileSize) {
        throw new Error(`File size (${stats.size} bytes) exceeds maximum allowed size (${config.maxFileSize} bytes)`);
    }
 ...
 */
async function deletePath(filePath, options) {
    const absolutePath = validatePath(filePath);
    const stats = await stat(absolutePath);
    if (stats.isDirectory()) {
 ...
 */
async function searchFiles(dirPath = ".", options) {
    const files = await listDirectory(dirPath, {
        recursive: options.recursive,
        includeHidden: false,
    });
    const results = [];
    const namePattern = new RegExp(options.pattern, options.caseSensitive ? "g" : "gi");
        ...
        if (options.content) {
            try {
                ...
                if (matches && matches.length > 0) {
                    ...
                    const matchingLines = lines.filter((line) => namePattern.test(line));
                    results.push({
                        file: file.path,
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    try {
        switch (name) {
            ...
            case "write_file": {
                const { path, content, encoding, create_directories } = args;
            ...
            case "list_files": {
                const { path, recursive, include_hidden, pattern } = args;
            ...
            case "search_files": {
                const { path, pattern, search_content, case_sensitive, recursive } = args;
