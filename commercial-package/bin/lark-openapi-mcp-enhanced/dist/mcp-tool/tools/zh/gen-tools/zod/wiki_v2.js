...
exports.wikiV2SpaceCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            name: zod_1.z.string().describe('知识空间名称').optional(),
            description: zod_1.z.string().describe('知识空间描述').optional(),
...
exports.wikiV2SpaceGet = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-云文档-知识库-知识空间-获取知识空间信息-此接口用于根据知识空间 ID 查询知识空间的信息，包括空间的类型、可见性、分享状态等',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z.object({
            lang: zod_1.z
                .enum(['zh', 'id', 'de', 'en', 'es', 'fr', 'it', 'pt', 'vi', 'ru', 'hi', 'th', 'ko', 'ja', 'zh-HK', 'zh-TW'])
                .describe('当查询个人文档库时，指定返回的文档库名称展示语言。 Options:zh(LangZH 简体中文),id(LangID 印尼语),de(LangDE 德语),en(LangEN 英语),es(LangES 西班牙语),fr(LangFR 法语),it(LangIT 意大利语),pt(LangPT 葡萄牙语),vi(LangVI 越南语),ru(LangRU 俄语),hi(LangHI 印地语),th(LangTH 泰语),ko(LangKO 韩语),ja(LangJA 日语),zh-HK(LangZHHK 繁体中文（中国香港）),zh-TW(LangZHTW 繁体中文（中国台湾）)')
                .optional(),
        }),
        path: zod_1.z.object({
            space_id: zod_1.z
                .string()
                .describe('知识空间 ID。可通过以下两种方式获取。了解更多，参考。- 调用 获取- 如果你是知识库管理员，可以进入知识库设置页面，复制地址栏的数字部分：https://sample.feishu.cn/wiki/settings/==6870403571079249922==')
                .optional(),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
...
exports.wikiV2SpaceGetNode = {
    ...
    schema: {
        params: zod_1.z.object({
            token: zod_1.z
                .string()
                .describe('知识库节点或对应云文档的实际 token。- 知识库节点 token：如果 URL 链接中 token 前为 wiki，该 token 为知识库的节点 token。- 云文档实际 token：如果 URL 链接中 token 前为 docx、base、sheets 等非 wiki 类型，则说明该 token 是当前云文档的实际 token。了解更多，请参考。**注意**：使用云文档 token 查询时，需要对 obj_type 参数传入文档对应的类型'),
            obj_type: zod_1.z
                .enum(['doc', 'docx', 'sheet', 'mindnote', 'bitable', 'file', 'slides', 'wiki'])
                .describe('文档类型。不传时默认以 wiki 类型查询。 Options:doc(ObjTypeDoc 旧版文档),docx(ObjTypeDocx 新版文档),sheet(ObjTypeSheet 表格),mindnote(ObjTypeMindNote 思维导图),bitable(ObjTypeBitable 多维表格),file(ObjTypeFile 文件),slides(ObjTypeSlides 幻灯片),wiki(ObjTypeWiki 知识库节点)')
                .optional(),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
...
exports.wikiV2SpaceMemberCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            member_type: zod_1.z
                .string()
                .describe('要添加的成员或管理员的身份类型。可选值：- openchat：群组 ID。参考获取- userid：用户 ID。详情参考- email：用户邮箱- opendepartmentid：部门 ID。参考获取- openid：用户的 Open ID。详情参考- unionid：用户的 Union ID。详情参考'),
            member_id: zod_1.z
                .string()
                .describe('成员或管理员的 ID，值的类型由 member_type 参数决定。参考 member_type 的描述获取不同类型的 ID'),
            member_role: zod_1.z.string().describe('成员的角色类型。可选值:- admin：管理员- member：成员'),
        }),
        params: zod_1.z.object({ need_notification: zod_1.z.boolean().describe('添加权限后是否通知对方').optional() }),
        path: zod_1.z.object({
            space_id: zod_1.z
                .string()
                .describe('知识空间 ID。可通过以下两种方式获取。了解更多，参考。- 调用 获取- 如果你是知识库管理员，可以进入知识库设置页面，复制地址栏的数字部分：https://sample.feishu.cn/wiki/settings/==6870403571079249922==')
...
exports.wikiV2SpaceMemberDelete = {
    ...
    schema: {
        data: zod_1.z.object({
            member_type: zod_1.z
                .string()
                .describe('“openchat” - 群id “userid” - 用户id“email” - 邮箱“opendepartmentid” - 部门id“openid” - 应用openid“unionid” - [unionid](/:ssltoken/home/user-identity-introduction/union-id)'),
            member_role: zod_1.z.string().describe('角色:“admin” - 管理员“member” - 成员'),
            type: zod_1.z
        ...
        path: zod_1.z.object({
            space_id: zod_1.z.string().describe('知识空间id'),
            member_id: zod_1.z.string().describe('成员id，值的类型由请求体的 member_type 参数决定'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.wikiV2SpaceMemberList = {
    ...
    schema: {
        ...
        path: zod_1.z.object({ space_id: zod_1.z.string().describe('知识空间 ID') }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
    ...
    schema: {
        data: zod_1.z.object({
            ...
            target_space_id: zod_1.z
                ...
                .optional(),
            title: zod_1.z.string().describe('复制后的新标题。如果填空，则新标题为空。如果不填，则使用原节点标题').optional(),
        }),
        path: zod_1.z.object({
            space_id: zod_1.z.string().describe('知识空间id').optional(),
            node_token: zod_1.z.string().describe('节点token').optional(),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
    ...
    schema: {
        data: zod_1.z.object({
            obj_type: zod_1.z
                ...
                .describe('文档类型，对于快捷方式，该字段是对应的实体的obj_type。 Options:doc(ObjTypeDoc 已废弃，创建文档请使用`docx`。详情参考。),sheet(ObjTypeSheet 表格),mindnote(ObjTypeMindNote 思维导图),bitable(ObjTypeBitable 多维表格),file(ObjTypeFile 文件),docx(ObjTypeDocx 新版文档),slides(ObjTypeSlides 幻灯片)'),
            parent_node_token: zod_1.z.string().describe('父节点 token。若当前节点为一级节点，父节点 token 为空').optional(),
            node_type: zod_1.z
                .enum(['origin', 'shortcut'])
                .describe('节点类型 Options:origin(NodeTypeEntity 实体),shortcut(NodeTypeShortCut 快捷方式)'),
            origin_node_token: zod_1.z.string().describe('快捷方式对应的实体node_token，当节点为快捷方式时，该值不为空').optional(),
...
exports.wikiV2SpaceNodeList = {
    ...
    path: '/open-apis/wiki/v2/spaces/:space_id/nodes',
    httpMethod: 'GET',
    description: '[Feishu/Lark]-云文档-知识库-节点-获取知识空间子节点列表-此接口用于分页获取Wiki节点的子节点列表。此接口为分页接口。由于权限过滤，可能返回列表为空，但分页标记（has_more）为true，可以继续分页请求',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z.object({
            page_size: zod_1.z.number().describe('分页大小').optional(),
            page_token: zod_1.z
                ...
                .describe('分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果')
                .optional(),
            parent_node_token: zod_1.z.string().describe('父节点token').optional(),
        }),
        path: zod_1.z.object({ space_id: zod_1.z.string().describe('知识空间id').optional() }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.wikiV2SpaceNodeMove = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云文档-知识库-节点-移动知识空间节点-此方法用于在Wiki内移动节点，支持跨知识空间移动。如果有子节点，会携带子节点一起移动',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            target_parent_token: zod_1.z.string().describe('移动到的父节点token').optional(),
            target_space_id: zod_1.z.string().describe('移动到的知识空间ID').optional(),
        }),
        path: zod_1.z.object({
            space_id: zod_1.z.string().describe('知识空间id'),
            node_token: zod_1.z.string().describe('需要迁移的节点token'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.wikiV2SpaceNodeMoveDocsToWiki = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云文档-知识库-云文档-移动云空间文档至知识空间-该接口允许移动云空间文档至知识空间，并挂载在指定位置',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            parent_wiki_token: zod_1.z.string().describe('节点的父亲token。传空或不传时将移动为知识空间一级节点').optional(),
            obj_type: zod_1.z
                .enum(['doc', 'sheet', 'bitable', 'mindnote', 'docx', 'file', 'slides'])
                .describe('文档类型 Options:doc(ObjTypeDoc 旧版文档),sheet(ObjTypeSheet 表格),bitable(ObjTypeBitable 多维表格),mindnote(ObjTypeMindNote 思维导图),docx(ObjTypeDocx 新版文档),file(ObjTypeFile 文件),slides(ObjTypeSlides slides（幻灯片）)'),
            obj_token: zod_1.z.string().describe('文档token'),
            apply: zod_1.z
                .boolean()
                .describe('没有权限时，是否申请移动文档。如果申请移动，文档将在处理人同意时自动移动至指定位置')
                .optional(),
        }),
        path: zod_1.z.object({ space_id: zod_1.z.string().describe('知识库id') }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.wikiV2SpaceNodeUpdateTitle = {
    ...
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({ title: zod_1.z.string().describe('节点新标题') }),
        path: zod_1.z.object({
            space_id: zod_1.z.string().describe('知识空间ID').optional(),
            node_token: zod_1.z.string().describe('节点token').optional(),
        }),
    ...
    schema: {
        data: zod_1.z.object({
            ...
            security_setting: zod_1.z
                ...
                .optional(),
            comment_setting: zod_1.z.string().describe('可阅读用户可否评论： "allow" - 允许 "not_allow" - 不允许').optional(),
        }),
        path: zod_1.z.object({ space_id: zod_1.z.string().describe('知识空间id').optional() }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
...
exports.wikiV2TaskGet = {
    ...
    schema: {
        params: zod_1.z.object({
            task_type: zod_1.z.literal('move').describe('任务类型 Options:move(任务)'),
        }),
