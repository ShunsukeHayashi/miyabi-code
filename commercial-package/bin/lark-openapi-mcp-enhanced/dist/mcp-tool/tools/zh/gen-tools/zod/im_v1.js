...
exports.imV1BatchMessageGetProgress = {
    ...
    schema: {
        path: zod_1.z.object({
            batch_message_id: zod_1.z
                .string()
                .describe('待查询的批量消息任务 ID，该 ID 为[批量发送消息]( https://open.feishu.cn/document/ukTMukTMukTM/ucDO1EjL3gTNx4yN4UTM)接口返回值中的 `message_id` 字段，用于标识一次批量发送消息请求'),
        }),
    },
};
exports.imV1BatchMessageReadUser = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-消息-批量消息-查询批量消息推送和阅读人数-后，可通过该接口查询消息推送的总人数以及消息已读人数',
    accessTokens: ['tenant'],
    schema: {
        path: zod_1.z.object({
            batch_message_id: zod_1.z
...
exports.imV1ChatAnnouncementGet = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            chat_id: zod_1.z
                .string()
                .describe('群 ID。获取方式：- ，从返回结果中获取该群的 chat_id。- 调用接口，可以查询用户或机器人所在群的 chat_id。- 调用，可搜索用户或机器人所在的群、对用户或机器人公开的群的 chat_id。**注意**：单聊（群类型为 `p2p`）不支持获取群公告'),
        }),
...
exports.imV1ChatAnnouncementPatch = {
    ...
    path: '/open-apis/im/v1/chats/:chat_id/announcement',
    httpMethod: 'PATCH',
    description: '[Feishu/Lark]-群组-群公告-更新群公告信息-更新指定群组中的群公告信息。更新的公告内容格式和更新的格式相同，不支持新版云文档格式',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            revision: zod_1.z
                .string()
                .describe('文档当前版本号 int64 类型，可调用接口，从返回结果中获取。**注意**：传入的版本号和最新版本号的差距不能超过 100'),
            requests: zod_1.z
        ...
        path: zod_1.z.object({
            chat_id: zod_1.z
                .string()
                .describe('群 ID。获取方式：- ，从返回结果中获取该群的 chat_id。- 调用接口，可以查询用户或机器人所在群的 chat_id。- 调用，可搜索用户或机器人所在的群、对用户或机器人公开的群的 chat_id。**注意**：单聊（群类型为 `p2p`）不支持更新群公告'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.imV1ChatCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            avatar: zod_1.z
                .string()
                .describe('群头像对应的 Image Key- 可通过获取（注意：上传图片的 ==image_type== 需要指定为 ==avatar==）- 不传值则使用系统默认头像')
                .optional(),
            name: zod_1.z
                .string()
            ...
            owner_id: zod_1.z
                .string()
                .describe('创建群时指定的群主，不填时指定建群的机器人为群主。群主 ID 类型在查询参数 ==user_id_type== 中指定；推荐使用 OpenID，获取方式可参考文档**注意**：开启对外共享能力的机器人在创建外部群时，机器人不能为群主，必须指定某一用户作为群主。此外，添加外部用户进群时，外部用户必须和群主已成为飞书好友')
                .optional(),
            user_id_list: zod_1.z
                .array(zod_1.z.string().describe('user_id'))
                .describe('创建群时邀请的群成员，不填则不邀请成员。ID 类型在查询参数 ==user_id_type== 中指定；推荐使用 OpenID，获取方式可参考文档**注意**：- 最多同时邀请 50 个用户- 为便于在客户端查看效果，建议调试接口时加入开发者自身 ID- 如果需要邀请外部用户，则外部用户必须和群主已成为飞书好友- 如何获取外部用户的 open_id，参考')
                .optional(),
            bot_id_list: zod_1.z
                .array(zod_1.z.string().describe('app_id'))
                .describe('创建群时邀请的群机器人，不填则不邀请机器人。可参考来获取应用的 App ID **注意：**- 操作此接口的机器人会自动入群，无需重复填写- 拉机器人入群请使用 `app_id`- 最多同时邀请 5 个机器人，且邀请后群组中机器人数量不能超过 15 个')
                .optional(),
...
exports.imV1ChatDelete = {
    ...
    schema: {
        path: zod_1.z.object({
            chat_id: zod_1.z
                .string()
                .describe('群 ID。获取方式：- ，从返回结果中获取该群的 chat_id。- 调用接口，可以查询用户或机器人所在群的 chat_id。- 调用，可搜索用户或机器人所在的群、对用户或机器人公开的群的 chat_id。**注意**：仅支持群模式为 `group` 的群组 ID。你可以调用接口，在返回结果中查看 `chat_mode` 参数取值是否为 `group`'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.imV1ChatGet = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            chat_id: zod_1.z
                .string()
                .describe('群 ID。获取方式：- ，从返回结果中获取该群的 chat_id。- 调用接口，可以查询用户或机器人所在群的 chat_id。- 调用，可搜索用户或机器人所在的群、对用户或机器人公开的群的 chat_id'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.imV1ChatLink = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            chat_id: zod_1.z
                .string()
                .describe('群 ID。获取方式：- ，从返回结果中获取该群的 chat_id。- 调用接口，可以查询用户或机器人所在群的 chat_id。- 调用，可搜索用户或机器人所在的群、对用户或机器人公开的群的 chat_id。**注意**：单聊、密聊、团队群不支持分享群链接'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
...
exports.imV1ChatManagersAddManagers = {
    ...
    schema: {
        data: zod_1.z.object({
            manager_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('要设置为管理员的 ID，ID 类型与查询参数 member_id_type 取值一致。- 如果是用户（member_id_type 取值为 user_id/open_id/union_id），推荐使用用户的 open_id，获取方式可参考文档。- 如果是机器人（member_id_type 取值为 app_id），请参考**注意**：- 对于普通群，最多可指定 10 个管理员。- 对于超大群，最多可指定 20 个管理员。- 单次请求指定机器人时，最多可指定 5 个机器人。超大群和普通群的区别，参见')
                .optional(),
        }),
        params: zod_1.z.object({
            member_id_type: zod_1.z
                .enum(['open_id', 'union_id', 'user_id', 'app_id'])
                .describe('用户 ID 类型 Options:open_id(标识一个用户在某个应用中的身份。同一个用户在不同应用中的 Open ID 不同。),union_id(标识一个用户在某个应用开发商下的身份。同一用户在同一开发商下的应用中的 Union ID 是相同的，在不同开发商下的应用中的 Union ID 是不同的。通过 Union ID，应用开发商可以把同个用户在多个应用中的身份关联起来。),user_id(标识一个用户在某个租户内的身份。同一个用户在租户 A 和租户 B 内的 User ID 是不同的。在同一个租户内，一个用户的 User ID 在所有应用（包括商店应用）中都保持一致。User ID 主要用于在不同的应用间打通用户数据。),app_id(飞书开放平台应用的唯一标识。在创建应用时，由系统自动生成，用户不能自行修改。)')
                .optional(),
        }),
        path: zod_1.z.object({
            chat_id: zod_1.z
                .string()
                .describe('群 ID。获取方式：- ，从返回结果中获取该群的 chat_id。- 调用接口，可以查询用户或机器人所在群的 chat_id。- 调用，可搜索用户或机器人所在的群、对用户或机器人公开的群的 chat_id。**注意**：仅支持群模式为 **群组（group）**、**话题（topic）** 的群组 ID。你可以调用接口，在返回结果中查看 `chat_mode` 参数取值是否为 `group`、`topic`'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.imV1ChatManagersDeleteManagers = {
    ...
    schema: {
        data: zod_1.z.object({
            manager_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('要删除的管理员 ID，ID 类型与查询参数 member_id_type 取值一致。- 如果是用户（member_id_type 取值为 user_id/open_id/union_id），推荐使用用户的 open_id，获取方式可参考文档。- 如果是机器人（member_id_type 取值为 app_id），请参考**注意**：每次请求最多指定 50 个用户或者 5 个机器人')
                .optional(),
        }),
        params: zod_1.z.object({
            member_id_type: zod_1.z
    ...
    schema: {
        data: zod_1.z.object({
            id_list: zod_1.z
                ...
                .describe('成员 ID 列表。- 邀请用户进群时推荐使用 OpenID，获取方式可参考文档- 邀请机器人进群时需填写应用的 App ID，请参考**注意**：- 成员列表不可为空- 列表中填写的成员 ID 类型应与 ==member_id_type== 参数中选择的类型相对应- 每次请求最多拉 50 个用户且不超过群人数上限。对于已认证企业的飞书的群人数默认上限：普通群 5000 人，会议群 3000 人，话题群 5000 人。若租户管理员配置了群人数上限，则群人数上限为该人数上限- 最多同时邀请 5 个机器人，且邀请后群组中机器人数量不能超过 15 个')
                .optional(),
        }),
        params: zod_1.z.object({
            member_id_type: zod_1.z
    ...
    schema: {
        data: zod_1.z.object({
            id_list: zod_1.z
                ...
                .describe('成员 ID 列表。ID 类型与查询参数 member_id_type 的取值一致。- 移除群内的用户时推荐使用 OpenID，获取方式可参考文档。- 移除群内的机器人时需填写应用的 App ID，请参考。**注意**：- 成员列表不可为空。- 每次请求，最多移除 50 个用户或者 5 个机器人')
                .optional(),
        }),
        params: zod_1.z.object({
            member_id_type: zod_1.z
...
exports.imV1ChatMembersMeJoin = {
    ...
    schema: {
        path: zod_1.z.object({
            chat_id: zod_1.z
                .string()
                .describe('群 ID。获取方式：- ，从返回结果中获取该群的 chat_id。- 调用接口，可以查询用户或机器人所在群的 chat_id。- 调用，可搜索用户或机器人所在的群、对用户或机器人公开的群的 chat_id。**注意**：- 仅支持公开群（public），你可以调用接口，在返回结果中查看 `chat_type ` 参数取值是否为 `public`。- 群成员数量达到上限时无法进群。对于已认证企业的飞书的群人数默认上限：普通群 5000 人，会议群 3000 人，话题群 5000 人'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
    ...
    schema: {
        data: zod_1.z.object({
            ...
            chat_menu_item: zod_1.z
                ...
                image_key: zod_1.z
                    .string()
                    .describe('图标的 key 值。通过  接口上传 message 类型图片获取 image_key，并传入该值。**注意**：如果一级菜单有二级菜单，则此一级菜单不能设置图标')
                    .optional(),
        ...
        path: zod_1.z.object({
            chat_id: zod_1.z
                .string()
                .describe('群 ID。获取方式：- ，从返回结果中获取该群的 chat_id。- 调用接口，可以查询用户或机器人所在群的 chat_id。- 调用，可搜索用户或机器人所在的群、对用户或机器人公开的群的 chat_id。**注意**：仅支持群模式为 **群组（group）** 的群组 ID。你可以调用接口，在返回结果中查看 `chat_mode` 参数取值是否为 `group`')
                .optional(),
            menu_item_id: zod_1.z.string().describe('一级菜单或者二级菜单的 ID，ID 可通过  接口获取').optional(),
        }),
    },
};
            ...
            menu_tree: zod_1.z
                ...
                chat_menu_top_levels: zod_1.z
                    ...
                    chat_menu_item: zod_1.z
                        ...
                        image_key: zod_1.z
                            .string()
                            .describe('一级菜单图标的 key 值。通过  接口上传 message 类型图片获取 image_key，并传入该值。**注意**：如果一级菜单有二级菜单，则此一级菜单不能设置图标')
                            .optional(),
                    ...
                    children: zod_1.z
                        ...
                        chat_menu_item: zod_1.z
                            ...
                            image_key: zod_1.z
                                .string()
                                .describe('二级菜单图标的 key 值。通过  接口上传 message 类型图片获取 image_key，并传入该值')
                                .optional(),
                            name: zod_1.z
                                .string()
    ...
    schema: {
        data: zod_1.z.object({
            chat_menu_top_level_ids: zod_1.z
                ...
                .describe('通过一级菜单的 ID 进行排序。数组内的元素排序对应群组内一级菜单从左往右的排序。ID 可通过  接口获取。**说明**：进行排序的 ID 列表需要跟群内存在的一级菜单 ID 列表对齐'),
        }),
        path: zod_1.z.object({
            chat_id: zod_1.z
                .string()
    ...
    schema: {
        data: zod_1.z.object({
            ...
            moderator_added_list: zod_1.z
                ...
                .describe('当 `moderation_setting ` 取值为 `moderator_list` 时，以 ID 列表形式添加可发言的用户。**注意**：- ID 类型与查询参数 user_id_type 取值一致，推荐使用 OpenID，获取方式可参考文档。- 列表内的用户如果不在群组内，则会被自动过滤掉。- 请求时，请确保 `moderator_added_list` 和 `moderator_removed_list` 两个参数内的 ID 不重复')
                .optional(),
            moderator_removed_list: zod_1.z
                .array(zod_1.z.string().describe('用户id'))
                .describe('当 `moderation_setting ` 取值为 `moderator_list` 时，以 ID 列表形式移除可发言的用户。**注意**：- ID 类型与查询参数 user_id_type 取值一致，推荐使用 OpenID，获取方式可参考文档。- 列表内的用户如果不在群组内，则会被自动过滤掉。- 请求时，请确保 `moderator_added_list` 和 `moderator_removed_list` 两个参数内的 ID 不重复')
                .optional(),
        }),
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() }),
        path: zod_1.z.object({
...
exports.imV1ChatTabCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            chat_tabs: zod_1.z
                ...
                tab_type: zod_1.z
                    ...
                    .describe('会话标签页类型**注意**：只支持添加 doc、url 类型的标签页，其他字段为只读字段 Options:message(消息类型),doc_list(DocList 云文档列表),doc(文档),pin(Pin),meeting_minute(MeetingMinute 会议纪要),chat_announcement(ChatAnnouncement 群公告),url(URL),file(文件),files_resources(合并类型，包含文件、Doc 文档、URL 链接),images_videos(合并类型，包含图片、视频)'),
                tab_content: zod_1.z
                    .object({
                    url: zod_1.z
                        .string()
                ...
                tab_config: zod_1.z
                    ...
                    icon_key: zod_1.z
                        .string()
                        .describe('会话标签页的图标。需要先调用 接口，图片类型设置为 message 上传图片，然后获取 `image_key` 后传入当前参数')
                        .optional(),
        ...
        path: zod_1.z.object({
            chat_id: zod_1.z
                .string()
                .describe('群 ID。获取方式：- ，从返回结果中获取该群的 chat_id。- 调用接口，可以查询用户或机器人所在群的 chat_id。- 调用，可搜索用户或机器人所在的群、对用户或机器人公开的群的 chat_id。**注意**：仅支持群模式为 **群组（group）**、**单聊（p2p）** 的群组 ID。你可以调用接口，在返回结果中查看 `chat_mode` 参数取值是否为 `group`、`p2p`'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.imV1ChatTabDeleteTabs = {
    ...
    schema: {
        data: zod_1.z.object({
            tab_ids: zod_1.z.array(zod_1.z.string().describe('群标签id')).describe('会话标签页 ID 列表，ID 可以在或者接口返回值中获取'),
        }),
        path: zod_1.z.object({
            chat_id: zod_1.z
                .string()
    ...
    schema: {
        data: zod_1.z.object({
            tab_ids: zod_1.z
                ...
                .describe('会话标签页 ID 列表，ID 可以在或者接口返回值中获取。**注意**：- 必须包含会话内全部标签页的 ID。- 当前参数的排序，对应会话内从左往右的排序。- 会话内消息类型的标签页固定在第一顺位')
                .optional(),
        }),
        path: zod_1.z.object({
            chat_id: zod_1.z
...
exports.imV1ChatTabUpdateTabs = {
    ...
    schema: {
        data: zod_1.z.object({
            chat_tabs: zod_1.z
                .array(zod_1.z.object({
                tab_id: zod_1.z.string().describe('会话标签页 ID，ID 可以在或者接口返回值中获取').optional(),
                tab_name: zod_1.z
                    .string()
                    .describe('会话标签页名称**注意**：会话标签页的名称不能超过 30 个字符（最多 10 个汉字）')
                    .optional(),
                tab_type: zod_1.z
                    ...
                    .describe('会话标签页类型**注意**：只支持更新 doc、url 类型的标签页，其他字段为只读字段 Options:message(消息类型),doc_list(DocList 云文档列表),doc(文档),pin(Pin),meeting_minute(MeetingMinute 会议纪要),chat_announcement(ChatAnnouncement 群公告),url(URL),file(文件),files_resources(合并类型，包含文件、Doc文档、URL链接),images_videos(合并类型，包含图片、视频)'),
                tab_content: zod_1.z
                    .object({
                    url: zod_1.z
                        .string()
    ...
    schema: {
        data: zod_1.z.object({
            chat_top_notice: zod_1.z
                ...
                message_id: zod_1.z
                    .string()
                    .describe('消息 ID。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取')
                    .optional(),
            }))
                .describe('群置顶配置'),
        }),
...
exports.imV1ChatUpdate = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            owner_id: zod_1.z
                .string()
                .describe('新群主的用户 ID，不转让群主时无需填写。ID 类型与查询参数 user_id_type 取值一致，ID 类型推荐使用 OpenID，获取方式可参考文档')
                .optional(),
            join_message_visibility: zod_1.z
                .string()
                .describe('成员入群提示消息的可见性**可选值有**：- `only_owner`：仅群主和管理员可见- `all_members`：所有成员可见- `not_anyone`：任何人均不可见')
...
exports.imV1MessageCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            receive_id: zod_1.z
                .string()
                .describe('消息接收者的 ID，ID 类型与查询参数 `receive_id_type` 的取值一致。**注意事项**：- 给用户发送消息时，用户需要在机器人的内。例如，你需要给企业全员发送消息，则需要将应用的可用范围设置为全体员工。- 给群组发送消息时，机器人需要在该群组中，且在群组内拥有发言权限。- 如果消息接收者为用户，推荐使用用户的 `open_id`'),
            msg_type: zod_1.z
                .string()
                .describe('消息类型。**可选值有**：- text：文本- post：富文本- image：图片- file：文件- audio：语音- media：视频- sticker：表情包- interactive：卡片- share_chat：分享群名片- share_user：分享个人名片- system：系统消息。该类型仅支持在机器人单聊内推送系统消息，不支持在群聊内使用，例如下图所示突出新会话。 不同消息类型的详细介绍，参见'),
            content: zod_1.z
                .string()
        ...
        params: zod_1.z.object({
            receive_id_type: zod_1.z
                ...
                .describe('消息接收者 ID 类型。支持 open_id/union_id/user_id/email/chat_id Options:open_id(标识一个用户在某个应用中的身份。同一个用户在不同应用中的 Open ID 不同。),union_id(标识一个用户在某个应用开发商下的身份。同一用户在同一开发商下的应用中的 Union ID 是相同的，在不同开发商下的应用中的 Union ID 是不同的。通过 Union ID，应用开发商可以把同个用户在多个应用中的身份关联起来。),user_id(标识一个用户在某个租户内的身份。同一个用户在租户 A 和租户 B 内的 User ID 是不同的。在同一个租户内，一个用户的 User ID 在所有应用（包括商店应用）中都保持一致。User ID 主要用于在不同的应用间打通用户数据。。),email(以用户的真实邮箱来标识用户。),chat_id(以群 ID 来标识群聊。</md-enum-item>)'),
        }),
    },
};
exports.imV1MessageDelete = {
    ...
    schema: {
        path: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('待撤回的消息 ID。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
    ...
    schema: {
        ...
        params: zod_1.z.object({
            receive_id_type: zod_1.z
                ...
                .describe('消息接收者 ID 类型。 Options:open_id(标识一个用户在某个应用中的身份。同一个用户在不同应用中的 Open ID 不同。),user_id(标识一个用户在某个租户内的身份。同一个用户在租户 A 和租户 B 内的 User ID 是不同的。在同一个租户内，一个用户的 User ID 在所有应用（包括商店应用）中都保持一致。User ID 主要用于在不同的应用间打通用户数据。),union_id(标识一个用户在某个应用开发商下的身份。同一用户在同一开发商下的应用中的 Union ID 是相同的，在不同开发商下的应用中的 Union ID 是不同的。通过 Union ID，应用开发商可以把同个用户在多个应用中的身份关联起来。),email(以用户的真实邮箱来标识用户。),chat_id(以群 ID 来标识群聊。),thread_id(以话题 ID 来标识话题。了解更多：</md-enum-item>**当值为 `user_id`，字段权限要求**：<md-perm name="contact:user.employee_id:readonly" desc="获取用户 user ID" support_app_types="custom" tags="">获取用户 user ID</md-perm>)'),
            uuid: zod_1.z
                .string()
        ...
        path: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('待转发的消息 ID。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
        }),
    },
};
    ...
    schema: {
        params: zod_1.z.object({
            ...
            container_id: zod_1.z
                .string()
                .describe('容器 ID。ID 类型与 container_id_type 取值一致。- 群聊或单聊的 ID 获取方式参见。- 话题 ID 获取参见的 **如何获取 thread_id** 章节'),
            start_time: zod_1.z
                .string()
    ...
    schema: {
        data: zod_1.z.object({
            ...
            message_id_list: zod_1.z
                .array(zod_1.z.string())
                .describe('待转发的消息 ID 列表，列表内的消息必须来自同一个会话。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
        }),
        params: zod_1.z.object({
            receive_id_type: zod_1.z
                .enum(['open_id', 'user_id', 'union_id', 'email', 'chat_id', 'thread_id'])
...
exports.imV1MessagePatch = {
    ...
    path: '/open-apis/im/v1/messages/:message_id',
    httpMethod: 'PATCH',
    description: '[Feishu/Lark]-消息-消息卡片-更新应用发送的消息卡片-调用该接口，通过消息 ID（message_id）更新指定的消息卡片内容。如果你需要在用户与卡片进行交互后延迟更新卡片，或者通过用户 ID 更新部分成员接收到的卡片内容，可调用接口',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            content: zod_1.z
                .string()
                .describe('消息卡片的内容，支持卡片 JSON 或构建的卡片，需为 JSON 结构序列化后的字符串。 - 要使用卡片 JSON，参考。 - 要使用构建的卡片模板，你需传入 `type` 和 `data` 参数。参考 文档末尾的字段说明传值。**注意**：- 更新的卡片消息最大不能超过 30 KB。若消息中包含大量样式标签，会使实际消息体长度大于你输入的请求体长度。- 以下示例值未转义，使用时请注意将其转为 JSON 序列化后的字符串'),
        }),
        path: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('待更新的消息 ID，仅支持更新卡片（消息类型为 `interactive`）。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.imV1MessagePushFollowUp = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('机器人发送的消息 ID。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
        }),
    },
};
    ...
    schema: {
        data: zod_1.z.object({
            reaction_type: zod_1.z
                .object({
                emoji_type: zod_1.z.string().describe('emoji 类型。支持的表情与对应的 emoji_type 值参见'),
            })
                .describe('表情类型'),
        }),
        path: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('待添加表情回复的消息 ID。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.imV1MessageReactionDelete = {
    ...
    schema: {
        path: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('待删除表情回复的消息 ID。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
            reaction_id: zod_1.z
                .string()
                .describe('待删除的表情回复 ID，该 ID 获取方式：- 调用接口添加表情回复后，在返回结果中获取。- 调用接口，获取某一表情回复的 ID'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.imV1MessageReactionList = {
    ...
    schema: {
        params: zod_1.z.object({
            reaction_type: zod_1.z
                .string()
                .describe('待查询的表情类型，支持的枚举值参考中的 emoji_type 值。**注意**：该参数为可选参数，不传入该参数时将查询消息内所有的表情回复')
                .optional(),
            page_token: zod_1.z
                .string()
                .describe('分页标记，第一次请求不填，表示从头开始遍历；分页查询结果还有更多项时会同时返回新的 page_token，下次遍历可采用该 page_token 获取查询结果')
        ...
        path: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('待查询的消息ID。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.imV1MessageReadUsers = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('待查询的消息 ID。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
        }),
    },
};
exports.imV1MessageReply = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            msg_type: zod_1.z
                .string()
                .describe('消息类型。**可选值有**：- text：文本- post：富文本- image：图片- file：文件- audio：语音- media：视频- sticker：表情包- interactive：卡片- share_chat：分享群名片- share_user：分享个人名片不同消息类型的详细介绍，参见'),
        ...
        path: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('待回复的消息的 ID。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
        }),
    },
};
exports.imV1MessageUpdate = {
    ...
    path: '/open-apis/im/v1/messages/:message_id',
    httpMethod: 'PUT',
    description: '[Feishu/Lark]-消息-消息管理-编辑消息-调用该接口编辑已发送的消息内容，支持编辑文本、富文本消息。如需编辑卡片消息，请使用接口',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            ...
            content: zod_1.z
                .string()
                .describe('消息内容，JSON 结构序列化后的字符串。该参数的取值与 `msg_type` 对应，例如 `msg_type` 取值为 `text`，则该参数需要传入文本类型的内容。**注意：**- JSON字符串需进行转义，如换行符转义后为`\\`- 文本消息请求体最大不能超过 150 KB- 富文本消息请求体最大不能超过 30 KB- 如果消息中包含样式标签，会使实际消息体长度大于您输入的请求体长度。了解不同类型的消息内容格式、使用限制，可参见'),
        }),
        path: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('待编辑的消息 ID，仅支持编辑文本（text）、富文本（post）消息。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
        }),
    },
};
exports.imV1MessageUrgentApp = {
    ...
    httpMethod: 'PATCH',
    description: '[Feishu/Lark]-消息-消息加急-发送应用内加急-调用该接口把指定消息加急给目标用户，加急仅在飞书客户端内通知。了解加急可参见',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            user_id_list: zod_1.z
        ...
        path: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('待加急的消息 ID。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取。**注意**：不支持加急（对应的消息ID 格式为 `bm_xxx`）'),
        }),
    },
};
exports.imV1MessageUrgentPhone = {
    ...
    httpMethod: 'PATCH',
    description: '[Feishu/Lark]-消息-消息加急-发送电话加急-调用该接口把指定消息加急给目标用户，加急将通过飞书客户端和电话进行通知。了解加急可参见',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            user_id_list: zod_1.z
...
exports.imV1MessageUrgentSms = {
    ...
    httpMethod: 'PATCH',
    description: '[Feishu/Lark]-消息-消息加急-发送短信加急-调用该接口把指定消息加急给目标用户，加急将通过飞书客户端和短信进行通知。了解加急可参见',
    accessTokens: ['tenant'],
    schema: {
        data: zod_1.z.object({
            user_id_list: zod_1.z
...
exports.imV1PinCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('待 Pin 的消息 ID。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.imV1PinDelete = {
    ...
    schema: {
        path: zod_1.z.object({
            message_id: zod_1.z
                .string()
                .describe('待移除 Pin 的消息 ID。ID 获取方式： - 调用接口后，从响应结果的 `message_id` 参数获取。- 监听事件，当触发该事件后可以从事件体内获取消息的 `message_id`。- 调用接口，从响应结果的 `message_id` 参数获取'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.imV1PinList = {
    ...
    schema: {
        params: zod_1.z.object({
            chat_id: zod_1.z.string().describe('待获取 Pin 消息的群组 ID。获取方式参见'),
            start_time: zod_1.z
                .string()
    ...
    schema: {
        ...
        params: zod_1.z.object({
            receive_id_type: zod_1.z
                ...
                .describe('消息接收者 ID 类型。 Options:open_id(OpenID 标识一个用户在某个应用中的身份。同一个用户在不同应用中的 Open ID 不同。),union_id(UnionID 标识一个用户在某个应用开发商下的身份。同一用户在同一开发商下的应用中的 Union ID 是相同的，在不同开发商下的应用中的 Union ID 是不同的。通过 Union ID，应用开发商可以把同个用户在多个应用中的身份关联起来。),user_id(UserID 标识一个用户在某个租户内的身份。同一个用户在租户 A 和租户 B 内的 User ID 是不同的。在同一个租户内，一个用户的 User ID 在所有应用（包括商店应用）中都保持一致。User ID 主要用于在不同的应用间打通用户数据。),email(以用户的真实邮箱来标识用户。),chat_id(ChatID 以群 ID 来标识群聊。。),thread_id(ThreadID 以话题 ID 来标识话题。了解更多：</md-enum-item>**当值为 `user_id`，字段权限要求**：<md-perm name="contact:user.employee_id:readonly" desc="获取用户 user ID" support_app_types="custom" tags="">获取用户 user ID</md-perm>)'),
            uuid: zod_1.z
                .string()
        ...
        path: zod_1.z.object({
            thread_id: zod_1.z.string().describe('要转发的话题ID，获取方式参见的 **如何获取 thread_id** 章节'),
        }),
    },
};
exports.imV1Tools = [
