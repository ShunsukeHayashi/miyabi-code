...
exports.driveV1ExportTaskCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云空间-文件-导出云文档-创建导出任务-该接口用于创建导出文件的任务，并返回导出任务 ID。导出文件指将飞书文档、电子表格、多维表格导出为本地文件，包括 Word、Excel、PDF、CSV 格式。该接口为异步接口，需要继续调用接口获取导出结果。了解完整的导出步骤，参考',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            file_extension: zod_1.z
                .enum(['docx', 'pdf', 'xlsx', 'csv'])
                .describe('将云文档导出为本地文件后，本地文件的扩展名。了解各类云文档支持导出的文件格式，参考。 Options:docx(Microsoft Word 格式),pdf(PDF 格式),xlsx(Microsoft Excel 格式),csv(CSV 格式)'),
            token: zod_1.z.string().describe('要导出的云文档的 token。获取方式参考 '),
            type: zod_1.z
            ...
            sub_id: zod_1.z
                .string()
                .describe('导出飞书电子表格或多维表格为 CSV 文件时，需传入电子表格工作表的 ID 或多维表格数据表的 ID：- 电子表格可调用 接口获取返回的 `sheet_id` 的值作为该参数的值- 多维表格可调用接口获取返回的 `table_id` 的值作为该参数的值')
                .optional(),
        }),
...
exports.driveV1ExportTaskGet = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-云空间-文件-导出云文档-查询导出任务结果-根据返回的导出任务 ID（ticket）轮询导出任务结果，并返回导出文件的 token。你可使用该 token 继续调用接口将导出的产物下载到本地。了解完整的导出文件步骤，参考',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z.object({
            token: zod_1.z.string().describe('要导出的云文档的 token。获取方式参考。你可参考以下请求示例了解如何使用查询参数'),
        }),
        path: zod_1.z.object({
            ticket: zod_1.z.string().describe('导出任务 ID。调用 获取'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.driveV1FileCommentBatchQuery = {
    ...
    schema: {
        ...
        path: zod_1.z.object({ file_token: zod_1.z.string().describe('文档 Token').optional() }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
...
exports.driveV1FileCommentList = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            file_token: zod_1.z.string().describe('云文档的 token。获取方式参考 '),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
...
exports.driveV1FileCopy = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云文档-云空间-文件-复制文件-该接口用于将用户云空间中的文件复制至其它文件夹下。不支持复制文件夹。该接口为异步接口',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            ...
            type: zod_1.z
                ...
                .describe('被复制的源文件的类型。该参数为必填，请忽略左侧必填列的“否”。若该参数值为空或与实际文件类型不匹配，接口将返回失败。 Options:file(文件类型),doc(文档类型),sheet(电子表格类型),bitable(多维表格类型),docx(新版文档类型),mindnote(思维笔记类型),slides(幻灯片类型)')
                .optional(),
            folder_token: zod_1.z
                .string()
                .describe('目标文件夹的 token。若传入根文件夹 token，表示复制的新文件将被创建在云空间根目录。了解如何获取文件夹 token，参考'),
            extra: zod_1.z
        ...
        path: zod_1.z.object({
            file_token: zod_1.z.string().describe('被复制的源文件的 token。了解如何获取文件 token，参考').optional(),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.driveV1FileCreateFolder = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            folder_token: zod_1.z
                .string()
                .describe('父文件夹的 token。参数为空字符串时，表示在根目录下创建文件夹。你可参考获取某个文件夹的 token。了解更多，参考'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.driveV1FileCreateShortcut = {
    ...
    schema: {
        data: zod_1.z.object({
            parent_token: zod_1.z.string().describe('目标父文件夹的 token，获取方式见'),
            refer_entity: zod_1.z
                .object({
                refer_token: zod_1.z.string().describe('源文件的 token。获取方式见'),
                refer_type: zod_1.z
...
exports.driveV1FileDeleteSubscribe = {
    ...
    httpMethod: 'DELETE',
    description: '[Feishu/Lark]-云文档-云空间-事件-取消云文档事件订阅-该接口用于取消订阅云文档的通知事件。了解事件订阅的配置流程和使用场景，参考。了解云文档支持的事件类型，参考',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z.object({
            file_type: zod_1.z
        ...
        path: zod_1.z.object({
            file_token: zod_1.z.string().describe('云文档的 token。了解如何获取各类云文档的token，参考'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.driveV1FileGetSubscribe = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-云文档-云空间-事件-查询云文档事件订阅状态-该接口用于查询云文档事件的订阅状态。了解事件订阅的配置流程和使用场景，参考。了解云文档支持的事件类型，参考',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z.object({
            file_type: zod_1.z
        ...
        path: zod_1.z.object({
            file_token: zod_1.z.string().describe('云文档的 token。了解如何获取各类云文档的 token，参考'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.driveV1FileList = {
    ...
    schema: {
        params: zod_1.z.object({
            ...
            folder_token: zod_1.z
                .string()
                .describe('文件夹的 token。不填写或填空字符串，将获取用户云空间根目录下的清单，且不支持分页。了解如何获取文件夹 token，参考')
                .optional(),
            order_by: zod_1.z
    ...
    schema: {
        data: zod_1.z.object({
            type: zod_1.z
                ...
                .optional(),
            folder_token: zod_1.z.string().describe('目标文件夹的 token。了解如何获取文件夹 token，参考').optional(),
        }),
        path: zod_1.z.object({
            file_token: zod_1.z
                .string()
...
exports.driveV1FileStatisticsGet = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            file_token: zod_1.z.string().describe('文件 token。了解如何获取文件 token，参考').optional(),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.driveV1FileSubscribe = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云文档-云空间-事件-订阅云文档事件-该接口用于订阅云文档的各类通知事件。了解事件订阅的配置流程和使用场景，参考。了解云文档支持的事件类型，参考',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z.object({
            file_type: zod_1.z
...
exports.driveV1FileUploadFinish = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-文件-上传文件-分片上传文件-分片上传文件-完成上传-调用接口将分片全部上传完毕后，你需调用本接口触发完成上传。否则将上传失败。了解完整的上传文件流程，参考',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            upload_id: zod_1.z.string().describe('分片上传事务 ID。通过调用接口获取'),
            block_num: zod_1.z.number().describe('分片的数量。通过调用接口获取'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.driveV1FileUploadPrepare = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-文件-上传文件-分片上传文件-分片上传文件-预上传-发送初始化请求，以获取上传事务 ID 和分片策略，为做准备。平台固定以 4MB 的大小对文件进行分片。了解完整的上传文件流程，参考',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            ...
            parent_node: zod_1.z.string().describe('云空间中文件夹的 token。了解如何获取文件夹 token，参考'),
...
exports.driveV1FileVersionCreate = {
    ...
    schema: {
        ...
        params: zod_1.z.object({ user_id_type: zod_1.z.enum(['open_id', 'union_id', 'user_id']).describe('用户ID类型').optional() }),
        path: zod_1.z.object({
            file_token: zod_1.z.string().describe('源文档的 token，获取方式参考 '),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.driveV1FileVersionDelete = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            file_token: zod_1.z.string().describe('当前版本对应的源文档的 token。获取方式参考'),
...
exports.driveV1FileViewRecordList = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            file_token: zod_1.z.string().describe('文件 token。获取方式参考'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.driveV1ImportTaskCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云空间-文件-导入文件-创建导入任务-该接口用于创建导入文件的任务，并返回导入任务 ID。导入文件指将本地文件如 Word、TXT、Markdown、Excel 等格式的文件导入为某种格式的飞书在线云文档。该接口为异步接口，需要继续调用接口获取导入结果。了解完整的导入文件步骤，参考',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            file_extension: zod_1.z
                .string()
                .describe('要导入的文件的扩展名。了解支持的文件格式，参考。**注意**：此处填写的文件扩展名需与实际文件的后缀名保持严格一致。请注意区分后缀为 “markdown”、“md”、“mark” 的 Markdown 文件，并在填写相关参数时保持后缀名一致。否则将返回 1069910 错误码'),
            file_token: zod_1.z
                .string()
                .describe('要导入文件的 token。创建任务前，你需先调用或接口获取源文件的 token。了解更多，参考'),
            type: zod_1.z
                .string()
                .describe('目标云文档格式。不同文件支持的云文档格式不同。详情参考。可选值如下所示：- `docx`：新版文档- `sheet`：电子表格- `bitable`：多维表格'),
            ...
            point: zod_1.z
                ...
                mount_key: zod_1.z
                    .string()
                    .describe('云文档挂载的文件夹的 token，即云空间下文件夹的 token。空表示云空间根目录。了解如何获取文件夹 token，参考'),
            })
...
exports.driveV1ImportTaskGet = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-云空间-文件-导入文件-查询导入任务结果-根据返回的导入任务 ID（ticket）轮询导入结果。了解完整的导入文件步骤，参考',
    accessTokens: ['tenant', 'user'],
    schema: {
        path: zod_1.z.object({
            ticket: zod_1.z.string().describe('导入任务 ID。调用 获取'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
...
exports.driveV1MediaUploadFinish = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云空间-素材-上传素材-分片上传素材-完成上传-调用接口将分片全部上传完毕后，你需调用本接口触发完成上传。了解完整的分片上传素材流程，参考',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            upload_id: zod_1.z.string().describe('分片上传事务 ID。通过调用接口获取'),
            block_num: zod_1.z.number().describe('分片数量。通过调用接口获取'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.driveV1MediaUploadPrepare = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云空间-素材-上传素材-分片上传素材-预上传-发送初始化请求，以获取上传事务 ID 和分片策略，为做准备。平台固定以 4MB 的大小对素材进行分片。了解完整的分片上传素材流程，参考',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            ...
            parent_node: zod_1.z
                .string()
                .describe('上传点的 token，即要上传的云文档的 token，用于指定素材将要上传到的云文档或位置。参考  了解上传点类型与上传点 token 的对应关系'),
            size: zod_1.z.number().describe('文件的大小，单位为字节'),
            extra: zod_1.z
                .string()
                .describe('以下场景的上传点需通过该参数传入素材所在云文档的 token。extra 参数的格式为`"{\\"drive_route_token\\":\\"素材所在云文档的 token\\"}"`。详情参考')
                .optional(),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
    ...
    schema: {
        data: zod_1.z.object({
            request_docs: zod_1.z
                .array(zod_1.z.object({
                doc_token: zod_1.z.string().describe('文件的 token，获取方式见'),
                doc_type: zod_1.z
...
exports.driveV1PermissionMemberAuth = {
    ...
    httpMethod: 'GET',
    description: '[Feishu/Lark]-云文档-权限-成员-判断当前用户是否有某权限-该接口用于根据 filetoken 判断当前登录用户是否具有某权限',
    accessTokens: ['tenant', 'user'],
    schema: {
        params: zod_1.z.object({
            type: zod_1.z
                .enum(['doc', 'sheet', 'file', 'wiki', 'bitable', 'docx', 'mindnote', 'minutes', 'slides'])
                .describe('文件类型，需要与文件的 token 相匹配 Options:doc(文档),sheet(电子表格),file(云空间文件),wiki(知识库节点),bitable(多维表格),docx(新版文档),mindnote(思维笔记),minutes(妙记),slides(幻灯片)'),
            action: zod_1.z
        ...
        path: zod_1.z.object({ token: zod_1.z.string().describe('文件的 token') }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.driveV1PermissionMemberBatchCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云文档-权限-成员-批量增加协作者权限-该接口可根据云文档 token 批量将用户添加为云文档的协作者',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            members: zod_1.z
        ...
        params: zod_1.z.object({
            type: zod_1.z
                ...
                .describe('云文档类型，需要与云文档的 token 相匹配。 Options:doc(旧版文档),sheet(电子表格),file(云空间文件),wiki(知识库节点),bitable(多维表格),docx(新版文档),folder(文件夹。使用 <md-tag mode="inline" type="token-tenant">tenant_access_token</md-tag> 调用时，需确保文件夹所有者为应用或应用拥有文件夹的可管理权限，你需要将应用作为群机器人添加至群内，然后授予该群组可管理权限。),mindnote(思维笔记),minutes(妙记。目前妙记还不支持 `full_access` 权限角色),slides(幻灯片)'),
            need_notification: zod_1.z
                .boolean()
                .describe('添加权限后是否通知对方。仅当使用 <md-tag mode="inline" type="token-user">user_access_token</md-tag> 调用时有效')
                .optional(),
        }),
        path: zod_1.z.object({
            token: zod_1.z.string().describe('云文档的 token，需要与 type 参数指定的云文档类型相匹配。可参考 '),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
exports.driveV1PermissionMemberCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            member_type: zod_1.z
                .enum(['email', 'openid', 'unionid', 'openchat', 'opendepartmentid', 'userid', 'groupid', 'wikispaceid'])
                .describe('协作者 ID 类型，与协作者 ID 需要对应 Options:email(飞书邮箱),openid(open_id 应用或用户的 Open ID。 - 获取应用 OpenID，参考 - 获取用户 OpenID，参考),unionid(union_id 开放平台 UnionID),openchat(open_chat 开放平台群组 ID),opendepartmentid(open_department_id 开放平台部门 ID。仅当使用 <md-tag mode="inline" type="token-user">user_access_token</md-tag> 调用时有效),userid(user_id 用户自定义 ID),groupid(group_id 自定义用户组 ID),wikispaceid(wiki_space_id 知识空间 ID。仅知识库文档支持该参数，当需要操作知识库文档里的「知识库成员」类型协作者时传该参数)'),
            member_id: zod_1.z.string().describe('协作者 ID，与协作者 ID 类型需要对应'),
            perm: zod_1.z
                .enum(['view', 'edit', 'full_access'])
        ...
        params: zod_1.z.object({
            type: zod_1.z
                .enum(['doc', 'sheet', 'file', 'wiki', 'bitable', 'docx', 'folder', 'mindnote', 'minutes', 'slides'])
                .describe('云文档类型，需要与云文档的 token 相匹配 Options:doc(文档),sheet(电子表格),file(云空间文件),wiki(知识库节点),bitable(多维表格),docx(新版文档),folder(文件夹。使用 <md-tag mode="inline" type="token-tenant">tenant_access_token</md-tag> 调用时，需确保文件夹所有者为应用或应用拥有文件夹的可管理权限，你需要将应用作为群机器人添加至群内，然后授予该群组可管理权限。),mindnote(思维笔记),minutes(妙记),slides(幻灯片)'),
            need_notification: zod_1.z
                .boolean()
                .describe('添加权限后是否通知对方。仅当使用 <md-tag mode="inline" type="token-user">user_access_token</md-tag> 调用时有效')
                .optional(),
...
exports.driveV1PermissionMemberDelete = {
    ...
    schema: {
        ...
        params: zod_1.z.object({
            type: zod_1.z
                .enum(['doc', 'sheet', 'file', 'wiki', 'bitable', 'docx', 'folder', 'mindnote', 'minutes', 'slides'])
                .describe('文件类型，需要与文件的 token 相匹配 Options:doc(文档),sheet(电子表格),file(云空间文件),wiki(知识库节点),bitable(多维表格),docx(新版文档),folder(文件夹),mindnote(思维笔记),minutes(妙记),slides(幻灯片)'),
            member_type: zod_1.z
                .enum(['email', 'openid', 'openchat', 'opendepartmentid', 'userid', 'unionid', 'groupid', 'wikispaceid'])
                .describe('协作者 ID 类型，与协作者 ID 需要对应 Options:email(邮箱地址),openid(开放平台 ID),openchat(OpenChatID 开放平台群组 ID),opendepartmentid(开放平台部门 ID),userid(用户自定义 ID),unionid(开放平台 UnionID),groupid(自定义用户组 ID),wikispaceid(知识空间 ID - **注意**：仅知识库文档支持该参数，当需要操作知识库文档里的「知识库成员」类型协作者时传该参数)'),
        }),
        path: zod_1.z.object({
            token: zod_1.z.string().describe('文件的 token，获取方式见 '),
            member_id: zod_1.z.string().describe('协作者 ID，与协作者 ID 类型需要对应'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
    ...
    schema: {
        data: zod_1.z.object({
            member_type: zod_1.z
                ...
                .describe('文件所有者的 ID 类型 Options:email(飞书邮箱),openid(open_id 开放平台ID),userid(user_id 用户自定义ID)'),
        ...
        params: zod_1.z.object({
            type: zod_1.z
                ...
                .describe('文件类型，需要与文件的 token 相匹配 Options:doc(文档),sheet(电子表格),file(云空间文件),wiki(知识库节点),bitable(多维表格),docx(新版文档),mindnote(思维笔记),minutes(妙记),slides(幻灯片),folder(文件夹)'),
            need_notification: zod_1.z.boolean().describe('是否需要通知新的文件所有者').optional(),
            remove_old_owner: zod_1.z.boolean().describe('转移后是否需要移除原文件所有者的权限').optional(),
            stay_put: zod_1.z
                .boolean()
                .describe('仅当文件在个人文件夹下，此参数才会生效。如果设为`false`，系统会将该内容移至新所有者的空间下。如果设为`true`，则留在原位置')
                .optional(),
...
exports.driveV1PermissionMemberUpdate = {
    ...
    schema: {
        data: zod_1.z.object({
            member_type: zod_1.z
                .enum(['email', 'openid', 'unionid', 'openchat', 'opendepartmentid', 'userid', 'groupid', 'wikispaceid'])
                .describe('协作者 ID 类型，与协作者 ID 需要对应 Options:email(飞书邮箱),openid(open_id 开放平台 ID),unionid(union_id 开放平台 UnionID),openchat(open_chat 开放平台群组 ID),opendepartmentid(open_department_id 开放平台部门 ID),userid(user_id 用户自定义 ID),groupid(group_id 自定义用户组 ID),wikispaceid(wiki_space_id 知识空间 ID - **注意**：仅知识库文档支持该参数，当需要操作知识库文档里的「知识库成员」类型协作者时传该参数)'),
            perm: zod_1.z
                .enum(['view', 'edit', 'full_access'])
                .describe('协作者对应的权限角色 **注意：** 妙记还不支持可管理角色 Options:view(可阅读角色),edit(可编辑角色),full_access(可管理角色)'),
            perm_type: zod_1.z
...
exports.driveV1PermissionPublicPasswordCreate = {
    ...
    schema: {
        params: zod_1.z.object({
            type: zod_1.z
                .enum(['doc', 'sheet', 'file', 'wiki', 'bitable', 'docx', 'mindnote', 'minutes', 'slides'])
                .describe('文件类型，需要与文件的 token 相匹配 Options:doc(文档),sheet(电子表格),file(云空间文件),wiki(知识库节点),bitable(多维表格),docx(新版文档),mindnote(思维笔记),minutes(妙计（暂不支持）),slides(幻灯片)'),
        }),
        path: zod_1.z.object({
            token: zod_1.z.string().describe('文件的 token，获取方式见 ').optional(),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
...
exports.driveV1PermissionPublicPatch = {
    ...
    schema: {
        data: zod_1.z.object({
            external_access: zod_1.z.boolean().describe('允许内容被分享到组织外').optional(),
            ...
            share_entity: zod_1.z
                ...
                .describe('谁可以添加和管理协作者 Options:anyone(所有可阅读或编辑此文档的用户),same_tenant(SameTenant 组织内所有可阅读或编辑此文档的用户),only_full_access(OnlyFullAccess 拥有可管理权限（包括我）的用户)')
                .optional(),
            link_share_entity: zod_1.z
                .enum(['tenant_readable', 'tenant_editable', 'anyone_readable', 'anyone_editable', 'closed'])
                .describe('链接分享设置 Options:tenant_readable(TenantReadable 组织内获得链接的人可阅读),tenant_editable(TenantEditable 组织内获得链接的人可编辑),anyone_readable(AnyoneReadable 互联网上获得链接的任何人可阅读),anyone_editable(AnyoneEditable 互联网上获得链接的任何人可编辑),closed(关闭链接分享)')
                .optional(),
            invite_external: zod_1.z.boolean().describe('允许非「可管理权限」的人分享到组织外').optional(),
        }),
        params: zod_1.z.object({
            type: zod_1.z
                .enum(['doc', 'sheet', 'file', 'wiki', 'bitable', 'docx', 'mindnote', 'minutes', 'slides'])
