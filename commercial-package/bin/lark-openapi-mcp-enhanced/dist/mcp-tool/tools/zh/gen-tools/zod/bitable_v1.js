    ...
    schema: {
        data: zod_1.z.object({
            ...
            folder_token: zod_1.z
                .string()
                .describe('了解如何获取文件夹 Token，参考。**注意**：请确保调用身份拥有在该文件夹中的编辑权限。若应用使用的是 `tenant_access_token` 权限，此处仅可指定应用创建的文件夹。详情参考')
                .optional(),
            without_content: zod_1.z
                .boolean()
                .describe('是否复制多维表格中的内容，默认 false，即复制多维表格中的内容。可取值：* true：不复制* false：复制')
                .optional(),
            time_zone: zod_1.z.string().describe('文档时区，详情参考').optional(),
        }),
...
exports.bitableV1AppCreate = {
    ...
    schema: {
        data: zod_1.z.object({
            ...
            folder_token: zod_1.z
                .string()
                .describe('多维表格 App 归属文件夹。默认为空，表示多维表格将被创建在云空间根目录。了解如何获取文件夹 Token，参考。**注意**：请确保调用身份拥有在该文件夹中的编辑权限。若应用使用的是 `tenant_access_token` 权限，此处仅可指定应用创建的文件夹。详情参考')
                .optional(),
            time_zone: zod_1.z.string().describe('文档时区，详情参考').optional(),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
...
exports.bitableV1AppRoleCreate = {
    ...
    schema: {
                ...
                rec_rule: zod_1.z
                    ...
                    conditions: zod_1.z
                        ...
                        operator: zod_1.z
                            ...
                            .optional(),
                        value: zod_1.z.array(zod_1.z.string()).describe('条件的值，可以是单个值或多个值的数组。详情参考').optional(),
                    }))
        ...
        path: zod_1.z.object({ app_token: zod_1.z.string().describe('多维表格 App 的唯一标识').optional() }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
    ...
    schema: {
        data: zod_1.z.object({
            member_list: zod_1.z
                ...
                type: zod_1.z
                    .enum(['open_id', 'union_id', 'user_id', 'chat_id', 'department_id', 'open_department_id'])
                    .describe('协作者 ID 类型 Options:open_id(OpenID 以 open_id 来识别协作者。获取方式参考),union_id(UnionID 以 union_id 来识别协作者。获取方式参考),user_id(UserID 以 user_id 来识别协作者。获取方式参考),chat_id(ChatID 以 chat_id 来识别协作者。获取方式参考),department_id(DepartmentID 以 department_id 来识别协作者。调用前，请确保应用有部门的可见性，参考。获取 department_id 方式参考),open_department_id(OpenDepartmentID 以 open_department_id 来识别协作者。调用前，请确保应用有部门的可见性，参考。获取 open_department_id 方式参考)')
                    .optional(),
                id: zod_1.z.string().describe('协作者的 ID，需与 type 的类型需一致。获取 ID 方式参考 type 参数描述'),
            }))
                .describe('协作者列表'),
...
exports.bitableV1AppTableBatchCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云文档-多维表格-数据表-新增多个数据表-新增多个数据表，仅可指定数据表名称。最多支持新增 100 个数据表',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            tables: zod_1.z
                ...
                name: zod_1.z
                    .string()
                    .describe('数据表名称。**注意**：- 名称中的首尾空格将会被默认去除。- 数据表名称不可以包含 `/ \\ ? * : [ ]` 等特殊字符')
                    .optional(),
            }))
                .describe('tables')
                .optional(),
...
exports.bitableV1AppTableBatchDelete = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云文档-多维表格-数据表-删除多个数据表-删除多个数据表。如果多维表格中只剩最后一张数据表，则不允许被删除',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            table_ids: zod_1.z
                .array(zod_1.z.string())
                .describe('待删除的数据表的 ID。当前一次操作最多支持 50 个数据表。获取方式：- 你可通过多维表格 URL 获取 `table_id`，下图高亮部分即为当前数据表的 `table_id`。- 也可通过接口获取 `table_id`。 ')
                .optional(),
        }),
        path: zod_1.z.object({ app_token: zod_1.z.string().describe('多维表格 App 的唯一标识') }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
...
exports.bitableV1AppTableCreate = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云文档-多维表格-数据表-新增一个数据表-新增一个数据表，默认仅包含索引字段，也可以指定更多字段。最多支持新增 100 个数据表',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            table: zod_1.z
                ...
                default_view_name: zod_1.z
                    .string()
                    .describe('默认表格视图的名称，不填则默认为“表格视图 1”。注意：- 名称中的首尾空格将会被去除。- 名称中不允许包含 [ ] 两个字符')
                    .optional(),
                fields: zod_1.z
                    .array(zod_1.z.object({
                    ...
                    property: zod_1.z
                        ...
                        options: zod_1.z
                            ...
                            color: zod_1.z.number().describe('选项颜色，详情参考').optional(),
                        }))
                            .describe('单选、多选字段的选项信息')
                            .optional(),
                        formatter: zod_1.z.string().describe('数字、公式字段的显示格式。详情参考').optional(),
                    ...
                    .describe('数据表的初始字段。了解如何填写字段，参考。**注意**：- 如果 `default_view_name` 字段和 `fields` 字段都不填写，将会创建一个仅包含索引字段的空数据表。- 如果指定了本字段，将会创建一个包含初始字段的数据表、且默认第一个字段为索引字段。- 索引字段仅支持以下类型： - 1：多行文本 - 2：数字 - 5：日期 - 13：电话号码 - 15：超链接 - 20：公式 - 22：地理位置')
                    .optional(),
            })
                .describe('数据表')
                .optional(),
...
exports.bitableV1AppTableDelete = {
    ...
    httpMethod: 'DELETE',
    description: '[Feishu/Lark]-云文档-多维表格-数据表-删除一个数据表-删除一个数据表，如果多维表格中只剩最后一张数据表，则不允许被删除',
    accessTokens: ['tenant', 'user'],
    schema: {
        path: zod_1.z.object({
            app_token: zod_1.z.string().describe('多维表格 App 的唯一标识'),
        ...
        data: zod_1.z.object({
            ...
            property: zod_1.z
                ...
                options: zod_1.z
                    ...
                    .describe('单选、多选字段的选项信息')
                    .optional(),
                formatter: zod_1.z.string().describe('数字和公式字段的显示格式。详情参考').optional(),
                date_formatter: zod_1.z
                    .string()
                ...
                location: zod_1.z
                    ...
                    .describe('地理位置输入方式')
                    .optional(),
                formula_expression: zod_1.z.string().describe('公式字段的表达式。参考了解如何设置公式').optional(),
                allowed_edit_modes: zod_1.z
                    .object({
                ...
                type: zod_1.z
                    ...
                    ui_type: zod_1.z
                        ...
                        .describe('公式字段在界面上的展示类型，例如进度字段是数字的一种展示形态。了解更多，参考。 Options:Number(数字),Progress(进度),Currency(货币),Rating(评分),DateTime(日期)')
                        .optional(),
                })
                    .describe('设置公式字段的数据类型**注意**：非所有多维表格都支持该能力。请参考接口返回的formula_type 判断，当 `formula_type` 等于 2 时，表示需要设置该字段')
                    .optional(),
            })
            ...
            ui_type: zod_1.z
                ...
                .describe('字段在界面上的展示类型，例如进度字段是数字的一种展示形态。了解更多，参考。 Options:Text(文本),Email(邮箱地址),Barcode(条码),Number(数字),Progress(进度),Currency(货币),Rating(评分),SingleSelect(单选),MultiSelect(多选),DateTime(日期),Checkbox(复选框),User(人员),GroupChat(群组),Phone(电话号码),Url(超链接),Attachment(附件),SingleLink(单向关联),Formula(公式),DuplexLink(双向关联),Location(地理位置),CreatedTime(创建时间),ModifiedTime(最后更新时间),CreatedUser(创建人),ModifiedUser(修改人),AutoNumber(自动编号)')
                .optional(),
        }),
        params: zod_1.z.object({
        ...
        data: zod_1.z.object({
            ...
            property: zod_1.z
                ...
                options: zod_1.z
                    ...
                    .optional(),
                formatter: zod_1.z.string().describe('数字、公式字段的显示格式。详情参考').optional(),
                date_formatter: zod_1.z.string().describe('日期、创建时间、最后更新时间字段的显示格式。详情参考').optional(),
                auto_fill: zod_1.z.boolean().describe('日期字段中新纪录自动填写创建时间').optional(),
...
exports.bitableV1AppTableFormFieldList = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            ...
            form_id: zod_1.z
                .string()
                .describe('多维表格中表单的唯一标识。表单也是视图的一种，其获取方式与获取 `view_id` 相同：- 在多维表格的 URL 地址栏中，`form_id` 是下图中高亮部分： - 通过接口获取。暂时无法获取到嵌入到云文档中的多维表格的 `form_id`'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
...
exports.bitableV1AppTableRecordBatchDelete = {
    ...
    httpMethod: 'POST',
    description: '[Feishu/Lark]-云文档-多维表格-记录-删除多条记录-删除多维表格数据表中现有的多条记录，单次调用中最多删除 500 条记录',
    accessTokens: ['tenant', 'user'],
    schema: {
        data: zod_1.z.object({
            records: zod_1.z.array(zod_1.z.string()).describe('删除的多条记录 ID 列表。通过接口获取'),
        }),
        path: zod_1.z.object({
            app_token: zod_1.z.string().describe('多维表格 App 的唯一标识'),
            table_id: zod_1.z.string().describe('多维表格数据表的唯一标识'),
...
exports.bitableV1AppTableRecordBatchGet = {
    ...
    schema: {
        data: zod_1.z.object({
            record_ids: zod_1.z.array(zod_1.z.string()).describe('记录 ID 列表。调用获取'),
...
exports.bitableV1AppTableRecordList = {
    ...
    schema: {
        params: zod_1.z.object({
            ...
            filter: zod_1.z
                .string()
                .describe('筛选参数，用于指定本次查询的筛选条件注意：1.不支持对“人员”以及“关联字段”的属性进行过滤筛选，如人员的 OpenID。2.指定筛选条件时，参数长度不超过2000个字符。详细请参考')
                .optional(),
            sort: zod_1.z
                .string()
        ...
        data: zod_1.z.object({
            ...
            filter: zod_1.z
                ...
                conditions: zod_1.z
                    ...
                    value: zod_1.z
                        .array(zod_1.z.string())
                        .describe('条件的值，可以是单个值或多个值的数组。不同字段类型和不同的 operator 可填的值不同。详情参考')
                        .optional(),
                }))
                    .describe('筛选条件集合')
                    .optional(),
            })
                .describe('包含条件筛选信息的对象。了解 filter 填写指南和使用示例（如怎样同时使用 `and` 和 `or` 逻辑链接词），参考')
                .optional(),
            automatic_fields: zod_1.z
                .boolean()
...
exports.bitableV1AppWorkflowUpdate = {
    ...
    schema: {
        ...
        path: zod_1.z.object({
            app_token: zod_1.z.string().describe('多维表格 App 的唯一标识').optional(),
            workflow_id: zod_1.z.string().describe('自动化工作流 ID，通过接口获取'),
        }),
        useUAT: zod_1.z.boolean().describe('使用用户身份请求, 否则使用应用身份').optional(),
    },
};
