...
class Gemini3AdaptiveRuntimeServer {
    ...
    // Tool Handlers
    async handleGenerateDynamicUI(args) {
    ...
    async handleIterateUI(args) {
        const response = await this.uiGenerator.iterateUI(args.originalCode, args.feedback, args.thinkingLevel);
        return {
            content: [
                {
                    type: 'text',
    ...
    async handleDeepReasoning(args) {
    ...
    async handleCompareOptions(args) {
        const response = await this.reasoningEngine.compareOptions(args.question, args.options, args.criteria);
        return {
            content: [
                {
                    type: 'text',
    ...
    async handleAnalyzeDecision(args) {
        const response = await this.reasoningEngine.analyzeDecision(args.decision, args.context, args.timeHorizon);
        return {
            content: [
                {
                    type: 'text',
    ...
    async handleAnalyzeCode(args) {
        const response = await this.codeExecutor.analyzeCode(args.code, args.language, args.analysisGoals);
        return {
            content: [
                {
                    type: 'text',
    ...
    async handleGenerateTests(args) {
        const response = await this.codeExecutor.generateTests(args.code, args.language, args.testFramework);
        return {
            content: [
                {
                    type: 'text',
    ...
    async handleSolveAlgorithm(args) {
        const response = await this.codeExecutor.solveAlgorithm(args.problem, args.constraints, args.language);
        return {
            content: [
                {
                    type: 'text',
