...
async function fetchCloudRules() {
    ...
    try {
        ...
        return response.data.rules.map((r) => ({
            ...r,
            source: "cloud",
        }));
    }
 ...
 */
async function validateTask(taskDescription, ruleIds) {
        ...
        switch (rule.id) {
            ...
            case "P0.2": // Inter-Agent Communication Protocol
                if (taskDescription.includes("tmux send-keys") && !taskDescription.includes("sleep 0.5")) {
                    ...
                    suggestions.push("Use: tmux send-keys -t <PANE_ID> \"<MESSAGE>\" && sleep 0.5 && tmux send-keys -t <PANE_ID> Enter");
                }
                break;
            ...
            case "P1.2": // Context7 for External Libraries
                if (taskDescription.match(/\b(axios|express|react|vue)\b/i) &&
...
async function syncRulesToCloud() {
    ...
    for (const rule of localRules) {
        try {
            await axios.post(`${MIYABI_CLOUD_API_URL}/api/rules`, rule, {
                headers: {
                    Authorization: `Bearer ${MIYABI_API_KEY}`,
                },
                timeout: 5000,
    ...
    try {
        switch (name) {
            ...
            case "miyabi_rules_validate": {
                const { task_description, rule_ids } = args;
