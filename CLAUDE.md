...
#### P0.4 - Spec-Driven Development (仕様駆動開発)
```
コード実装前に、必ず仕様を日本語で詳細に定義する
AIが予期しないコードを書かせない
```
- **Step 1**: Issue/タスクの要件を日本語でプログラムレベルまで詳細化
- **Step 2**: 入力・出力・エッジケースを明確に定義
- **Step 3**: 設計パターン（オブジェクト指向/デザインパターン）を事前決定
- **Step 4**: 仕様が固まってから初めてCodeGenAgentを実行
- **禁止**: 「とりあえず実装して後で直す」アプローチ

#### P0.5 - Minimal Code Principle (最小コード原則)
```
必要最小限のコードのみ生成する
冗長なコード、過剰な抽象化を禁止
```
- 不要なコメント・docstringを書かない
- 使われない可能性のあるコードを書かない
- 1つの機能 = 1つの責務（Single Responsibility）
- 設計に沿ったコードのみ生成
