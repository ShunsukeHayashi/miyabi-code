...
/**
 * Gemini 3 Adaptive Runtime HTTP MCP Server
 *
 * HTTP/SSE version for ChatGPT integration via ngrok tunneling
 */
class Gemini3HttpServer {
  ...
  constructor() {
    ...
    this.server = new Server(
      {
        name: 'gemini3-adaptive-runtime-http',
        version: '1.0.0',
      },
      {
        capabilities: {
  ...
  private setupRoutes(): void {
    ...
    this.app.get('/health', (req, res) => {
      res.json({
        status: 'ok',
        server: 'gemini3-adaptive-runtime',
    ...
    this.app.post('/execute/:toolName', async (req, res) => {
      try {
        ...
        switch (toolName) {
          case 'generate_dynamic_ui':
            result = await this.handleGenerateDynamicUI(args);
            break;
          case 'iterate_ui':
            result = await this.handleIterateUI(args);
            break;
          case 'deep_reasoning':
            result = await this.handleDeepReasoning(args);
            break;
          case 'compare_options':
            result = await this.handleCompareOptions(args);
            break;
          case 'analyze_decision':
            result = await this.handleAnalyzeDecision(args);
            break;
          case 'execute_code':
            result = await this.handleExecuteCode(args);
            break;
          case 'analyze_code':
          ...
          case 'generate_tests':
            ...
            break;
          case 'solve_algorithm':
            result = await this.handleSolveAlgorithm(args);
            break;
          default:
  ...
  // Tool Handlers (same as the original server)
  ...
  async start(port: number = 3000): Promise<void> {
    return new Promise((resolve) => {
      this.app.listen(port, () => {
        console.log(`Gemini 3 Adaptive Runtime HTTP Server running on port ${port}`);
        console.log(`Health check: http://localhost:${port}/health`);
        console.log(`Tools list: http://localhost:${port}/tools`);
        console.log(`SSE endpoint: http://localhost:${port}/sse`);
        console.log(`Model: ${this.client.getModelInfo()}`);
        resolve();
      });
    });
  }
...
const server = new Gemini3HttpServer();
