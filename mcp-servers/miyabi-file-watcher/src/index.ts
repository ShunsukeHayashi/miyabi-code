 ...
 */
async function searchFiles(pattern: string, directory?: string): Promise<any> {
  const searchDir = directory ? join(BASE_DIR, directory) : BASE_DIR;
 ...
 */
async function getDirectoryTree(directory?: string, maxDepth?: number): Promise<any> {
 ...
 */
async function compareFiles(path1: string, path2: string): Promise<any> {
  ...
  try {
    ...
    switch (name) {
      ...
      case 'file_changes_since': {
        ...
        const data = await getChangesSince(
          validated.since,
          validated.directory,
          validated.pattern
        );
