 ...
 */
async function getProcessInfo(pid: number): Promise<any> {
 ...
 */
async function getProcessList(sortBy?: string, limit?: number): Promise<any> {
  const processes = await si.processes();
 ...
 */
async function searchProcesses(query: string, limit?: number): Promise<any> {
  const processes = await si.processes();
  const queryLower = query.toLowerCase();

  const matches = processes.list.filter(
    (p) =>
 ...
 */
async function getProcessTree(): Promise<any> {
  try {
    ...
    const buildTree = (pid: number, depth = 0): any => {
      ...
      return {
        ...
        children: childPids.map((childPid: number) => buildTree(childPid, depth + 1)).filter(Boolean),
      };
    };
 ...
 */
async function getProcessFileDescriptors(pid: number): Promise<any> {
  try {
 ...
 */
async function getProcessEnvironment(pid: number): Promise<any> {
  try {
 ...
 */
async function getProcessChildren(pid: number): Promise<any> {
  const processes = await si.processes();
 ...
 */
async function getTopProcesses(limit: number = 10): Promise<any> {
  const processes = await si.processes();
