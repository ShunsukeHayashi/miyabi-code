 ...
 */
async function execCodex(args: string[]): Promise<string> {
  try {
...
async function codexExecYolo(
  ...
  if (!confirmDanger) {
    throw new Error(
      '⚠️ YOLO mode is EXTREMELY DANGEROUS! It runs all commands without approval or sandboxing. ' +
  ...
  const warning =
    '🚨 WARNING: Running codex in --yolo mode!\n' +
 ...
 */
async function codexResume(cd?: string): Promise<string> {
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
  tools: [
    ...
    },
    {
      ...
      description:
        '🚨 DANGEROUS: Execute codex in YOLO mode (NO approvals, NO sandboxing). Only use in isolated environments!',
      inputSchema: {
        type: 'object',
        properties: {
          prompt: {
  ...
  try {
    ...
    switch (name) {
      ...
      case 'codex_exec_yolo': {
        ...
        result = await codexExecYolo(
          validated.prompt,
          validated.model,
          validated.cd,
          validated.confirm_danger
        );
        break;
      }
