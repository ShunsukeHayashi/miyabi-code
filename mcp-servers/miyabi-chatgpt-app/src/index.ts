...
server.tool("get_project_status", "Get current project status", async () => {
  if (!session.owner || !session.repo) {
    return {
      content: [{ type: "text" as const, text: "No project selected. Use switch_project first." }]
    };
  }
  return {
    content: [{ type: "text" as const, text: `Current project: ${session.owner}/${session.repo}` }]
  };
});
...
server.tool(
  "list_repos",
  { filter: z.string().optional().describe("Filter repos by name") },
  async (args) => {
    if (!session.githubToken) {
      return {
        content: [{ type: "text" as const, text: "❌ GitHub認証が必要です。github_auth_start を使って認証を開始してください。" }]
      };
    }
    ...
    try {
      ...
      if (response.status === 401) {
        ...
        return {
          content: [{ type: "text" as const, text: "❌ トークンが無効です。github_auth_start で再認証してください。" }]
        };
      }
...
server.tool(
  ...
  async (args) => {
    ...
    return {
      content: [{ type: "text" as const, text: `✅ Switched to ${args.owner}/${args.repo}` }]
    };
  }
);
