...
interface NotificationPayload {
  channel: 'slack' | 'discord' | 'line' | 'webhook';
  message: string;
  data?: AnalysisResult;
}
...
const alertHistory: AnalysisResult[] = [];
...
function determineTrend(price: number, sma20: number, sma50: number, sma200: number): string {
...
async function analyzeSymbol(symbol: string, alertType: string): Promise<AnalysisResult> {
  // Get current quote
...
function formatSlackMessage(result: AnalysisResult): object {
...
function formatDiscordMessage(result: AnalysisResult): object {
  const priceEmoji = result.quote.change >= 0 ? '📈' : '📉';
  const changeSign = result.quote.change >= 0 ? '+' : '';
...
function formatLineMessage(result: AnalysisResult): string {
  const priceEmoji = result.quote.change >= 0 ? '📈' : '📉';
  const changeSign = result.quote.change >= 0 ? '+' : '';
...
function parseTradingViewAlert(body: any): TradingViewAlert {
  ...
  if (typeof body === 'string') {
    ...
    if (body.toLowerCase().includes('buy')) alert.action = 'buy';
    else if (body.toLowerCase().includes('sell')) alert.action = 'sell';
...
app.get('/alerts/:symbol', (req: Request, res: Response) => {
