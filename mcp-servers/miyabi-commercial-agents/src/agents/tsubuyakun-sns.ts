...
export class TsubuyakunAgent {
   ...
   * Twitter API設定の読み込み
   */
  private loadTwitterConfig(): TwitterApiConfig | null {
   ...
   * X投稿を生成（AI最適化）
   */
  async generateXPost(params: XPostParams): Promise<XPostResult> {
    const {
      content,
      topic,
      style = 'engaging',
   ...
   * Xに投稿を実行
   */
  async postToX(params: XPostParams): Promise<XPostResult> {
    // まず投稿を生成
    const postResult = await this.generateXPost(params);

    if (!this.twitterConfig) {
      throw new Error(
        'Twitter API credentials not configured. Set TWITTER_API_KEY, TWITTER_API_SECRET, TWITTER_ACCESS_TOKEN, TWITTER_ACCESS_SECRET, and TWITTER_BEARER_TOKEN environment variables.'
      );
    }
   ...
   */
  private async sendTweet(text: string): Promise<string> {
    if (!this.twitterConfig) {
    ...
    // 署名生成
    const signature = await this.generateOAuthSignature(
      method,
      url,
      oauthParams,
      this.twitterConfig.apiSecret,
      this.twitterConfig.accessSecret
    );
   ...
   * Xアカウント分析
   */
  async analyzeXAccount(params: XAccountAnalysisParams): Promise<XAccountAnalysis> {
  ...
  private async fetchAccountInfo(
    username: string
  ): Promise<XAccountAnalysis['account']> {
  ...
  private analyzeContentPatterns(
    username: string,
    depth: string
  ): XAccountAnalysis['content_analysis'] {
    // 独自アルゴリズム：投稿パターン分析
  ...
  private analyzeEngagementPatterns(
    followers: number
  ): XAccountAnalysis['engagement_patterns'] {
  ...
  private generateAccountRecommendations(
    contentAnalysis: XAccountAnalysis['content_analysis'],
    engagementPatterns: XAccountAnalysis['engagement_patterns'],
    depth: string
  ): XAccountAnalysis['recommendations'] {
    ...
    if (depth === 'comprehensive') {
      growthTactics.push(
        'ニュースレター連携でクロスプロモーション',
        'Spaces開催で音声でのエンゲージメント',
        'コミュニティ作成でコアファン育成'
      );
    }
   ...
   * 投稿内容生成（スタイル別）
   */
  private generatePostContent(topic: string, style: string, language: string): string {
   ...
   * 最適ハッシュタグ生成
   */
  private generateOptimalHashtags(content: string, language: string): string[] {
  ...
  private predictEngagement(
    content: string,
    hashtagCount: number
  ): XPostResult['engagement_prediction'] {
    ...
    // スコア計算
    let score = 50;
    if (contentLength > 100 && contentLength < 250) score += 10;
    if (hasQuestion) score += 15;
    if (hasEmoji) score += 5;
    if (hashtagCount >= 2 && hashtagCount <= 3) score += 10;
   ...
   * 最適投稿時間取得
   */
  private getOptimalPostingTime(language: string): string {
    const hour = new Date().getHours();

    if (language === 'ja') {
      if (hour < 7) return '7:00-8:00 (朝の通勤時間)';
      if (hour < 12) return '12:00-13:00 (ランチタイム)';
      if (hour < 18) return '18:00-19:00 (帰宅時間)';
      return '21:00-22:00 (夜のリラックスタイム)';
    } else {
      if (hour < 9) return '9:00-10:00 AM (Morning commute)';
      if (hour < 13) return '12:00-1:00 PM (Lunch break)';
      if (hour < 18) return '5:00-6:00 PM (After work)';
