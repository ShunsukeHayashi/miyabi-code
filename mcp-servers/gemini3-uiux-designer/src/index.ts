...
class Gemini3UXDesignerServer {
  ...
  private client: Gemini3UXClient;
  ...
  // Tool Handlers
  private async handleReviewDesign(args: any) {
    const response = await this.designReviewer.reviewDesign(
      args.designCode,
  ...
  private async handleGenerateDesignSystem(args: any) {
    const response = await this.designSystemGenerator.generateDesignSystem(
      args.projectName,
      args.brandIdentity,
      args.accentColor
    );
  ...
  private async handleCreateWireframe(args: any) {
...
Focus on layout structure, component hierarchy, and user flow. No visual styling.`;
  ...
  private async handleGenerateHiFiMockup(args: any) {
...
Provide detailed compliance report.`;
  ...
  private async handleAnalyzeUsability(args: any) {
...
Identify friction points and provide solutions.`;
  ...
  private async handleOptimizeUXWriting(args: any) {
...
Provide optimized version with rationale.`;
  ...
  private async handleDesignInteractionFlow(args: any) {
...
Provide complete interaction specification.`;
  ...
  private async handleCreateAnimationSpecs(args: any) {
...
Provide Tailwind CSS and Framer Motion code.`;
  ...
  private async handleEvaluateConsistency(args: any) {
    const designsList = args.designs.map((d: any) => `${d.name}:\n\`\`\`tsx\n${d.code}\n\`\`\``).join('\n\n');
