// MCP Apps SDK Handler for ChatGPT Integration
...
// MCP Apps SDK ハンドラー
export async function handleMcpAppsRequest(req: Request, res: Response) {
...
// ツール呼び出しハンドラー
async function handleToolCall(req: Request, res: Response, params: any) {
...
// エージェント一覧
function handleListAgents(res: Response, args: any) {
...
${filteredAgents.map(a =>
  `| ${a.name} | ${a.category} | ${a.status === 'active' ? '✅ 稼働中' : '⏸️ 停止中'} | ${new Date(a.lastRun).toLocaleString('ja-JP')} |`
).join('\n')}
...
// エージェント詳細
function handleGetAgentDetails(res: Response, args: any) {
...
// tmuxセッション一覧
function handleListTmuxSessions(res: Response) {
  try {
...
|------------|------------|---------|------|
${sessions.map(s =>
  `| ${s.name} | ${s.windows} | ${s.created} | ${s.status} |`
).join('\n')}
`;
...
// tmuxセッション作成
function handleCreateTmuxSession(res: Response, args: any) {
...
// tmuxセッション終了
function handleKillTmuxSession(res: Response, args: any) {
  const { name } = args;
...
// デイリーレポート生成
function handleGenerateDailyReport(res: Response, args: any) {
...
// システムステータス
function handleGetSystemStatus(res: Response) {
  const activeAgents = MIYABI_AGENTS.filter(a => a.status === 'active').length;
  const totalAgents = MIYABI_AGENTS.length;
