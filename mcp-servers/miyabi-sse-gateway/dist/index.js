...
app.get('/mcp/status', bearerAuth, async (req, res) => {
    const router = getMCPRouter();
    const status = router.getStatus();
    res.json(status);
});
// MCP Router tools listing endpoint
app.get('/mcp/tools', bearerAuth, async (req, res) => {
    try {
        const router = getMCPRouter();
...
app.post('/mcp/tmux', apiLimiter, bearerAuth, promptInjectionGuard, async (req, res) => {
    try {
        const router = getMCPRouter();
...
app.post('/mcp/rules', apiLimiter, bearerAuth, promptInjectionGuard, async (req, res) => {
    try {
        const router = getMCPRouter();
        const { method, params } = req.body;
        if (method === 'tools/list') {
...
app.post('/mcp/obsidian', apiLimiter, bearerAuth, promptInjectionGuard, async (req, res) => {
    try {
        const router = getMCPRouter();
        const { method, params } = req.body;
        if (method === 'tools/list') {
...
app.post('/mcp', apiLimiter, bearerAuth, promptInjectionGuard, async (req, res) => {
...
async function startServer() {
    try {
        ...
        app.listen(PORT, () => {
            ...
            Object.entries(status.servers).forEach(([name, info]) => {
