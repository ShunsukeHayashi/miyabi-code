 ...
 */
async function listIssues(args: any) {
 ...
 */
async function getIssue(args: any) {
  const owner = args.owner || DEFAULT_OWNER;
  const repo = args.repo || DEFAULT_REPO;
...
async function updateIssue(args: any) {
  ...
  const updateData: any = { owner, repo, issue_number };
 ...
 */
async function listPRs(args: any) {
  const owner = args.owner || DEFAULT_OWNER;
  const repo = args.repo || DEFAULT_REPO;
  const state = args.state || "open";
  const per_page = args.per_page || 30;
 ...
 */
async function createPR(args: any) {
  const owner = args.owner || DEFAULT_OWNER;
  const repo = args.repo || DEFAULT_REPO;
  const title = args.title;
 ...
 */
async function mergePR(args: any) {
  const owner = args.owner || DEFAULT_OWNER;
  const repo = args.repo || DEFAULT_REPO;
  const pull_number = args.pull_number;
 ...
 */
async function listLabels(args: any) {
  const owner = args.owner || DEFAULT_OWNER;
  const repo = args.repo || DEFAULT_REPO;
 ...
 */
async function addLabels(args: any) {
  const owner = args.owner || DEFAULT_OWNER;
  const repo = args.repo || DEFAULT_REPO;
  const issue_number = args.issue_number;
  const labels = args.labels;
 ...
 */
async function listMilestones(args: any) {
  const owner = args.owner || DEFAULT_OWNER;
  const repo = args.repo || DEFAULT_REPO;
  const state = args.state || "open";
 ...
 */
async function devIssueCreate(args: any) {
  const title = args.title;
  const body = args.body || "";
  const labels = args.labels || [];
 ...
 */
async function devIssueList(args: any) {
  const status = args.status; // Optional filter by status

  await ensureDevIssuesDir();

  let devIssues: DevIssue[] = [];
 ...
 */
async function devIssueSync(args: any) {
  const owner = args.owner || DEFAULT_OWNER;
  const repo = args.repo || DEFAULT_REPO;
