 ...
 */
export function generateClaudeCodeSettings(catalog) {
    ...
    const deferredCategories = Object.entries(catalog.byCategory)
        ...
        const lowPriorityCount = tools.filter((t) => t.priority === "low" || t.priority === "medium").length;
 ...
 */
export function generateMCPConfig(catalog, existingConfig) {
    const mcpServers = {};
    // Get unique servers
    ...
    for (const server of servers) {
        ...
        const existing = existingConfig?.mcpServers?.[server] || {};
        ...
        mcpServers[server] = {
            ...
            toolConfig: {
                ...
                overrides: Object.fromEntries(serverTools
 ...
 */
export function generateClaudeMDSection(catalog) {
    const stats = {
 ...
 */
export async function writeAllConfigs(catalog, outputDir) {
    ...
    fs.writeFileSync(path.join(outputDir, "tool-search-settings.json"), JSON.stringify(settings, null, 2));
    // Write MCP config
    const mcpConfig = generateMCPConfig(catalog);
    fs.writeFileSync(path.join(outputDir, "tool-search-mcp.json"), JSON.stringify(mcpConfig, null, 2));
 ...
 */
export function mergeIntoExistingSettings(existingPath, catalog) {
    let existing = {};
