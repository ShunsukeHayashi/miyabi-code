...
interface CategoriesConfig {
  ...
  highPriorityServers: string[];
  lowPriorityServers: string[];
}
...
export class CatalogBuilder {
  ...
  private tools: ToolCatalogEntry[] = [];
   ...
   */
  async build(): Promise<ToolCatalog> {
   ...
   */
  private async collectMCPTools(): Promise<void> {
    ...
    for (const [serverName, config] of servers) {
      ...
      try {
        const tools = await this.getMCPServerTools(serverName, config as MCPServerConfig);
   ...
   */
  private getPredefinedTools(serverName: string): MCPToolDefinition[] {
    const predefinedTools: Record<string, MCPToolDefinition[]> = {
      "miyabi-github": [
   ...
   */
  private async collectRustAgentTools(): Promise<void> {
    ...
    const rustAgentTools: Array<{name: string; description: string; crate: string}> = [
   ...
   */
  private async collectSubagentTools(): Promise<void> {
    ...
    const subagentTools: Array<{name: string; description: string}> = [
   ...
   */
  private createToolEntry(params: {
    name: string;
    description: string;
   ...
   */
  private inferCategory(name: string, description: string): { category: string; subcategory?: string } {
   ...
   */
  private determinePriority(server: string, name: string): ToolPriority {
    // Always loaded tools
   ...
   */
  private generateAliases(name: string, description: string): string[] {
   ...
   */
  private createCatalog(): ToolCatalog {
    ...
    return {
      ...
      byId,
      byCategory,
      bySource: bySource as Record<ToolSource, string[]>,
      byPriority: byPriority as Record<ToolPriority, string[]>,
      byServer,
    };
  }
}
 ...
 */
export async function buildAndSaveCatalog(outputPath: string): Promise<ToolCatalog> {
