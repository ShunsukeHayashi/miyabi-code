 ...
 */
async function findLogFiles(): Promise<LogSource[]> {
  const patterns = [
    '**/logs/**/*.log',
    '**/*.log',
    '**/npm-debug.log',
 ...
 */
async function readLogFile(path: string, maxLines?: number): Promise<string[]> {
  try {
 ...
 */
function parseLogEntry(line: string, source: string, lineNumber: number): LogEntry | null {
 ...
 * Search logs
 */
async function searchLogs(query: string, source?: string, level?: string): Promise<LogEntry[]> {
  const sources = await findLogFiles();
  const filteredSources = source
    ? sources.filter((s) => s.name.includes(source))
    : sources;
 ...
 */
async function getErrors(minutes?: number): Promise<LogEntry[]> {
 ...
 */
async function getWarnings(minutes?: number): Promise<LogEntry[]> {
 ...
 * Tail log file
 */
async function tailLog(source: string, lines: number): Promise<string[]> {
  const sources = await findLogFiles();
