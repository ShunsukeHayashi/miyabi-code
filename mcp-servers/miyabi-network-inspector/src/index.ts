 ...
 */
async function getNetworkInterfaces(): Promise<any> {
 ...
 */
async function getNetworkConnections(): Promise<any> {
 ...
 */
async function getListeningPorts(protocol?: 'tcp' | 'udp' | 'all'): Promise<any> {
  try {
 ...
 * Ping a host
 */
async function pingHost(host: string): Promise<any> {
  try {
 ...
 */
async function getBandwidthUsage(): Promise<any> {
  const stats = await si.networkStats();
 ...
 */
async function getNetworkOverview(): Promise<any> {
  ...
  const activeInterfaces = interfaces.filter((iface: any) => iface.operstate === 'up' && !iface.internal);
  ...
  return {
    ...
    interfaces: {
      ...
      list: activeInterfaces.map((iface: any) => ({
        name: iface.name,
        type: iface.type,
        ip4: iface.ip4,
        speed: iface.speed,
    ...
    connections: {
      ...
      established: connections.filter((c: any) => c.state === 'ESTABLISHED').length,
    },
    listening_ports: {
      total: listeningPorts.length,
      tcp: listeningPorts.filter((p: any) => p.protocol === 'tcp').length,
      udp: listeningPorts.filter((p: any) => p.protocol === 'udp').length,
    },
