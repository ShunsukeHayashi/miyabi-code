...
このディレクトリには、**Git Worktree内でCodexが実行する際の具体的な手順書**が格納されています。
...
CoordinatorAgentがWorktreeを作成し、該当するプロンプトを自動的にCodexに渡します：
...
.worktrees/
└── issue-270/
    ├── .codex/agents/prompts/coding/
    │   └── codegen-agent-prompt.md  # ← Codexが読み込む
    └── [その他のファイル]
```
...
# Codexでプロンプトファイルを開く
claude .codex/agents/prompts/coding/codegen-agent-prompt.md
```
