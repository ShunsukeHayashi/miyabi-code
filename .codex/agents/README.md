...
│       └── analytics-agent.md
│
└── prompts/               # Worktree実行プロンプト（Codex用）
...
**Git Worktree内でCodexが実行する際の具体的な手順書**です。以下の情報を含みます：
...
| **prompts/coding/** | Codex（AI実行環境） | Worktree内で開発作業を実行する | Git Worktree並列実行時 |
| **prompts/business/** | Codex（AI実行環境） | Worktree内でビジネス分析を実行する | ビジネスAgent実行時（将来） |
...
- **/review コマンド**: `.codex/commands/review.md`
