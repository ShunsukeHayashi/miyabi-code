...
3. **Agent準備**: 各Agent仕様（`.codex/agents/specs/`）を確認
...
# 3. Agent仕様を確認
cat .codex/agents/specs/business/jonathan-ive-design-agent.md
cat .codex/agents/specs/coding/coordinator-agent.md
```
...
### Agent仕様
- **Coding Agents**: `.codex/agents/specs/coding/`
  - `coordinator-agent.md` - タスク統括・DAG分解
  - `codegen-agent.md` - コード生成
  - `review-agent.md` - 品質レビュー
  - `deployment-agent.md` - デプロイ自動化

- **Business Agents**: `.codex/agents/specs/business/`
  - `jonathan-ive-design-agent.md` - Jonathan Iveデザイン哲学
  - `product-design-agent.md` - プロダクトデザイン
  - その他14個のビジネスAgent

### 実行プロンプト
- `.codex/agents/prompts/coding/` - Agent実行プロンプト（6個）

### テンプレート
- `.codex/templates/reporting-protocol.md` - 報告プロトコル標準形式
- `.codex/prompts/task-management-protocol.md` - タスク管理ルール
...
3. **Agent仕様の確認**
   - 新しいAgentが必要な場合は `.codex/agents/specs/` に追加
   - 実行プロンプトを `.codex/agents/prompts/` に追加
...
```bash
# このインデックスを参照
cat .codex/agents/WORKFLOW_INDEX.md
