...
1. 利用可能なAgent一覧を確認:
```bash
cat .codex/agents/agent-name-mapping.json | jq
```
...
- これにより**すべてのGit操作がブロック**され、Codexセッションが正常に動作できない
...
```bash
#!/bin/bash
# .codex/scripts/cleanup-worktrees.sh
...
1. MCPサーバーのヘルスチェック:
```bash
.codex/mcp-servers/health-check.sh
```
...
**症状**:
```
Error: MCP server 'foo-mcp' not found in .codex/mcp.json
```
...
1. 利用可能なMCPサーバーを確認:
```bash
cat .codex/mcp.json | jq '.mcpServers | keys'
```
...
3. MCP設定確認:
```bash
cat .codex/mcp.json | jq '.mcpServers[] | select(.disabled == false) | keys'
```
...
- [README.md](README.md) - Codex統合ガイド
