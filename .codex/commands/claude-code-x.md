# Codex X - Autonomous Codex Executor

**Version**: 1.0.0
**Purpose**: Run Codex autonomously in background (like Codex X)

---

## 🎯 Overview

Codex Xは、Codex Xと同じインターフェースでCodexをバックグラウンド実行するツールです。

### Codex X vs Codex X

| Feature | Codex X | Codex X |
|---------|---------|---------------|
...
| Metric | Codex | Codex X | Codex X (Estimated) |
|--------|-------------|---------|---------------------------|
...
### Hybrid Strategy: Codex + Codex X

```bash
# Main session: Planning & Orchestration (Current Codex)
> "Let's implement Feature X. First, plan the tasks..."

# Background execution: Autonomous implementation (Codex X)
> /claude-code-x exec "Implement Feature X based on plan"

# While Codex X runs in background, continue main work
> "Now let's work on Feature Y..."

# Check Codex X status
...
```bash
#!/bin/bash
# .codex/commands/claude-code-x.sh
...
case "$COMMAND" in
  exec)
    ...
    echo "🚀 Starting Codex X session: $SESSION_ID"
...
- **Codex X Docs**: `.codex/commands/codex.md`
