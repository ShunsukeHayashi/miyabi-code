...
Pattern 3 Hybrid Orchestrationを起動するコマンド。Main Session（Codex）を維持しつつ、Codex XとCodex Xを**独立したWorktreeで**バックグラウンド並列実行し、生産性を最大化します。
...
1. **Main Task**: Codex（このセッション）で実行するメインタスク
2. **Codex X Task**: Zero-bug品質が必要なタスク（テスト、複雑アルゴリズム等）
3. **Codex X Task**: 高速実装タスク（ドキュメント、リファクタリング等）
...
```
Main (Codex): 統合・意思決定・レビュー
 ├── Background 1 (Codex X): Zero-bug品質コード
 └── Background 2 (Codex X): 高速ドキュメント
```
...
nohup claude code "<Codex X Task>" > /tmp/claude-code-x-log.txt 2>&1 &
CLAUDE_PID=$!
cd ../..
```
...
if [ $CLAUDE_STATUS -eq 0 ]; then
  git merge --no-ff feat/claude-x-issue-$ISSUE_NUMBER -m "feat: Codex X - <task description>"
...
Codex X Task: Message Queue統合ドキュメント作成
```
...
- **Codex X (Worktree)**: ドキュメント、リファクタリング、高速実装
...
- **Codex X**: `.codex/commands/claude-code-x.md`
- **Codex**: `.codex/commands/codex.md`
