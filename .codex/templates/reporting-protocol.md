# 報告プロトコル - Codex 標準テンプレート

## 目的

このテンプレートは、複数のCodexセッション（特にWorktreeベースの並列実行環境）において、どのAgentがどの作業を担当したかを明確にするための標準報告形式です。
...
### 1. 報告者の明記
- 全ての報告で「Codex」として名乗る
- 並列実行時はWorktree名も記載（例: `Codex (Worktree: issue-270)`）

### 2. 担当者の明記
- 各作業項目に「担当: Codex」または「実行者: Codex」を記載
- ユーザー指示のもと実行した場合は「実行者: Codex（ユーザー指示のもと）」

### 3. 確認者の明記
- 動作確認結果に「確認者: Codex」を記載
- テスト実行結果も含める

### 4. 提案者の明記
- 次のステップ提案に「提案者: Codex」を記載
- 提案は実行せず、ユーザーの指示を待つ

### 5. 報告終了の明記
- 報告の最後に「**報告終了** Codex」を記載
...
**このテンプレートは全てのCodexセッションで使用してください。**
