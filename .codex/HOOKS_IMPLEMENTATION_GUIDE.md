# Codex Hooks - Implementation Guide

**Version**: 1.0.0
**Last Updated**: 2025-10-22
**Target Audience**: Developers implementing Codex Hooks
...
|-------|------|-------|----------|
| **User** | `~/.codex/settings.json` | All projects | Global defaults |
| **Project** | `.codex/settings.json` | Current project, team-shared | Project-specific rules |
| **Local** | `.codex/settings.local.json` | Current project, personal | Personal overrides (gitignored) |

**Recommendation for Miyabi**:
- Use `.codex/settings.json` for team-shared hooks
- Use `.codex/settings.local.json` for personal experiments
    ...
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "python3 .codex/hooks/scripts/file-protection.py",
            "timeout": 60
          }
        ]
      }
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .codex/hooks/scripts/auto-format.sh",
            "timeout": 120
          }
        ]
      }
...
**Goal**: Log all Bash commands executed by Codex
    ...
    "Notification": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash .codex/hooks/scripts/notify.sh"
          }
        ]
      }
    ]
...
**Script**: `.codex/hooks/scripts/notify.sh`
...
if [[ "$OSTYPE" == "darwin"* ]]; then
  ...
  echo "[$(date)] Notification: $message" >> ~/.codex/notifications.log
fi
```
...
```bash
# Validate JSON
cat .codex/settings.json | jq . > /dev/null
echo "✅ JSON syntax valid"

# Validate command syntax (dry-run)
bash -n .codex/hooks/scripts/your-script.sh
...
**File**: `.codex/hooks/scripts/test-hooks.sh`

```bash
#!/bin/bash
set -euo pipefail

echo "🧪 Testing Codex Hooks..."
...
```bash
bash .codex/hooks/scripts/test-hooks.sh
```
...
3. Verify file permissions: `chmod +x .codex/hooks/scripts/*.sh`
    ...
    "Notification": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "osascript -e 'display notification \"Codex needs attention\" with title \"Miyabi\" sound name \"Glass\"' 2>/dev/null || notify-send 'Miyabi' 'Codex needs attention' 2>/dev/null || echo 'Notification' >> ~/.codex/notifications.log",
            "timeout": 30
          }
        ]
      }
    ...
    "SessionStart": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash .codex/hooks/scripts/session-start.sh"
          }
        ]
      }
    ],
    "UserPromptSubmit": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash .codex/hooks/scripts/context-injection.sh"
          }
        ]
      }
    ],
    "PreToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "bash .codex/hooks/scripts/bash-audit.sh"
          }
        ]
      },
      {
    ...
    "SessionEnd": [
      {
        "hooks": [
          {
            "type": "command",
            "command": "bash .codex/hooks/scripts/session-end.sh"
          }
        ]
      }
    ]
...
**Required Scripts** (`.codex/hooks/scripts/`):
...
2. **Test Thoroughly**: Run `.codex/hooks/scripts/test-hooks.sh`
3. **Iterate**: Add hooks incrementally
4. **Monitor**: Use `claude --debug` to verify execution
5. **Document**: Update `.codex/hooks/INDEX.md` with your hooks
...
- **Individual Triggers**: `.codex/hooks/triggers/*.md`
- **Hooks Index**: `.codex/hooks/INDEX.md`
- **Official Docs**: https://docs.codex.com/en/docs/claude-code/hooks
