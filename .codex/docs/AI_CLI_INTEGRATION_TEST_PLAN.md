# AI CLI統合テストプラン - Codex × Gemini CLI × Codex

## 概要

このドキュメントは、Miyabiプロジェクトで3つのAI CLIツール（Codex、Codex、Gemini CLI）が正しく連携動作することを確認するためのテストプランです。
...
**Codexへの指示**:
```markdown
これまでのテスト結果を、`.codex/templates/reporting-protocol.md`のテンプレートに従って報告してください。
```
...
### 最適化3.1: .codex/settings.json調整

**課題**: Codexセッションで特定のhooksが不要な可能性

**対応**:
- `.codex/settings.local.json`でCodexセッション専用設定を作成
- 軽量化されたhooks設定

**作成ファイル**: `.codex/settings.local.json`
```json
{
  "hooks": {
    "UserPromptSubmit": [],
...
**対応**: `.codex/CODEX_SESSION_README.md`作成
...
**対応**: `.codex/scripts/codex-miyabi.sh`作成
...
**ファイル**: `.codex/gemini-aliases.sh`

```bash
#!/bin/bash
# Gemini CLI Aliases for Miyabi Project

# Source this file in your shell: source .codex/gemini-aliases.sh
...
alias gm-agent='gemini "@.codex/agents/ 配下のAgentをリストアップして"'
```
...
**実行手順**:
1. **Codex**: ドキュメント作成 → `.codex/docs/NEW_FEATURE.md`
...
source .codex/scripts/codex-miyabi.sh
```

#### 問題: Codexが.envを読み込まない
**原因**: セッション起動時に.envが読み込まれていない
**解決策**: `.codex/scripts/codex-miyabi.sh`を使用
...
- **AI CLI比較**: `.codex/docs/AI_CLI_COMPARISON.md`
- **報告プロトコル**: `.codex/templates/reporting-protocol.md`
