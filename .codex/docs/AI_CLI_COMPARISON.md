# AI CLI比較 - Codex vs Gemini CLI

## 概要

このドキュメントは、MiyabiプロジェクトでCodexとGemini CLIを併用する際のガイドラインです。
...
- `.codex/commands/*.md` - カスタムコマンド
- `.codex/agents/` - Agent仕様・プロンプト
- `.codex/templates/` - 報告プロトコル等
...
### シナリオ1: Issue処理の全体統括（Codex）+ 個別ファイル修正（Gemini）
...
### シナリオ2: ドキュメント生成（Codex）+ レビュー（Gemini）

```bash
# Step 1: Codexで大規模ドキュメント生成
# （報告プロトコルに準拠した360行の仕様書）

# Step 2: Geminiでレビュー
gemini
> @.codex/templates/reporting-protocol.md このドキュメントを要約して、改善点を3つ提案して
```

### シナリオ3: Git操作（Codex）+ コード解説（Gemini）

```bash
# Step 1: Codexでコミット・PR作成
...
### Phase 2: 連携強化（v1.1.0）
- Codex → Gemini CLI: タスク移譲機能
- Gemini CLI → Codex: 実験結果のフィードバック
- 共通のタスクキュー（Redis等）
