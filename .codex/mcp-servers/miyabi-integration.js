 ...
 * Miyabi MCP Server
 *
 * Codex内でMiyabi CLIの全機能を直接呼び出せるMCPサーバー
 *
 * 提供ツール:
...
server.setRequestHandler(ListToolsRequestSchema, async () => {
  return {
    tools: [
      {
        name: 'miyabi__init',
        description: '新しいMiyabiプロジェクトを作成します。GitHub連携、Agent設定、Codex統合を含む完全なセットアップを実行します。',
        inputSchema: {
          type: 'object',
          properties: {
            projectName: {
      ...
      },
      {
        ...
        description: '既存プロジェクトにMiyabiをインストールします。.codex/、GitHub Actions、組織設計ラベル体系を追加します。',
        inputSchema: {
          type: 'object',
          properties: {
            dryRun: {
      ...
      },
      {
        ...
        description: '現在のプロジェクトのMiyabi/Codex統合状態を取得します（軽量・高速）',
        inputSchema: {
          type: 'object',
          properties: {},
        },
  ...
  try {
    switch (name) {
      ...
      case 'miyabi__get_status': {
        ...
        statusText += `Codex統合: ${status.hasCodex ? '✅ あり' : '❌ なし'}\n\n`;
