# Codex X - Autonomous Execution Subagent
...
Execute Codex autonomously in the background for well-defined tasks, enabling parallel execution of up to 5 concurrent sessions while the user continues other work.
...
### Core Features
- **Parallel Execution**: Run up to 5 Codex sessions concurrently
...
### Session Lifecycle
1. **Spawn**: Create new background Codex session
...
```
Please use the Codex X skill to execute the following task in the background:
...
```
Use Codex X to execute these 5 tasks in parallel:
...
```bash
# Execute task
./.codex/commands/claude-code-x.sh exec "Task description here"

# Optional: Custom configuration
./.codex/commands/claude-code-x.sh exec "Task description" \
  --timeout 900 \
...
```bash
# Check status
./.codex/commands/claude-code-x.sh status claude-code-x-20251027-123456-abc123

# List all sessions
./.codex/commands/claude-code-x.sh sessions
```
...
```bash
./.codex/commands/claude-code-x.sh result claude-code-x-20251027-123456-abc123
```
...
Phase 2 (interactive): Critical logic (use regular Codex)
...
```bash
./.codex/commands/claude-code-x.sh status <session-id>
```

### Get Full Results
```bash
./.codex/commands/claude-code-x.sh result <session-id>
```

### Kill Running Session
```bash
./.codex/commands/claude-code-x.sh kill <session-id>
```

### Cleanup Old Sessions
```bash
./.codex/commands/claude-code-x.sh cleanup
```
...
- **Execution**: Similar to interactive Codex
...
**Agent Action**:
```bash
./.codex/commands/claude-code-x.sh exec \
  "Add comprehensive logging to crates/miyabi-auth/src/lib.rs:
...
**Agent Response**:
```
✅ Started Codex X session: claude-code-x-20251027-100000-abc123
...
./.codex/commands/claude-code-x.sh exec "Add logging to crates/miyabi-auth/src/lib.rs"
./.codex/commands/claude-code-x.sh exec "Add logging to crates/miyabi-database/src/lib.rs"
./.codex/commands/claude-code-x.sh exec "Add logging to crates/miyabi-api/src/lib.rs"
./.codex/commands/claude-code-x.sh exec "Add logging to crates/miyabi-cache/src/lib.rs"
./.codex/commands/claude-code-x.sh exec "Add logging to crates/miyabi-worker/src/lib.rs"
```

**Agent Response**:
```
✅ Started 5 parallel Codex X sessions:
...
Phase 2: Infrastructure (Codex X - Parallel)
...
Phase 4: Providers (Codex X - Parallel)
...
- **Command Reference**: [.codex/commands/claude-code-x.md](../../commands/claude-code-x.md)
...
./.codex/commands/claude-code-x.sh exec "Complex task" --timeout 1200
```
...
Track these metrics to evaluate Codex X effectiveness:
